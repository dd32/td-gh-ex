<div class="content-wrapper">
    <h1 class="sidebar-title"><span>Editor Pick`s</span></h1>
        <div class="sidebar-posts-wrapper">            
            <div class="single_post clearfix first-post">
                    <div class="post-image"><a href="#">
                    <img src="<?php echo get_template_directory_uri();?>/images/demo-images/photo-1421986527537-888d998adb74-364x200.jpg" alt="" /></a></div>                                
                    <h3 class="post-title"><a href="#">Network technologies would give us more powerfull servers</a></h3>
                    <div class="block-poston"><span class="posted-on"><a href="#" rel="bookmark"><time class="entry-date published" datetime="2015-03-10T06:24:15+00:00">March 10, 2015</time><time class="updated" datetime="2015-03-16T12:00:45+00:00">March 16, 2015</time></a></span><span class="comment_count"><i class="fa fa-comments"></i>5</span><span class="apmag-post-views"><i class="fa fa-eye"></i>81</span></div>
                    <div class="block-poston"></div>
                    <div class="post-content"><p>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec</p></div>            
            </div>
                    <div class="single_post clearfix ">
                   
                    <div class="post-image"><a href="#"><img src="<?php echo get_template_directory_uri();?>/images/demo-images/photo-1421916780003-47d5a692d999-114x76.jpg" alt=""></a></div>                                
                <div class="post-desc-wrapper">                    
                    <h3 class="post-title"><a href="#">Basics of HDR Photography Tutorials HD</a></h3>
                    <div class="block-poston"><span class="posted-on"><a href="#" rel="bookmark"><time class="entry-date published" datetime="2015-03-08T09:38:36+00:00">March 8, 2015</time><time class="updated" datetime="2015-03-20T05:30:48+00:00">March 20, 2015</time></a></span><span class="comment_count"><i class="fa fa-comments"></i>9</span><span class="apmag-post-views"><i class="fa fa-eye"></i>39</span></div>
                    <div class="block-poston"></div>
                    </div>            
                    </div>
                    <div class="single_post clearfix ">
                   
                    <div class="post-image"><a href="#"><img src="<?php echo get_template_directory_uri();?>/images/demo-images/mac-glasses-114x76.jpg" alt="mac-glasses"></a></div>                                
                <div class="post-desc-wrapper">                    <h3 class="post-title"><a href="#">How to make people love you best tips and tricks</a></h3>
                    <div class="block-poston"><span class="posted-on"><a href="#" rel="bookmark"><time class="entry-date published" datetime="2015-03-08T09:33:18+00:00">March 8, 2015</time><time class="updated" datetime="2015-03-18T11:19:11+00:00">March 18, 2015</time></a></span><span class="comment_count"><i class="fa fa-comments"></i>2</span><span class="apmag-post-views"><i class="fa fa-eye"></i>17</span></div>
                    <div class="block-poston"></div>
                    </div>            
                    </div>
        </div>            
</div>