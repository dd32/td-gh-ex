<?php
 class Quill_Fp_Social_Profile extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'description' => __('Add your social profile.', 'quill') );
		parent::__construct( 'nav_menu', __('Quill FP: Social Profile', 'quill'), $widget_ops );
	}
		...
		<section id="social" class="social-area">		
			...
			<?php
				if ( !empty($instance['title']) )
					echo $args['before_title'] . '<span class="wow fadeInRight">' . $instance['title'] . '</span>' . $args['after_title'];
	...
	function form( $instance ) {
		...
		if ( !$menus ) {
			echo '<p>'. sprintf( __('No menus have been created yet. <a href="%s">Create some</a>.', 'quill'), admin_url('nav-menus.php') ) .'</p>';
			return;
		}
		?>
		<p>
		...
		<p><em><?php _e('In order to display your social icons in a widget, all you need to do is go to <strong>Appearance > Menus</strong> and create a menu containing links to your social profiles, then assign that menu here. Supported networks: Facebook, Twitter, Google Plus, Instagram, Dribble, Vimeo, Linkedin, Youtube, Flickr, Pinterest, Tumblr, Foursquare, Behance.', 'quill'); ?></em></p>
		<p>
			<label for="<?php echo $this->get_field_id('nav_menu'); ?>"><?php _e('Select your social menu:', 'quill'); ?></label>
			<select id="<?php echo $this->get_field_id('nav_menu'); ?>" name="<?php echo $this->get_field_name('nav_menu'); ?>">
				<option value="0"><?php _e( '&mdash; Select &mdash;', 'quill' ) ?></option>
		<?php
