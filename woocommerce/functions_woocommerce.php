...
add_filter('woocommerce_breadcrumb_defaults', 'ariel_woocommerce_breadcrumbs');
function ariel_woocommerce_breadcrumbs()
{
	return array(
		'delimiter' => '',
		...
		switch ( $args['type'] ) {
			case 'country' :
				...
				if ( 1 === sizeof( $countries ) ) {
					...
					$field = '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="country_to_state country_select form-control selectpicker ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . '>' . '<option value="">' . esc_html__( 'Select a country&hellip;', 'ariel' ) . '</option>';
					...
					$field .= '<noscript><input type="submit" name="woocommerce_checkout_update_totals" value="' . esc_attr__( 'Update country', 'ariel' ) . '" /></noscript>';
			...
			case 'state' :
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select form-control selectpicker' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ) . '">
						<option value="">' . esc_html__( 'Select a state&hellip;', 'ariel' ) . '</option>';
...
function ariel_product_review_submit_button( $submit_field, $args ) {
	if ( 'product' == get_post_type() ) :
		if ( ! is_user_logged_in() ) :
			$field = '<div class="col-md-2 form-actions">';
			$field .= '<label class="form-label">&nbsp;</label>';
			$field .= $submit_field;
			$field .= '</div><!-- col-md-2 form-actions -->';
			$field .= '</div><!-- row -->';
			$field .= '</div><!-- col-md-8 col-md-offset-2 -->';
			$field .= '</div><!-- row form-group -->';
		else :
			$field = '<div class="row">';
			$field .= '<div class="col-md-8 col-md-offset-2">';
			$field .= $submit_field;
			$field .= '</div><!-- col-md-8 col-md-offset-2 -->';
			$field .= '</div><!-- row -->';
		endif;
...
function ariel_woo_pagination( $array ) {
	$array['prev_text'] = '<i class="fa fa-long-arrow-left"></i>';
	$array['next_text'] = '<i class="fa fa-long-arrow-right"></i>';
