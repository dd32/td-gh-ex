	...
	<div id="comments">
		...
		<h2 class="woocommerce-Reviews-title"><?php
			if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count = $product->get_review_count() ) ) {
				...
				printf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'basicstore' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
			} else {
				_e( 'Reviews', 'basicstore' );
			}
		?></h2>
			...
			<div id="review_form">
				...
				<?php
					...
					if ( $account_page_url = wc_get_page_permalink( 'myaccount' ) ) {
						$comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a review.', 'basicstore' ), esc_url( $account_page_url ) ) . '</p>';
					}
