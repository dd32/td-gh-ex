<?php get_header();?>
	<div id="content">
		<?php if (have_posts()) : ?>
		<?php while (have_posts()) : the_post(); ?>
		<div class="post" id="post-<?php the_ID(); ?>">
			<h2><?php the_time('F j, Y') ?></h2>
			<div align="center" class="entrywhole">
				<div class="postmetadata-top">
					<div class="float-left"><small><a href="<?php the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>"><?php the_title(); ?></a> <?php edit_post_link(); ?></small></div>
				</div>
				<div align="left" class="entry">
					<?php the_content('Read more &raquo;'); ?>
				</div>
				<div class="postmetadata-bottom" align="left">
					Categories: <?php the_category(' <span class="cat-slash">/</span> '); ?>
				</div>
			</div>
		</div>
		<?php endwhile; ?>
		<?php else : ?>
			<h2>Error 404</h2>
			<p>Dot dot dot...</p>
		<?php endif; ?>
		<div align="center"><?php posts_nav_link(); ?></div>
	</div>
	<div id="navigationwrap1">
		<?php include (TEMPLATEPATH . '/sidebar1.php'); ?>
	</div>
	<div id="navigationwrap2">
		<?php include (TEMPLATEPATH . '/sidebar2.php'); ?>
	</div>
</div>
<?php get_footer();?>