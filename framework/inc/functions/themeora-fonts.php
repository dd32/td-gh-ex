 ...
 * This file hooks with the themeora-admin-fonts-library.php file, in order to achieve 
...
function themeora_font_family($font) {
...
function themeora_enqueue_fonts() {
    //DEFAULTS
    $default = array(
        'default',
        'Default',
    ...
    foreach ($fonts as $font) {
        ...
        if (!in_array($font, $default)) {
            themeora_enqueue_google_fonts($font);
        } //END if(!in_array($font, $default)) 
    } //END foreach($fonts as $font)
}

//END function themeora_enqueue_fonts() 
add_action('wp_enqueue_scripts', 'themeora_enqueue_fonts');


/*  Enque the Google fonts	
--------------------------------------------------------------------- */

function themeora_enqueue_google_fonts($font) {
    $font = explode(',', $font);
    $font = $font[0];
