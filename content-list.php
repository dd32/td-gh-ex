...
<article id="post-<?php the_ID(); ?>" <?php post_class('post-list'); ?>>
	<?php if ( is_sticky() && is_home() && ! is_paged() ) : ?>
		<p class="sticky-title"><?php _e( 'Featured post', 'shipyard' ); ?></p>
	<?php endif; ?>
	...
	<div class="entry-content">
		...
		<?php if ( get_theme_mod( 'shipyard_content_type' ) == "no" ) { ?>
			<?php the_content(); ?>
		<?php } else { ?>
			<?php the_excerpt(); ?>
		<?php } ?>
	...
	<?php if ( get_theme_mod( 'shipyard_read_more' ) != "no" ) { ?>
		<div class="more">
			<a class="readmore" href="<?php the_permalink() ?>" rel="bookmark"><?php _e( 'Read More &raquo;', 'shipyard' ); ?><span class="screen-reader-text"> <?php the_title(); ?></span></a>
		</div>
	<?php } ?>
</article>
