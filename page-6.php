<?php
/*
Template Name: 6 block
*/
?>

<?php get_header(); ?>  
<div class="block1">
		<div class="block1-center">
			<div class="block1-center1">
	            <div class="sidebar-user2 span2">
	            	<?php dynamic_sidebar( 'sidebar-block1-user1' ); ?>
	            </div>
	            <div class="sidebar-user3 span2">
	                <?php dynamic_sidebar( 'sidebar-block1-user2' ); ?>
	            </div>
	           <div class="sidebar-user4 span2">
	                 <?php dynamic_sidebar( 'sidebar-block1-user3' ); ?>
	            </div>
			</div>
		</div>
</div>	

<div class="block2">
	<div class="block1-center">
		<div class="block2-center2">
	            <div class="sidebar-user2 span2">
	                 <?php dynamic_sidebar( 'sidebar-block2-user1' ); ?>
	            </div>
	            <div class="sidebar-user3 span2">
	                 <?php dynamic_sidebar( 'sidebar-block2-user2' ); ?>
	            </div>
				<div class="sidebar-user4 span2">
	                 <?php dynamic_sidebar( 'sidebar-block2-user3' ); ?>
	            </div>
		</div>
	</div>
</div>	



<div class="block3">
	<div class="block3-center">
		<div class="block3-center3">
	           <div class="sidebar-user2 span2">
	                 <?php dynamic_sidebar( 'sidebar-block3-user1' ); ?>
	            </div>
	            <div class="sidebar-user3 span2">
	                 <?php dynamic_sidebar( 'sidebar-block3-user2' ); ?>
	            </div>
	            <div class="sidebar-user4 span2">
	                 <?php dynamic_sidebar( 'sidebar-block3-user3' ); ?>
	            </div>
		</div>
	</div>
</div>	



<div class="block4">
	<div class="block4-center">
		<div class="block4-center1">
	            <div class="sidebar-user2 span2">
	                 <?php dynamic_sidebar( 'sidebar-block4-user1' ); ?>
	            </div>
	            <div class="sidebar-user3 span2">
	                 <?php dynamic_sidebar( 'sidebar-block4-user2' ); ?>
	            </div>
	            <div class="sidebar-user4 span2">
	                 <?php dynamic_sidebar( 'sidebar-block4-user3' ); ?>
	            </div>
		</div>
	</div>
</div>	



<div class="block5">
	<div class="block5-center">
		<div class="block5-center1">
	            <div class="sidebar-user2 span2">
	                 <?php dynamic_sidebar( 'sidebar-block5-user1' ); ?>
	            </div>
	            <div class="sidebar-user3 span2">
	                 <?php dynamic_sidebar( 'sidebar-block5-user2' ); ?>
	            </div>
	            <div class="sidebar-user4 span2">
	                 <?php dynamic_sidebar( 'sidebar-block5-user3' ); ?>
	            </div>
		</div>
	</div>
</div>


<div class="block7">
		        <div class="sidebar-user1 span2">
	                 <?php dynamic_sidebar( 'sidebar-block6-user1' ); ?>
	            </div>
	<div class="block7-center">
		<div class="block7-center1">
	            <div class="sidebar-user2 span2">
	                 <?php dynamic_sidebar( 'sidebar-block6-user2' ); ?>
	            </div>
	            <div class="sidebar-user3 span2">
	                 <?php dynamic_sidebar( 'sidebar-block6-user3' ); ?>
	            </div>
	            <div class="sidebar-user4 span2">
	                 <?php dynamic_sidebar( 'sidebar-block6-user4' ); ?>
	            </div>
		</div>
	</div>
</div>

</div>
<?php get_footer(); ?>