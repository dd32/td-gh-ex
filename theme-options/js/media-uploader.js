(function($) {
	$(document).ready(function() {
		...
		function impressive_add_file(event, selector) {
			...
			frame.on( 'select', function() {
				...
				selector.find('.remove-image, .remove-file').on('click', function() {
					impressive_remove_file( $(this).parents('.section') );
				});
			});
		...
		function impressive_remove_file(selector) {
			...
			selector.find('.upload-button').live('click', function() {
				impressive_add_file(event, $(this).parents('.section'));
			});
		}
