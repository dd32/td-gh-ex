...
?>
<nav id="nav-above">
	<h3 class="assistive-text"><?php echo esc_html__( 'Image navigation', 'weaver-xtreme' ); ?></h3>
...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'page-image' ); ?>>
	...
	<div class="entry-meta <?php weaverx_text_class( 'post_info_top', true ); ?>">
		...
		printf( weaverx_filter_text( __( '<span class="meta-prep meta-prep-entry-date">Published </span> <span class="entry-date"><abbr class="published" title="%1$s">%2$s</abbr></span> at <a href="%3$s" title="Link to full-size image">%4$s &times; %5$s</a> in <a href="%6$s" title="Return to %7$s" rel="gallery">%8$s</a>', 'weaver-xtreme' ) ),
