...
$('a.media-upload-button').click(function(){var $$=$(this).closest('td');wp.media.editor.open('settings');wp.media.editor.send.attachment=function(props,attachment){$$.find('.current .title').html(attachment.title);$$.find('input[type=hidden]').val(attachment.id);if(typeof attachment.sizes!='undefined'){if(typeof attachment.sizes.thumbnail!='undefined')
$$.find('.current .thumbnail').attr('src',attachment.sizes.thumbnail.url).fadeIn();else
$$.find('.current .thumbnail').attr('src',attachment.sizes.full.url).fadeIn();}
else{$$.find('.current .thumbnail').attr('src',attachment.icon).fadeIn();}}
return false;});$('.media-field-wrapper').mouseenter(function(){if($(this).closest('td').find('input[type=hidden]').val()!='')$(this).find('.media-remove-button').fadeIn('fast');}).mouseleave(function(){$(this).find('.media-remove-button').fadeOut('fast');})
