...
function siteorigin_settings_admin_menu() {
	$theme = wp_get_theme();
	$page = add_theme_page(
		sprintf(__( '%s Theme Settings', 'puro' ), $theme->get('Name') ),
		sprintf(__( 'Theme Settings', 'puro' ), $theme->get('Name') ),
		'edit_theme_options',
		'theme_settings_page',
...
function siteorigin_settings_render() {
	if( version_compare( get_bloginfo('version'), '3.4', '<' ) ) {
		?><div class="wrap"><div id="setting-error-settings_updated" class="updated settings-error"><p><strong><?php _e('Please update to the latest version of WordPress to use theme settings.', 'puro') ?></strong></p></div></div><?php
		return;
	}
...
function siteorigin_settings_enqueue_scripts( $prefix ) {
	...
	wp_localize_script( 'siteorigin-settings', 'siteoriginSettings', array(
		'premium' => array(
			...
			'name' => apply_filters('siteorigin_premium_theme_name', ucfirst( get_option( 'template' ) ) . ' ' . __( 'Premium', 'puro' ) ),
		),
		'tour' => array(
			'buttonText' => __('Theme Tour', 'puro'),
	...
	switch ( $args['type'] ) {
		case 'checkbox' :
			...
			<div class="checkbox-wrapper">
				...
				<label for="<?php echo esc_attr( $field_id ) ?>"><?php echo esc_attr( !empty( $args['label'] ) ? $args['label'] : __( 'Enabled', 'puro' ) ) ?></label>
			</div>
			<?php
			break;
		case 'text' :
		...
		case 'media':
			if(version_compare(get_bloginfo('version'), '3.5', '<')){
				printf(__('You need to <a href="%s">upgrade</a> to WordPress 3.5 to use media fields', 'puro'), admin_url('update-core.php'));
				break;
			}
			...
			$choose_title = empty($args['choose']) ? __('Choose Media', 'puro') : $args['choose'];
			$update_button = empty($args['update']) ? __('Set Media', 'puro') : $args['update'];
			...
			?>
				<div class="media-field-wrapper">
					...
					<a href="#" class="media-remove-button"><?php _e('Remove', 'puro') ?></a>
				</div>
		...
		case 'gallery' :
			...
			<input
				...
				type="text"
				value="<?php echo esc_attr( $current ) ?>" />
			<a href="#" class="so-settings-gallery-edit"><?php _e('Select Images', 'puro') ?></a>
			<?php
			break;
		...
		case 'pages' :
			...
			?>
			<select id="<?php echo esc_attr( $field_id ) ?>" name="<?php echo esc_attr( $field_name ) ?>">
				<option value="0"><?php _e('None', 'puro') ?></option>
		...
		case 'widget' :
			...
			if( !class_exists($args['widget_class']) ) {
				...
				printf( __('This field requires the %s plugin. ', 'puro'), $args['plugin_name']);
				if( function_exists('siteorigin_plugin_activation_install_url') ) {
					...
					printf( __('<a href="%s" target="_blank">Install %s</a> now. ', 'puro'), $install_url, $args['plugin_name']);
				}
				?></div>
...
function siteorigin_settings_change_message() {
	if ( get_theme_mod( 'siteorigin_settings_changed' ) ) {
		...
		<div id="setting-updated" class="updated">
			<p>
				<strong><?php _e( 'Settings saved.', 'puro' ) ?></strong>
			</p>
		</div>
		<?php
...
function siteorigin_settings_media_view_strings($strings, $post){
	...
	$strings['insertIntoPost'] = __('Set Media File', 'puro');
...
function siteorigin_settings_add_slider_options($options){
	...
	if( class_exists('MetaSliderPlugin') ){
		...
		foreach($sliders as $slider) {
			$options['[metaslider id="'.$slider->ID.'"]'] = __('Meta Slider: ', 'puro').$slider->post_title;
		}
	}
	...
	if( function_exists('rev_slider_shortcode') ) {
		...
		foreach($sliders as $slider) {
			$options['[rev_slider '.$slider->alias.']'] = __('Revolution Slider: ', 'puro').$slider->title;
		}
	}
	...
	if( function_exists('layerslider') ) {
		...
		foreach($sliders as $slider) {
			$options['[layerslider id="'.$slider->id.'"]'] = __('LayerSlider: ', 'puro').$slider->name;
		}
	}
