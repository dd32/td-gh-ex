...
if(panel==null)originalInsert(h);else window.panels.addPanel(panel,null,null,true);return;}
originalInsert(h);}
$('.so-gallery-widget-select-attachments').live('click',function(event){if(typeof wp==='undefined'||!wp.media||!wp.media.gallery)return;event.preventDefault();var $$=$(this);var dialog=$('.panels-admin-dialog:visible');var val=dialog.find('*[name$="[ids]"]').val();if(val.indexOf('{demo')===0||val.indexOf('{default')===0)val='-';dialog.find('.ui-dialog-content').dialog('close');var frame=wp.media.gallery.edit('[gallery ids="'+val+'"]');frame.state('gallery-edit').on('update',function(selection){dialog.find('.ui-dialog-content').dialog('open');var ids=selection.models.map(function(e){return e.id});dialog.find('input[name$="[ids]"]').val(ids.join(','));});frame.on('escape',function(){dialog.find('.ui-dialog-content').dialog('open');});return false;});});