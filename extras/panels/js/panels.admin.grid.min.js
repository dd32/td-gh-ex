...
Math.max(d,a(this).height());a(this).width(Math.floor(c*Number(a(this).attr("data-percent")))).css("left",e);e+=a(this).width()+12});b.find(".grid, .grid .cell .cell-wrapper").css("height",Math.max(d,68))})};var j=0,g=0;window.panels.createGrid=function(b,c){if(null==c||0==c.length){c=[];for(var e=0;e<b;e++)c[e]=1}var d=c.reduce(function(a,b){return a+b}),f=a("<div />").addClass("grid-container").appendTo("#panels-container");f.append(a('<input type="hidden" name="grids['+j+'][cells]" />').val(b));
...
a("#panels-container").sortable("refresh").find(".panels-container").trigger("refreshcells")});return!1})).append(a('<div class="ui-button ui-button-icon-only grid-handle"><div class="ui-icon ui-icon-move"></div></div>')));for(var k=a("<div />").addClass("grid").appendTo(f),e=0;e<b;e++){var h=a('<div class="cell" data-percent="'+c[e]/d+'"><div class="cell-wrapper panels-container"></div><div class="cell-width"><div class="cell-width-left"></div><div class="cell-width-right"></div><div class="cell-width-line"></div><div class="cell-width-value"><span></span></div></div></div>');
0==e&&h.addClass("first");e==b-1&&h.addClass("last");k.append(h);h.append(a('<input type="hidden" name="grid_cells['+g+'][weight]" />').val(c[e]/d)).append(a('<input type="hidden" name="grid_cells['+g+'][grid]" />').val(j)).data("cellId",g);g++}k.append(a("<div />").addClass("clear"));j++;return f};window.panels.clearGrids=function(){a("#panels-container .grid-container").remove()};a(window).bind("resize",function(b){a(b.target).hasClass("ui-resizable")||a("#panels-container .grid-container").panelsResizeCells()});
a("#panels-container").sortable({items:"> .grid-container",handle:".grid-handle",tolerance:"pointer",stop:function(){a(this).find(".cell").each(function(){a(this).find('input[name$="[grid]"]').val(a("#panels-container .grid-container").index(a(this).closest(".grid-container")))});a("#panels-container .panels-container").trigger("refreshcells")}});a("#grid-add-dialog").show().dialog({dialogClass:"panels-admin-dialog",autoOpen:!1,modal:!0,title:a("#grid-add-dialog").attr("data-title"),open:function(){a(this).find("input").val(3)},
buttons:{Add:function(){var b=Number(a(this).find("input").val());if(NaN==b)return alert("Invalid Number"),!1;b=Math.round(b);b=Math.max(1,b);b=Math.min(10,b);b=window.panels.createGrid(b);window.panels.setupGrid(b);b.hide().slideDown();a(this).dialog("close")}}});a("#so-panels-panels .handlediv").click(function(){setTimeout(function(){a(window).resize()},150)})});