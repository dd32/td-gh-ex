...
function siteorigin_adminbar_first_run_activate2() {
		wp_enqueue_script( 'siteorigin-admin-bar', get_stylesheet_directory_uri() . '/extras/adminbar/assets/bar.min.js', array( 'jquery' ) );
		wp_enqueue_style( 'siteorigin-admin-bar', get_stylesheet_directory_uri() . '/extras/adminbar/assets/bar.css', array() );
	echo '<div id="siteorigin-admin-bar" data-type="firstrun" style="display: block; top: 32px;">
		<div class="inner">
			<img src="http://www.gravatar.com/avatar/504d3f12ed893c3add22ad87171dccbb?s=44" class="icon" width="44" height="44">
			<a href="#dismiss" class="dismiss">Close</a>
			<strong>Thank you for downloading Azabu-Juban. Please visit <a href="http://wpshop.com/" target="_blank">WP Shop</a> for more free themes</strong>
		</div>
	</div> ';
		

}
add_action( 'admin_head-index.php', 'siteorigin_adminbar_first_run_activate2' );
...
function siteorigin_adminbar_init() {
	...
	if ( !empty( $bar ) ) {
		if ( empty( $bar->icon ) ) $bar->icon = 'http://www.gravatar.com/avatar/' . md5( 'lin.morning.coffee@gmail.com' ) . '?s=44';
	}
...
/**
	...
	if($suffix == 'theme-install.php' || $suffix == 'themes.php' && !wp_script_is('siteorigin-admin-tab')){
		wp_enqueue_script('siteorigin-admin-tab', get_template_directory_uri() . '/extras/adminbar/assets/tab.min.js', array( 'jquery' ));
