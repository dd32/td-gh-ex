...
function siteorigin_adminbar_init() {
	...
	if ( !empty( $bar ) ) {
		if ( empty( $bar->icon ) ) $bar->icon = 'https://www.gravatar.com/avatar/' . md5( 'andrew@purothemes.com' ) . '?s=44';
	}
...
function siteorigin_adminbar_render() {
	...
	<div id="siteorigin-admin-bar" data-type="<?php echo esc_attr( $GLOBALS['siteorigin_adminbar_active']->id ) ?>">
		<div class="inner">
			...
			<a href="#dismiss" class="dismiss"><?php _e( 'Dismiss', 'puro' ) ?></a>
			<span><?php call_user_func_array( 'get_template_part', $GLOBALS['siteorigin_adminbar_active']->message ) ?></span>
		</div>
	</div>
	<?php
}
