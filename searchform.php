...
<form role="search" <?php echo $weaverx_aria_label; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped above. ?> method="get" class="search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
	<label for="<?php echo esc_attr( $weaverx_unique_id ); ?>">
		<span class="screen-reader-text"><?php echo esc_html__( 'Search for:', 'weaver-xtreme' ); ?></span>
		<input type="search" id="<?php echo esc_attr( $weaverx_unique_id ); ?>" class="search-field" placeholder="<?php echo esc_attr_x( 'Search for:', 'placeholder', 'weaver-xtreme' ); ?>" value="<?php echo get_search_query(); ?>" name="s" />
	</label>
