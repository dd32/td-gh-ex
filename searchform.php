...
<form role="search" method="get" role="search" class="search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
	<label>
		...
		<input type="search" class="search-field" placeholder="<?php echo esc_attr_x( 'Search &hellip;', 'placeholder', 'artblog' ); ?>" value="<?php the_search_query(); ?>" name="s" id="s<?php echo absint( $search_form_id ); ?>" />
	</label>
