		...
		<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
            <header class="header">
                <h1 class="entry-title"><?php the_title(); ?> <span class="meta-sep">|</span> <a
                            ...
                            title="<?php printf( __( 'Return to %s', 'atlast-business' ), esc_html( get_the_title( $post->post_parent ), 1 ) ); ?>"
                            rev="attachment"><span
                ...
                <section class="entry-content">
                    <div class="entry-attachment">
						<?php if ( wp_attachment_is_image( $post->ID ) ) : $att_image = wp_get_attachment_image_src( $post->ID, "large" ); ?>
                            <p class="attachment"><a href="<?php echo wp_get_attachment_url( $post->ID ); ?>"
