...
<?php for($generator_loop=1 ; $generator_loop <5 ; $generator_loop++):?>
	<?php if(!empty($generator_options['slider-img-'.$generator_loop])){ ?>
    	<li><?php if(!empty($generator_options['slidelink-'.$generator_loop])) {?><a href="<?php echo $generator_options['slidelink-'.$generator_loop];?>" target="_blank"><img src="<?php echo $generator_options['slider-img-'.$generator_loop]; ?>" alt="" /></a><?php }else{?><img src="<?php echo $generator_options['slider-img-'.$generator_loop]; ?>" alt="" /><?php } ?></li>
    <?php } ?>
<?php endfor;?>
</ul>
</div>
...
<div class="container container-generator no-padding">
	<div class="col-md-12 no-padding next-prev">
    	<div class="back-radius">
	   		 <i class="fa fa-pencil project-icon-size"></i>
        </div>
        <span class="project-tag"> <?php if(!empty($generator_options['post-title'])) { echo $generator_options['post-title']; }else{ echo 'Recent Post'; }?></span>
    </div>
...
?>
<?php 
if ( $generator_query->have_posts() ) { 
?>
<div class="owl-carousel" id="owl-demo" >

<?php
    while($generator_query->have_posts()) { 
        $generator_query->the_post();
        ?>
        ...
        <div class="thumbnail">
                        <div class="caption " style="display: none;">
                           	<a href="<?php echo get_permalink(get_the_ID()) ?>">
                      		   <span class="back-radius-img-hover">
                               		<i class="fa fa-plus back-plus-center "></i>
                               </span>    
                            </a>
                        </div>
