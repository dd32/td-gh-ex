...
Full settings backup, including those marked with &diams;. Does <strong>NOT</strong> include Weaver Xtreme Plus settings. <em>File:</em>', 'weaver-xtreme' ) ); ?>
			...
			<?php echo weaverx_filter_text( __( 'Backup all current theme settings using the WordPress database.', 'weaver-xtreme' ) ); ?><br/><br/>
			<input type="button" class="button-primary" name="wvrx_save_todb" value="<?php echo esc_attr__( 'Save All Settings to WP Database', 'weaver-xtreme' ); ?>"/><br/><br/>
			<hr/>
		...
		static public function process_save( $wp_customize ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				if ( isset( $_REQUEST['wvrx_save'] ) ) {
					...
					{
						$opts = get_option( 'weaverx_settings' );
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'WeaverX_Restore_WX_Settings' ) ) :
	...
	class WeaverX_Restore_WX_Settings extends WP_Customize_Control {
		...
		public function render_content() {
			...
			?>
			<h3>
				<?php echo esc_html__( 'Restore settings from file on your computer', 'weaver-xtreme' ); ?>
			</h3>

			<?php echo weaverx_filter_text( __( 'You can restore the saved theme settings from a file on your computer.
...
You may need to check your folder permissions or other server settings.', 'weaver-xtreme' ) . esc_html__( 'Trying to use file', 'weaver-xtreme' ) . $openname;
		...
		static public function reset_options( $contents, $ext ) {
			...
			} elseif ( $ext == 'json' ) {         // restore new json format
	...
	class WeaverX_Load_WX_Subtheme extends WP_Customize_Control {
		...
		public function render_content() {
			...
			<div class="wvrx-settings-load-subtheme">
				...
				<p class="description customize-control-description"><strong>Click "Set to Selected Subtheme" to pick new selected subtheme.</strong> Selecting a subtheme will <em>reset</em> all existing theme settings. Site settings (&diams;) will not be
					changed.
				</p>
				<?php
