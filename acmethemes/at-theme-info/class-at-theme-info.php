 ...
 * @subpackage Admin
 */
if ( ! class_exists( 'Acmeblog_Theme_Info' ) ) {
    class Acmeblog_Theme_Info{
        ...
        public function prepare_class() {
            ...
            $this->page_title    = isset( $this->config['page_title'] ) ? $this->config['page_title'] : esc_html__('Info','acmeblog'). $this->theme_name;
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'acmeblog' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'acmeblog' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'acmeblog' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'acmeblog' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions','acmeblog').'</a>';
                }
                echo '</div>';
            }
        }
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download','acmeblog' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . $preview_link . '">' . esc_html__( 'Live Preview','acmeblog' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
        ...
        public function free_pro() {
            ...
            if ( ! empty( $free_pro ) ) {
                ...
                if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
                    ...
                    echo '<tr>';
                    echo '<th></th>';
                    echo '<th>' . esc_html__( 'AcmeBlog','acmeblog' ) . '</th>';
                    echo '<th>' . esc_html__( 'AcmeBlogPro','acmeblog' ) . '</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
                    ...
                    echo '<td></td>';
                    echo '<td colspan="2"><a href="https://www.acmethemes.com/themes/acmeblogpro/" target="_blank" class="button button-primary button-hero"> AcmeBlogPro</a></td>';
                    echo '</tr>';
...
$config = array(
    // Page title.
    'page_title'               => esc_html__( 'AcmeBlog Info', 'acmeblog' ),

    // Menu name under Appearance.
    'menu_title'               => esc_html__( 'AcmeBlog Info', 'acmeblog' ),

    // Main welcome title
    'info_title'         => sprintf( esc_html__( 'Welcome to %s - ', 'acmeblog' ), 'AcmeBlog' ),

    // Main welcome content
    'info_content'       => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help you. If you want to ask any query or just want to say hello, you can always contact us. We hope you enjoy it! ', 'acmeblog' ), '<b>AcmeBlog</b>' ),
    ...
    'quick_links'                    => array(
        'theme_url'  => array(
                'text' => __('Theme Details','acmeblog'),
                'url' => 'https://www.acmethemes.com/themes/acmeblog/'
        ),
        'demo_url'  => array(
            'text' => __('View Demo','acmeblog'),
            'url' => 'http://www.demo.acmethemes.com/acmeblog/'
        ),
        'pro_url'  => array(
            'text' => __('View Pro Version','acmeblog'),
            'url' => 'https://www.acmethemes.com/themes/acmeblogpro/'
        ),
        'rate_url'  => array(
            'text' => __('Rate This Theme','acmeblog'),
            'url' => 'https://wordpress.org/support/theme/acmeblog/reviews/?filter=5'
        ),
    ),

    'tabs'                    => array(
        'getting_started'  => __( 'Getting Started', 'acmeblog' ),
        'recommended_actions' => __( 'Recommended Actions', 'acmeblog' ),
        'recommended_plugins' => __( 'Useful Plugins','acmeblog' ),
        'support'       => __( 'Support', 'acmeblog' ),
        'changelog'        => __( 'Changelog', 'acmeblog' ),
        'faq'        => __( 'FAQ', 'acmeblog' ),
        'free_pro'         => __( 'Free VS PRO', 'acmeblog' ),
    ),

    /*Getting started tab*/
    'getting_started' => array(
        'first' => array(
            'title' => esc_html__( 'Step 1 : Read full documentation','acmeblog' ),
            'desc' => esc_html__( 'Please check our full documentation for detailed information on how to Setup and Use AcmeBlog.','acmeblog' ),
            'link_title' => esc_html__( 'Documentation','acmeblog' ),
            'link_url' => 'http://www.doc.acmethemes.com/acmeblog/',
            'is_button' => false,
            'recommended_actions' => false,
            'is_new_tab' => true
        ),
        'second' => array(
            'title' => esc_html__( 'Step 2 : Go to Customizer','acmeblog' ),
            'desc' => esc_html__( 'All Setting, Theme Options, Widgets and Menus are available via Customize screen. Have a quick look or start customization!','acmeblog' ),
            'link_title' => esc_html__( 'Go to Customizer','acmeblog' ),
        ...
        'third' => array(
	        'title' => esc_html__( 'Step 3 : Setting Featured Section ','acmeblog' ),
	        'desc' => esc_html__( 'Add Feature Image or Unlimited Slides on Feature Section. Please do the Step 3 before this.','acmeblog' ),
	        'link_title' => esc_html__( 'Featured Section','acmeblog' ),
	        'link_url' => esc_url( admin_url( 'customize.php' ) ).'?autofocus[panel]=acmeblog-feature-panel',
	        'is_button' => true,
	        'recommended_actions' => false,
	        'is_new_tab' => true
        )
    ...
    'recommended_actions'        => array(
	    'setting-image' => array(
		    'title' => esc_html__( 'Setting Image Size','acmeblog' ),
		    'desc' => sprintf( esc_html__( 'Set thumbnail size : 500 * 280, Set Medium size : 690 * 400, Set Large size : 1080 * 530 %1$s Set Image Size %2$s ', 'acmeblog' ), '<p><a href="#" class="acme-set-image button button-primary">','</a></p>' ),
		    'id' => 'setting-image'
	    ),
        'demo-content' => array(
    ...
    'faq'      => array(
	    'first' => array (
		    'title' => esc_html__( 'Where to get a dedicated support ? ','acmeblog' ),
		    'icon' => 'dashicons dashicons-sos',
		    'desc' => esc_html__( 'Follow the theme dedicated support forum.','acmeblog' ),
		    'button_label' => esc_html__( 'Click here to get the support','acmeblog' ),
	    ...
	    'second' => array (
		    'title' => esc_html__( 'Recent Updates of your Site ? ','acmeblog' ),
		    'icon' => 'dashicons dashicons-sos',
		    'desc' => esc_html__( 'Where i can get the recent updates of theme related updates ? ','acmeblog' ),
		    'button_label' => esc_html__( 'Visit our site blog page','acmeblog' ),
		    'button_link' => esc_url( 'https://www.acmethemes.com/blog/' ),
		    'is_button' => true,
		    'is_new_tab' => true
	    ),
	    'third' => array (
		    'title' => esc_html__( 'How to make image size suitable for the theme ? ','acmeblog' ),
		    'icon' => 'dashicons dashicons-sos',
		    'desc' => esc_html__( 'In this recent update, you can check Appearance > AcmeBlog Info > Recommended Action > Setting Image Size and click on Set Image size. Now your theme image size will be automatically updates on Settings > Media.','acmeblog' )
	    ),

	    'forth' => array (
		    'title' => esc_html__( 'How to hide/change footer text Proudly powered by WordPress | Theme: AcmeBlog by AcmeThemes. ? ','acmeblog' ),
		    'icon' => 'dashicons dashicons-sos',
		    'button_label' => esc_html__( 'Visit this blog post to know how to change or remove powered by text.','acmeblog' ),
    ...
    'support_content'      => array(
        'first' => array (
            'title' => esc_html__( 'Contact Support','acmeblog' ),
            'icon' => 'dashicons dashicons-sos',
            'desc' => esc_html__( 'Got theme support question or found bug? Best place to ask your query is our dedicated Support forum.','acmeblog' ),
            'button_label' => esc_html__( 'Contact Support','acmeblog' ),
            'button_link' => esc_url( 'https://www.acmethemes.com/supports/forum/acmeblog/' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'second' => array(
            'title' => esc_html__( 'Documentation','acmeblog' ),
            'icon' => 'dashicons dashicons-book-alt',
            'desc' => esc_html__( 'Please check our full documentation for detailed information on how to Setup and Use AcmeBlog.','acmeblog' ),
            'button_label' => esc_html__( 'Read full documentation','acmeblog' ),
            'button_link' => 'http://www.doc.acmethemes.com/acmeblog/',
            'is_button' => false,
            'is_new_tab' => true
        ),
        'third' => array(
            'title' => esc_html__( 'Need more features?','acmeblog' ),
            'icon' => 'dashicons dashicons-screenoptions',
            'desc' => esc_html__( 'Upgrade to PRO version for more exciting features and Priority Support.','acmeblog' ),
            'button_label' => esc_html__( 'View Pro Version','acmeblog' ),
            'button_link' => 'https://www.acmethemes.com/themes/acmeblogpro/',
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fourth' => array(
            'title' => esc_html__( 'Got sales related question?','acmeblog' ),
            'icon' => 'dashicons dashicons-cart',
            'desc' => esc_html__( "Have any query before purchase, you are more than welcome to ask.",'acmeblog' ),
            'button_label' => esc_html__( 'Pre-sale Question?','acmeblog' ),
            'button_link' => 'https://www.acmethemes.com/pre-sale-question/',
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fifth' => array(
            'title' => esc_html__( 'Customization Request','acmeblog' ),
            'icon' => 'dashicons dashicons-hammer',
            'desc' => esc_html__( 'Needed any customization for the theme, you can request from here.','acmeblog' ),
            'button_label' => esc_html__( 'Customization Request','acmeblog' ),
    ...
    'free_pro' => array(
	    array(
		    'title'       => __( 'Custom Widgets', 'acmeblog' ),
		    'desc' => __( 'Added custom Widgets', 'acmeblog' ),
		    'free'  => __('1+ Available','acmeblog'),
		    'pro'   => __('7+ Available','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Widget Options', 'acmeblog' ),
		    'desc' => __( 'Every widgets have multiple options to customize section', 'acmeblog' ),
		    'free'  => __('Limited','acmeblog'),
		    'pro'   => __('Multiple Options','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Google Fonts', 'acmeblog' ),
		    'desc' => __( 'Google fonts options for changing the overall site fonts ', 'acmeblog' ),
		    'free'  => 'yes',
		    'pro'   => __('100 +','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Color Options', 'acmeblog' ),
		    'desc' => __( 'Color options for changing overall site colors, Contat us', 'acmeblog' ),
		    'free'  => __('2','acmeblog'),
		    'pro'   => __('Unlimited','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Header Options', 'acmeblog' ),
		    'desc' => __( 'Customze the header section as your need.', 'acmeblog' ),
		    'free'  => __('Limited','acmeblog'),
		    'pro'   => __('Multiple Options','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Menu Options', 'acmeblog' ),
		    'desc' => __( 'Lots of menu options are available to manage the menu section. Appearance > Menus', 'acmeblog' ),
		    'free'  => __('Limited','acmeblog'),
		    'pro'   => __('Multiple Options','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Featured Selection', 'acmeblog' ),
		    'desc' => __( 'This is slider section. Check every options there','acmeblog' ),
		    'free'  => __('Recent Post/Category','acmeblog'),
		    'pro'   => __('Page, Post, Category, And Custom','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Slider Section Options', 'acmeblog' ),
		    'desc' => __( 'Options to manage the sliders', 'acmeblog' ),
		    'free'  => __('Limited','acmeblog'),
		    'pro'   => __('Multiple Options','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Sticky Sidebar', 'acmeblog' ),
		    'desc' => __( 'Enable/Disable sticky sidebars', 'acmeblog' ),
		    'free'  => 'yes',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Blog/Archive Options', 'acmeblog' ),
		    'desc' => __( 'Various Options are available for the blog and archive page management. Check on Appearance > Customize > Layout/Design Options.', 'acmeblog' ),
		    'free'  => __('Limited','acmeblog'),
		    'pro'   => __('Manage Every Entity','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Footer Options', 'acmeblog' ),
		    'desc' => __( 'Manage the every entity related to footer section.', 'acmeblog' ),
		    'free'  => __('Limited','acmeblog'),
		    'pro'   => __('Multiple Options','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Social Options', 'acmeblog' ),
		    'desc' => __( 'Options to manage the social icons like facebook, instagram, linkedin etc.', 'acmeblog' ),
		    'free'  => __('Limited','acmeblog'),
		    'pro'   => __('Multiple Options','acmeblog'),
	    ),
	    array(
	    ...
	    array(
		    ...
		    'desc' => __( 'Write your own credit text.', 'acmeblog' ),
		    'free'  => 'no',
		    'pro'   => __('Your Own Text','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Social Icons', 'acmeblog' ),
		    'desc' => __( 'Enter the URL for social icons. ', 'acmeblog' ),
		    'free'  => __('4 +','acmeblog'),
		    'pro'   => __('Available Mostly Used','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Intro Loader Options', 'acmeblog' ),
		    'desc' => __( 'It will load before the site loaded completely.', 'acmeblog' ),
		    'free'  => 'no',
		    'pro'   => __('Multiple Options','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Related Post Options', 'acmeblog' ),
		    'desc' => __( 'Related post will appear on single post of each blog article', 'acmeblog' ),
		    'free'   => __('Enable/Disable','acmeblog'),
		    'pro'   => __('Multiple Options','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Author Info Options', 'acmeblog' ),
		    'desc' => __( 'This will appear on single page and archive page of that author.', 'acmeblog' ),
		    'free'   => __('Default','acmeblog'),
		    'pro'   => __('Multiple Options','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Feature Image Options', 'acmeblog' ),
		    'desc' => __( 'Featured image options for the single page and post.', 'acmeblog' ),
		    'free'  => 'yes',
		    'pro'   => __('Manage As Your Need','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Navigation Options', 'acmeblog' ),
		    'desc' => __( 'Post Navigation means, previous post and next post. Check on single post.', 'acmeblog' ),
		    'free'   => __('Default','acmeblog'),
		    'pro'   => __('Advanced','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Post Comment Options', 'acmeblog' ),
		    'desc' => __( 'Manage the comment section on single post.', 'acmeblog' ),
		    'free'  => 'no',
		    'pro'   => __('Options Available','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Show/Hide Comment', 'acmeblog' ),
		    'desc' => __( 'Options to show hide comments on page and post.', 'acmeblog' ),
		    'free'  => 'no',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Breadcrumb', 'acmeblog' ),
		    'desc' => __( 'Advanced options for manage the Breadcrumb.', 'acmeblog' ),
		    'free'  => 'yes',
		    'pro'   => __('Advanced','acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Video Post On Featured', 'acmeblog' ),
		    'desc' => __( 'Create your video post easily from Youtube and Vimeo video.', 'acmeblog' ),
		    'free'  => 'no',
		    'pro'   => __('Add Own Video On Featured','acmeblog'),
	    ),
	    array(
		    'title'=> __( 'Single Post Options', 'acmeblog' ),
		    'desc' => __( 'Manage every entity of single post.', 'acmeblog' ),
		    'free' => __('Limited with Image size','acmeblog'),
		    'pro'  => __('Multiple Options','acmeblog'),
	    ),
	    array(
		    'title'=> __( 'Featured Image Size', 'acmeblog' ),
		    'desc' => __( 'Select the size of the featured image on the single post.', 'acmeblog' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
	    ...
	    array(
		    'title'       => __( 'Translation', 'acmeblog' ),
		    'desc' => __( 'Both theme are translation ready.', 'acmeblog' ),
		    'free'  => 'yes',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'SEO', 'acmeblog' ),
		    'desc' => __( 'Developed with high skilled SEO tools.', 'acmeblog' ),
		    'free'  => 'yes',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Widget Area On Footer', 'acmeblog' ),
		    'desc' => __( 'Change the number of footers on footer area as you need.', 'acmeblog' ),
		    'free'  => __('3 ', 'acmeblog'),
		    'pro'   => __('Choose On Need', 'acmeblog'),
	    ),
	    array(
		    'title'       => __( 'Support Forum', 'acmeblog' ),
		    'desc' => __( 'Highly dedicated support team are assigned for your help. Try this today.', 'acmeblog' ),
		    'free'  => __('Second', 'acmeblog'),
		    'pro'   => __('Dedicated With High Priority', 'acmeblog'),
	    )

    ),
);
return new Acmeblog_Theme_Info( $config );