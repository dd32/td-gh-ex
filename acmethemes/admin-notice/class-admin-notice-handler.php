 ...
 *
 * Class AcmePhoto_Notice_Handler
 */
class AcmePhoto_Notice_Handler {
	...
	public function replace_term_ids( $replace_term_ids ){
		...
		/*Terms IDS*/
		$term_ids = array(
            'acmephoto-slider-from-category',
		);
	...
	public function admin_notice() {
		...
		if ( ! $notice_nag ) {
			wp_enqueue_style( 'acmephoto-notice', get_template_directory_uri() . '/acmethemes/admin-notice/admin-notice.css', array(), '3.0.0' );
			wp_enqueue_script( 'acmephoto-adi-install', get_template_directory_uri()  . '/acmethemes/admin-notice/admin-notice.js', array( 'jquery' ), '', true );
	...
	public static function hide_notices() {
		...
		if ( isset( $_GET['at-gsm-hide-notice'] ) && isset( $_GET['at_gsm_admin_notice_nonce'] ) ) { // WPCS: input var ok.
			if ( ! wp_verify_nonce( wp_unslash( $_GET['at_gsm_admin_notice_nonce'] ), 'at_gsm_hide_notices_nonce' ) ) { // phpcs:ignore WordPress.VIP.ValidatedSanitizedInput.InputNotSanitized
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'acmephoto' ) ); // WPCS: xss ok.
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'acmephoto' ) ); // WPCS: xss ok.
			}

			$notice_type = sanitize_text_field( wp_unslash( $_GET['at-gsm-hide-notice'] ) );
			update_option( 'acmephoto_admin_notice_' . $notice_type, 1 );

			/*Update to Hide.*/
			if ( 'welcome' === $_GET['at-gsm-hide-notice'] ) {
				update_option( 'acmephoto_admin_notice_' . $notice_type, 1 );
			} else { // Show.
				delete_option( 'acmephoto_admin_notice_' . $notice_type );
			}
		}
                ...
                <div class="at-gsm-notice">
                    <h2>
						...
						/* translators: 1: welcome page link starting html tag, 2: welcome page link ending html tag. */
							esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer, please make sure you visit our %2$swelcome page%3$s.', 'acmephoto' ), '<strong>'. wp_get_theme()->get('Name'). '</strong>','<a href="' . esc_url( admin_url( 'themes.php?page=acmephoto-info' ) ) . '">','</a>' );
						?>
                    </h2>
	...
	public function setup_review_notice() {
		...
		// Set the installed time in `acmephoto_theme_installed_time` option table.
		$option = get_option('acmephoto_theme_installed_time' );

		if ( ! $option ) {
			update_option('acmephoto_theme_installed_time', time() );
		}
	...
	public function display_review_notice() {
		 ...
		 */
		if ( ( get_option('acmephoto_theme_installed_time' ) > strtotime( '-15 day' ) ) || ( $ignored_notice_partially > strtotime( '-15 day' ) ) || ( $ignored_notice ) ) {
			return;
		}
		?>
		<div class="notice updated at-review-notice">
			<p>
				...
				/* Translators: %1$s current user display name. */
					esc_html__(
						'Howdy, %1$s! It seems that you have been using this theme for more than 15 days. We hope you are happy with everything that the theme has to offer. If you can spare a minute, please help us by leaving a 5-star review on WordPress.org.  By spreading the love, we can continue to develop new amazing features in the future, for free!', 'acmephoto'
					),
			...
			<div class="links">
				<a href="https://wordpress.org/support/theme/acmephoto/reviews/?filter=5#new-post" class="btn button-primary" target="_blank">
					<span class="dashicons dashicons-thumbs-up"></span>
					<span><?php esc_html_e( 'Sure', 'acmephoto' ); ?></span>
				</a>

				<a href="?nag_remove_theme_review_notice_partially=0" class="btn at-danger-btn">
					<span class="dashicons dashicons-calendar"></span>
					<span><?php esc_html_e( 'Maybe later', 'acmephoto' ); ?></span>
				</a>

				<a href="?nag_remove_theme_review_notice=0" class="btn at-success-btn">
					<span class="dashicons dashicons-smiley"></span>
					<span><?php esc_html_e( 'I already did', 'acmephoto' ); ?></span>
				</a>

				<a href="<?php echo esc_url( 'https://wordpress.org/support/theme/acmephoto/' ); ?>" class="btn at-default-btn" target="_blank">
					<span class="dashicons dashicons-edit"></span>
					<span><?php esc_html_e( 'Got theme support question?', 'acmephoto' ); ?></span>
				</a>
			</div>
	...
	public function remove_review_notice() {
		...
		global $current_user;
		$user_id                  = $current_user->ID;
		$theme_installed_time     = get_option('acmephoto_theme_installed_time' );
		$ignored_notice           = get_user_meta( $user_id, 'remove_theme_review_notice', true );
		$ignored_notice_partially = get_user_meta( $user_id, 'nag_remove_theme_review_notice_partially', true );

		// Delete options data.
		if ( $theme_installed_time ) {
			delete_option('acmephoto_theme_installed_time' );
		}
	...
	public function review_notice_enqueue() {
		...
		wp_enqueue_style( 'acmephoto-review-notice', get_template_directory_uri()  . '/acmethemes/admin-notice/admin-notice.css' );
 ...
 * @since    1.0.0
 */
function acmephoto_notice_handler( ) {
	return AcmePhoto_Notice_Handler::instance();
}
acmephoto_notice_handler()->run();


/*For this theme only*/
if( !function_exists( 'acmephoto_ai_update_image_size') ){
	function acmephoto_ai_update_image_size(){
		/*Thumbnail Size*/
...
add_action( 'advanced_import_before_content_screen', 'acmephoto_ai_update_image_size' );