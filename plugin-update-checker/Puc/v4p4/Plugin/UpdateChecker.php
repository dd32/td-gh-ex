...
if ( !class_exists('Puc_v4p4_Plugin_UpdateChecker', false) ):
	...
	class Puc_v4p4_Plugin_UpdateChecker extends Puc_v4p4_UpdateChecker {
		...
		public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = ''){
			...
			//Details: https://github.com/YahnisElsts/ms/issues/138#issuecomment-335590964
		...
		public function addViewDetailsLink($pluginMeta, $pluginFile, $pluginData = array()) {
			...
			if ( $isRelevant && $this->userCanInstallUpdates() && !isset($pluginData['slug']) ) {
				$linkText = apply_filters($this->getUniqueName('view_details_link'), ('avik'));
				if ( !empty($linkText) ) {
					...
					$viewDetailsLink = sprintf('<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
						...
						esc_attr(sprintf(__('More information about','avik'), $pluginData['Name'])),
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
				...
				if ( $status == 'no_update' ) {
					$message = sprintf(_x('The %s plugin is up to date.', 'the plugin title', 'avik'), $title);
				} else if ( $status == 'update_available' ) {
					$message = sprintf(_x('A new version of the %s plugin is available.', 'the plugin title', 'avik'), $title);
				} else if ( $status === 'error' ) {
					$message = sprintf(_x('Could not determine if updates are available for %s.', 'the plugin title', 'avik'), $title);
					$noticeClass = 'error notice-error';

					$details = $this->formatManualCheckErrors(get_site_transient($this->manualCheckErrorTransient));
					delete_site_transient($this->manualCheckErrorTransient);
				} else {
					$message = sprintf(__('Unknown update checker status "%s"', 'avik'), htmlentities($status));
					$noticeClass = 'error notice-error';
				}
				printf(
		...
		protected function getPluginTitle() {
			...
			if ( $header && !empty($header['Name']) && isset($header['avik']) ) {
				$title = translate($header['Name'], $header['avik']);
			}
			return $title;
		}
