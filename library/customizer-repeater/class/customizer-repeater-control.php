...
class Avadanta_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'avadanta' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'avadanta' );
		...
		if ( file_exists( AVADANTA_THEME_DIR . '/library/customizer-repeater/inc/icons.php' ) ) {
	...
	public function enqueue() {
		wp_enqueue_style( 'font-awesome', AVADANTA_THEME_URI . '/assets/css/font-awesome/css/font-awesome.css', array(), 999 );

		wp_enqueue_style( 'avadanta_customizer-repeater-admin-stylesheet', AVADANTA_THEME_URI . '/library/customizer-repeater/css/admin-style.css', array(), 999 );

		wp_enqueue_style( 'wp-color-picker' );

		wp_enqueue_script( 'avadanta_customizer-repeater-script', AVADANTA_THEME_URI . '/library/customizer-repeater/js/customizer_repeater.js', array('jquery', 'jquery-ui-draggable', 'wp-color-picker' ), 999, true  );

    	wp_enqueue_style( 'avadanta-admin-style',  AVADANTA_THEME_URI. '/assets/css/avadanta-admin.css');    


		wp_enqueue_script( 'avadanta_customizer-repeater-fontawesome-iconpicker', AVADANTA_THEME_URI . '/library/customizer-repeater/js/fontawesome-iconpicker.js', array( 'jquery' ), 999, true );

		wp_enqueue_style( 'avadanta_customizer-repeater-fontawesome-iconpicker-script', AVADANTA_THEME_URI . '/library/customizer-repeater/css/fontawesome-iconpicker.min.css', array(), 999 );
	...
	public function render_content() {
		...
		<div class="customizer-repeater-general-control-repeater customizer-repeater-general-control-droppable">
			...
			<input type="hidden" value="3" id="avadanta_service_limit"/>
					...
					<div class="customizer-repeater-box-content-hidden">
						...
						if($this->customizer_repeater_title_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Title','avadanta' ), $this->id, 'customizer_repeater_title_control' ),
						...
						if($this->customizer_repeater_subtitle_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Subtitle','avadanta' ), $this->id, 'customizer_repeater_subtitle_control' ),
						...
						if($this->customizer_repeater_text_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Description','avadanta' ), $this->id, 'customizer_repeater_text_control' ),
						...
						if($this->customizer_repeater_button_text_control){
							...
							'avadanta'), $this->id, 'customizer_repeater_button_text_control'),
						...
						if($this->customizer_repeater_link_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Link','avadanta' ), $this->id, 'customizer_repeater_link_control' ),
						...
						if($this->customizer_repeater_video_url_control){
							...
							'avadanta'), $this->id, 'customizer_repeater_video_url_control'),
						...
						if($this->customizer_repeater_color_control == true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Color','avadanta' ), $this->id, 'customizer_repeater_color_control' ),
						...
						if($this->customizer_repeater_shortcode_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Shortcode','avadanta' ), $this->id, 'customizer_repeater_shortcode_control' ),
						...
						if($this->customizer_repeater_designation_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Designation','avadanta' ), $this->id, 'customizer_repeater_designation_control' ),
						...
						<button type="button" class="social-repeater-general-control-remove-field" <?php if ( $it == 0 ) {
							...
							<?php esc_html_e( 'Delete field', 'avadanta' ); ?>
						</button>
...
//print_r($options);
	...
	<?php esc_html_e('Open link in new tab:','avadanta'); ?>
	<span class="switch">
	...
	private function icon_picker_control($value = '', $show = ''){ ?>
		<div class="social-repeater-general-control-icon" <?php if( $show === 'customizer_repeater_image' || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Icon','avadanta'); ?>
            </span>
			<span class="description customize-control-description">
                <?php
                echo sprintf(
	                esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at %1$s.', 'avadanta' ),
	                sprintf( '<a href="%1$s" rel="nofollow">%s</a>', esc_html__( 'http://fontawesome.io/icons/', 'avadanta' ) )
                ); ?>
            </span>
	...
	private function image_control($value = '', $show = ''){ ?>
		<div class="customizer-repeater-image-control" <?php if( $show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Image','avadanta')?>
            </span>
			<input type="text" class="widefat custom-media-url" value="<?php echo esc_attr( $value ); ?>">
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e( 'Upload Image','avadanta' ); ?>" />
		</div>
		<?php
	}
	...
	<span class="customize-control-title">
	<?php esc_html_e('Slide Format','avadanta'); ?>
	</span>
	...
	private function icon_type_choice($value='customizer_repeater_icon'){ ?>
		<span class="customize-control-title">
            <?php esc_html_e('Image type','avadanta');?>
        </span>
		<select class="customizer-repeater-image-choice">
			<option value="customizer_repeater_icon" <?php selected($value,'customizer_repeater_icon');?>><?php esc_html_e('Icon','avadanta'); ?></option>
			<option value="customizer_repeater_none" <?php selected($value,'customizer_repeater_none');?>><?php esc_html_e('None','avadanta'); ?></option>
		</select>
		<?php
	}
	...
	private function repeater_control($value = ''){
		...
		$show_del        = 0; ?>
		<span class="customize-control-title"><?php esc_html_e( 'Social icons', 'avadanta' ); ?></span>
		<?php
		if(!empty($value)) {
			...
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<input type="text" class="customizer-repeater-social-repeater-link"
					       placeholder="<?php esc_attr_e( 'Link', 'avadanta' ); ?>">
					<input type="hidden" class="customizer-repeater-social-repeater-id" value="">
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'avadanta' ); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
			</div>
			<button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add Icon', 'avadanta' ); ?></button>
			<?php
		} else { ?>
			<div class="customizer-repeater-social-repeater">
				<?php
					...
					<div class="customizer-repeater-social-repeater-container">
						...
						<button class="social-repeater-remove-social-item"
						        style="<?php if ( $show_del == 1 ) {
							        echo "display:none";
						        } ?>"><?php esc_html_e( 'Remove Icon', 'avadanta' ); ?></button>
					</div>
					<?php
				} ?>
