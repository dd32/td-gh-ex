<?php
	...
	if($show_success_notice) :
		...
		<div class="action-tab success">
			<h3><?php printf( 'All Recommended action has been successfully completed.', 'accesspress-basic'); ?></h3>
			<a class="button button-primary" href="<?php echo esc_url(admin_url('customize.php')); ?>"><?php echo esc_html__('Customize Theme', 'accesspress-basic'); ?></a>
		</div>
		<?php
	else :
		...
		foreach($req_plugins as $plugin) :
			if($plugin['bundled'] == false) {
				...
				if(!isset($info->errors)) :
					...
					switch($status) {
						case 'install' :
							...
							$label = esc_html__('Install and Activate', 'accesspress-basic');
							break;

						case 'inactive' :
							$btn_class = 'button';
							$label = esc_html__('Deactivate', 'accesspress-basic');
							break;

						case 'active' :
							$btn_class = 'activate button button-primary';
							$label = esc_html__('Activate', 'accesspress-basic');
							break;
					}
					...
					<div class="action-tab warning">
						<h3><?php /* translators: %s : plugin name */ printf( esc_html__("Install : %s Plugin", 'accesspress-basic'), esc_html($info->name) ); ?></h3>
						<p><?php esc_html_e('Install Contact Form 7 to add the contact forms.', 'accesspress-basic'); ?></p>
				...
				?>
				<div class="action-tab warning">
					<h3><?php /* translators: %s : plugin name */ printf( esc_html__("Install : %s Plugin", 'accesspress-basic'), esc_html($plugin['name']) ); ?></h3>
					<p><?php esc_html_e('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'accesspress-basic'); ?></p>
