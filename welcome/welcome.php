<?php
	if(!class_exists('Accesspress_Mag_Welcome')) :

		class Accesspress_Mag_Welcome {
			...
			public function __construct() {
				...
				$this->req_plugins = array(
					...
					'instant-demo-importer' => array(
						...
						'name' => __('Instant Demo Importer', 'accesspress-mag'),
						...
						'info' => __('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'accesspress-mag'),
					),
				...
				$this->tab_sections = array(
					'getting_started' => __('Getting Started', 'accesspress-mag'),
				...
				$this->free_plugins = array(
					...
					'accesspress-anonymous-post' => array(
						'slug' => 'accesspress-anonymous-post',
						'filename' => 'accesspress-anonymous-post.php',
						'class' => 'AP_Class'
					),
				...
				add_action( 'load-themes.php', array( $this, 'accesspressmag_activation_admin_notice' ) );

				/* Create a Welcome Page */
				add_action( 'admin_menu', array( $this, 'accesspressmag_welcome_register_menu' ) );

				/* Enqueue Styles & Scripts for Welcome Page */
				add_action( 'admin_enqueue_scripts', array( $this, 'accesspressmag_welcome_styles_and_scripts' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_accesspressmag_plugin_installer', array( $this, 'accesspressmag_plugin_installer_callback' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_accesspressmag_plugin_offline_installer', array( $this, 'accesspressmag_plugin_offline_installer_callback' ) );

				/** Plugin Activation Ajax **/
				add_action( 'wp_ajax_accesspressmag_plugin_activation', array( $this, 'accesspressmag_plugin_activation_callback' ) );

				/** Plugin Activation Ajax (Offline) **/
				add_action( 'wp_ajax_accesspressmag_plugin_offline_activation', array( $this, 'accesspressmag_plugin_offline_activation_callback' ) );
			...
			public function accesspressmag_activation_admin_notice() {
				global $pagenow;
			...
			public function accesspressmag_welcome_register_menu() {
				$not = $this->get_required_plugin_notification();
				$title = $not > 0 ? 'Welcome <span class="pending-tasks">'.$not.'</span>' : esc_html__( 'Welcome', 'accesspress-mag' );
				add_theme_page( 'Welcome', $title , 'edit_theme_options', 'accesspressmag-welcome', array( $this, 'accesspressmag_welcome_screen' ));
			}
			...
			public function accesspressmag_welcome_styles_and_scripts() {
				...
				wp_localize_script( 'accesspress-basic-welcome-screen', 'accesspressmagWelcomeObject', array(
					'admin_nonce'	=> wp_create_nonce('accesspressmag_plugin_installer_nonce'),
					'activate_nonce'	=> wp_create_nonce('accesspressmag_plugin_activate_nonce'),
					'ajaxurl'		=> esc_url( admin_url( 'admin-ajax.php' ) ),
					'activate_btn' => __('Activate', 'accesspress-mag'),
					'installed_btn' => __('Activated', 'accesspress-mag'),
					'demo_installing' => __('Installing Demo', 'accesspress-mag'),
					'demo_installed' => __('Demo Installed', 'accesspress-mag'),
					'demo_confirm' => __('Are you sure to import demo content ?', 'accesspress-mag'),
				) );
			}

			/** Plugin API **/
			public function accesspressmag_call_plugin_api( $plugin ) {
			...
			/** Check For Icon **/
			public function accesspressmag_check_for_icon( $arr ) {
				if ( ! empty( $arr['svg'] ) ) {
			...
			public function accesspressmag_plugin_active($plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
				$status = 'install';
			...
			public function accesspressmag_plugin_generate_url($status, $plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
			...
			public function accesspressmag_plugin_installer_callback(){
				...
				// Get Plugin Info
				$api = $this->accesspressmag_call_plugin_api($plugin);
			...
			public function accesspressmag_plugin_activation_callback(){
				...
				// Get Plugin Info
				$api = $this->accesspressmag_call_plugin_api(esc_attr($plugin));
				...
				if($api->name){
					...
					$status = 'failed';
					$msg = esc_html__('There was an error activating $api->name', 'accesspress-mag');
				}
			...
			public static function get_plugin_file( $plugin_slug ) {
	      	...
	      	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=accesspressmag-welcome&section=import_demo'),'accesspressmag-file-installation');
	...
	if(class_exists('Instant_Demo_Importer')) :
		...
		$demoimporter->demos = array(
			'accesspress-mag' => array(
				'title' => __('Parallax Demo', 'accesspress-mag'),
				'name' => 'accesspress-mag',
