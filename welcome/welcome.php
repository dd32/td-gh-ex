<?php
	if(!class_exists('Accesspress_Basic_Welcome')) :
		...
		class Accesspress_Basic_Welcome {
			...
			public function __construct() {
				...
				$this->req_plugins = array(
					...
					'instant-demo-importer' => array(
						...
						'name' => __('Instant Demo Importer', 'accesspress-basic'),
						...
						'info' => __('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'accesspress-basic'),
					),
				...
				$this->tab_sections = array(
					'getting_started' => __('Getting Started', 'accesspress-basic'),
					'recommended_plugins' => __('Recommended Plugins', 'accesspress-basic'),
					'import_demo' => __('Import Demo', 'accesspress-basic'),
				...
				add_action( 'load-themes.php', array( $this, 'accesspressbasic_activation_admin_notice' ) );

				/* Create a Welcome Page */
				add_action( 'admin_menu', array( $this, 'accesspressbasic_welcome_register_menu' ) );

				/* Enqueue Styles & Scripts for Welcome Page */
				add_action( 'admin_enqueue_scripts', array( $this, 'accesspressbasic_welcome_styles_and_scripts' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_accesspressbasic_plugin_installer', array( $this, 'accesspressbasic_plugin_installer_callback' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_accesspressbasic_plugin_offline_installer', array( $this, 'accesspressbasic_plugin_offline_installer_callback' ) );

				/** Plugin Activation Ajax **/
				add_action( 'wp_ajax_accesspressbasic_plugin_activation', array( $this, 'accesspressbasic_plugin_activation_callback' ) );

				/** Plugin Activation Ajax (Offline) **/
				add_action( 'wp_ajax_accesspressbasic_plugin_offline_activation', array( $this, 'accesspressbasic_plugin_offline_activation_callback' ) );
			...
			public function get_required_plugin_notification() {
				...
				$accesspressbasic_plugin_installed_notif = get_option('accesspressbasic_plugin_installed_notif');
				...
				update_option('accesspressbasic_plugin_installed_notif', $notif_counter);
				return $notif_counter;
			}

			/** Welcome Message Notification on Theme Activation **/
			public function accesspressbasic_activation_admin_notice() {
				global $pagenow;
			...
			public function accesspressbasic_welcome_register_menu() {
				$action_count = get_option('accesspressbasic_plugin_installed_notif');
				$title        = $action_count > 0 ? esc_html__( 'Welcome', 'accesspress-basic' ) : esc_html__( 'Welcome', 'accesspress-basic' );
				add_theme_page( 'Welcome', $title , 'edit_theme_options', 'accesspressbasic-welcome', array( $this, 'accesspressbasic_welcome_screen' ));
			}
			...
			public function accesspressbasic_welcome_styles_and_scripts() {
				...
				wp_localize_script( 'accesspress-basic-welcome-screen', 'accesspressbasicWelcomeObject', array(
					'admin_nonce'	=> wp_create_nonce('accesspressbasic_plugin_installer_nonce'),
					'activate_nonce'	=> wp_create_nonce('accesspressbasic_plugin_activate_nonce'),
					'ajaxurl'		=> esc_url( admin_url( 'admin-ajax.php' ) ),
					'activate_btn' => __('Activate', 'accesspress-basic'),
					'installed_btn' => __('Activated', 'accesspress-basic'),
					'demo_installing' => __('Installing Demo', 'accesspress-basic'),
					'demo_installed' => __('Demo Installed', 'accesspress-basic'),
					'demo_confirm' => __('Are you sure to import demo content ?', 'accesspress-basic'),
				) );
			}

			/** Plugin API **/
			public function accesspressbasic_call_plugin_api( $plugin ) {
			...
			/** Check For Icon **/
			public function accesspressbasic_check_for_icon( $arr ) {
				if ( ! empty( $arr['svg'] ) ) {
			...
			public function accesspressbasic_plugin_active($plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
				$status = 'install';
			...
			public function accesspressbasic_plugin_generate_url($status, $plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
			...
			public function accesspressbasic_plugin_installer_callback(){
				...
				// Get Plugin Info
				$api = $this->accesspressbasic_call_plugin_api($plugin);
			...
			public function accesspressbasic_plugin_activation_callback(){
				...
				// Get Plugin Info
				$api = $this->accesspressbasic_call_plugin_api(esc_attr($plugin));
				...
				if($api->name){
					...
					$status = 'failed';
					$msg = esc_html__('There was an error activating $api->name', 'accesspress-basic');
				}
			...
			public static function get_plugin_file( $plugin_slug ) {
	      	...
	      	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=accesspressbasic-welcome&section=import_demo'),'accesspressbasic-file-installation');
	...
	if(class_exists('Instant_Demo_Importer')) :
		...
		$demoimporter->demos = array(
			'accesspress-basic' => array(
				'title' => __('Basic Demo', 'accesspress-basic'),
				'name' => 'accesspress-basic',
				'screenshot' => get_template_directory_uri().'/welcome/demos/accesspress-basic/screen.png',
				'home_page' => 'home-2',
				'menus' => array(
					'Main Menu' => 'primary'
				)
		...
		$demoimporter->options_replace_url = 'http://demo.accesspressthemes.com/accesspress-basic'; // Set the url to be replaced with current siteurl
		$demoimporter->option_name = 'apbasic_options'; // Set the the name of the option if the theme is based on theme option
	endif;
?>