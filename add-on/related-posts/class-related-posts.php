...
class Related_Posts {
	...
	public static function init() {
		...
		add_action( 'aamla_after_site_content', [ self::get_instance(), 'related_posts' ] );
		add_action( 'aamla_inside_main_content', [ self::get_instance(), 'remove_nav' ], 9 );
	}
	...
	public function related_posts() {
		...
		if ( ! ( is_single() && aamla_get_mod( 'aamla_related_posts', 'none' ) ) ) {
			return;
		}

		$number_posts = apply_filters( 'aamla_number_of_related_posts', 3 );
	...
	public function display_posts( $query ) {
		...
		echo '<div class="aml-related-posts">';
		echo '<h3 class="aml-related-posts-title">';
		esc_html_e( 'You may also like', 'aamla' );
		echo '</h3>';

		echo '<div class="aml-related-posts-content dp-wrapper grid-t index-view dp-grid dp-grid-3">';
		while ( $query->have_posts() ) :
			$query->the_post();
			aamla_get_template_partial( 'add-on/related-posts/templates', 'entry' );
		endwhile;
		echo '</div></div>';
	}
	...
	public function remove_nav() {
		if ( aamla_get_mod( 'aamla_related_posts', 'none' ) ) {
			remove_action( 'aamla_inside_main_content', 'aamla_post_navigation' );
		}
	}
}
Related_Posts::init();
