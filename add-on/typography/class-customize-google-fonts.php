...
class Customize_Google_Fonts extends \WP_Customize_Control {
	...
	public function to_json() {
		parent::to_json();
		$this->json['sans']  = Typography::get_instance()->get_google_sans_fonts();
		$this->json['serif'] = Typography::get_instance()->get_google_serif_fonts();
		...
		$this->json['weight']['choices']  = Typography::get_instance()->get_font_weights();
		...
		<# } #>
			...
			<p class="google-font-family">
				<select class="aamla-select" {{{ data.family.link }}}>
					...
					<# if ( data.sans ) { #>
						<optgroup label="<?php esc_html_e( 'Sans Serif Fonts', 'aamla' ); ?>">
					...
					<# if ( data.serif ) { #>
						<optgroup label="<?php esc_html_e( 'Serif Fonts', 'aamla' ); ?>">
			...
			<div class="google-font-additional-settings">
				<div class="google-font-selectors">
					...
					<# } #>
					<input type="text" value="{{ data.selectors.value }}" placeholder="<?php esc_html_e( 'examlple: .site-title, #main', 'aamla' ); ?>" {{{ data.selectors.link }}} />
				</div>
