 ...
 * new template file for each specific one. For example, Aadya already
		...
		<?php if ( have_posts() ) : ?>
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					...
					if ( is_day() ) :
						printf( __( 'Daily Archives: %s', 'aadya' ), '<span>' . get_the_date() . '</span>' );
					elseif ( is_month() ) :
						printf( __( 'Monthly Archives: %s', 'aadya' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'aadya' ) ) . '</span>' );
					elseif ( is_year() ) :
						printf( __( 'Yearly Archives: %s', 'aadya' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'aadya' ) ) . '</span>' );
					else :
						_e( 'Archives', 'aadya' );
					endif;
				?></h1>
