      ...
      <?php if(get_theme_mod('footer_widgets',true)){ ?>
        ...
        <div class="ftr-bottom">
          <div class="row">
            ...
            <a href="<?php echo esc_url(home_url('/')); ?>" class="ftr-logo"> <?php echo bloginfo('name');  ?></a>
           </div>
           <?php if(get_theme_mod('footer_copyright_bar',true)){ ?>
            <div class="col-lg-4 col-md-5 col-sm-6 col-xs-12 pull-right">
                <?php if(get_theme_mod('copyright')): ?>
	     <p class="copyright"><?php echo do_shortcode(get_theme_mod('copyright' , esc_html__('Copyright', 'backyard'))); ?></p>
             <?php else : ?>
             <p class="copyright"><?php $footertext = '[copyright] [the-year] [site-name] [theme-credit]';
        		...
        		$footertext = str_replace('[the-year]',date_i18n(__('Y','backyard')),$footertext);
        		$footertext = str_replace('[site-name]',get_bloginfo('name'),$footertext);
        		$footertext = str_replace('[theme-credit]',''.esc_html__('- WordPress Theme by','backyard').' <a href="'.esc_url(__( 'http://www.ydesignservices.com/', 'backyard')).'" target="_blank">'.esc_html__('Y Design Services','backyard').'</a>',$footertext);
