			...
			<div class="site-footer-left">
				...
				<?php if ( ! dynamic_sidebar( 'footer' ) ) : // footer widgetized area ?>
					...
					<?php
						...
						echo '&copy; '.date( 'Y' ).' ';
						echo '<a href="'.esc_url( home_url( '/' ) ).'" title="'.esc_attr( get_bloginfo( 'name', 'display' ) ).$description.'">'.get_bloginfo( 'name' ).'</a>';
					?>
		...
		<div class="col-sm-4">
			<div class="site-footer-right text-right">
				...
				<?php if ( is_home() || is_front_page() ) : // show credit links only on homepage
					...
					<a href="http://wordpress.org/" title="<?php _e( 'WordPress CMS', 'activetab' ); ?>"><?php _e( 'WordPress', 'activetab' ); ?></a><?php _e( ', ', 'activetab' ); ?>
