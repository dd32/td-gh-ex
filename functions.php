...
if ( ! function_exists( 'rockers_scripts_styles' ) ) : 
...
if ( ! function_exists( 'rockers_wp_title' ) ) : 
...
if ( ! function_exists( 'rockers_page_menu_args' ) ) : 
...
if ( ! function_exists( 'rockers_excerpt_length' ) ) : 
...
if ( ! function_exists( 'rockers_continue_reading_link' ) ) : 
    function rockers_continue_reading_link() {
        global $post;
        return '<p><a href="'. get_permalink( $post->ID ) .'" title="' . __('continue to read', 'rockers') . '">' . __('Continue to read...', 'rockers') . '</a></p>';
    }
endif;

if ( ! function_exists( 'rockers_auto_excerpt_more' ) ) : 
...
if ( ! function_exists( 'rockers_custom_excerpt_more' ) ) : 
...
if ( ! function_exists( 'rockers_widgets_init' ) ) : 
                    ...
                    <article id="comment-<?php comment_ID(); ?>" class="comment">
                        <header class="comment-meta comment-author vcard">
                            <?php echo get_avatar( $comment, 44 );
                                printf( '<cite class="fn">%1$s %2$s</cite>', get_comment_author_link(), ( $comment->user_id === $post->post_author ) ? '<span> ' . __( 'Post author', 'rockers' ) . '</span>' : '' );
                                printf( '<a href="%1$s"><time pubdate datetime="%2$s">%3$s</time></a>', 
                                    esc_url( get_comment_link( $comment->comment_ID ) ), 
                                    get_comment_time( 'c' ), 
                                    sprintf( 
                                        __( '%1$s at %2$s', 'rockers' ), 
                                        get_comment_date(), 
                                        get_comment_time() 
                                    ) 
                                ); ?>
                        ...
                        <div class="reply">
                            <?php comment_reply_link( 
                                array_merge( 
                                    $args, 
                                    array( 
                                        'reply_text' => __( 'Reply <span>&darr;</span>', 'rockers' ), 
                                        'depth' => $depth, 
...
if ( ! function_exists( 'rockers_entry_meta' ) ) : 
    function rockers_entry_meta() {
        ...
        $author = sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
            ...
            esc_attr( 
                sprintf( 
                    __( 'View all posts by %s', 'rockers' ), 
                    get_the_author() 
                ) 
            ),
            get_the_author()
