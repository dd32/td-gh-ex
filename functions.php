...
//FILTERS
	...
	//engines. If no short enough suffix is found, the default is used (blog title)
		...
		//Return the title_suffix
			function make_title($separator = "|"){
				$available_suffixes = get_option('appliance_dynamic_suffixes',array(get_bloginfo('description')));
				...
				//If it's not the frontpage and this feature is enabled, find an appropriate suffix in the array
					if(!is_front_page() && get_option('appliance_use_dynamic_suffix',false)){
	...
	//set, then the bloginfo description will be used. If disabled, the default behavior will be used.		
		...
		//Fill the description tags with a custom description, an excerpt or the blog description
			...
			if ( get_option('appliance_use_dynamic_descriptions',false) ){
	...
	//Use the "x days ago" date format
		if( get_option('appliance_use_human_readable_dates',false) ){
			function time_ago_date($date){
				return sprintf( _x("Posted %s ago",'The %s parameter is a date like "5 days" or "3 minutes"','appliance'), human_time_diff(get_the_time('U'), current_time('timestamp')) );
			}
		...
		function custom_admin_js() {
			echo '
				<script type="text/javascript">
					...
					elem.innerHTML = "<img src=\''.get_bloginfo('template_directory').'/img/mediaupload.png\'/>'.__('Click to add pictures or videos','appliance').'";</script>
			';
		}
...
//ADMIN
	...
	//Hide specific admin menus from non-admin users (if activated)
		if(!current_user_can('administrator')){ //Only hide menus for non-admins
			function remove_menus () {
				...
				if(get_option('appliance_hide_posts_menu',false)) 		array_push($menus_to_hide,__('Posts'));
				if(get_option('appliance_hide_pages_menu',false)) 		array_push($menus_to_hide,__('Pages'));
				if(get_option('appliance_hide_comments_menu',false)) 		array_push($menus_to_hide,__('Comments'));
				if(get_option('appliance_hide_media_menu',false)) 		array_push($menus_to_hide,__('Media'));
				if(get_option('appliance_hide_links_menu',false)) 		array_push($menus_to_hide,__('Links'));
				if(get_option('appliance_hide_profile_menu',false)) 		array_push($menus_to_hide,__('Profile'));
				if(get_option('appliance_hide_tools_menu',false)) 		array_push($menus_to_hide,__('Tools'));
	...
	//Disable the admin bar for logged in users
		if(get_option('appliance_hide_admin_bar',false)==true){
	...
	//Hide the admin bar logo for logged in users
		...
		if(get_option('appliance_hide_admin_bar_logo',false)==true){
			add_action('wp_before_admin_bar_render', 'annointed_admin_bar_remove', 0);
		}
		
	//Add the plugin options page
		add_action('admin_menu', 'appliance_barebones_menu');
		function appliance_barebones_menu() {
			add_theme_page('Theme options', 'Theme options', 'manage_options', 'appliance', 'appliance_theme_options');
		}
		function appliance_theme_options() {
	...
	//Hide the description and URL fields for attachments, as well as "Insert into post"
		function hide_attachment_fields($form_fields, $post) {
			if(!current_user_can('administrator')){
				if(get_option('appliance_hide_attachment_caption',false)==true){
					$form_fields['post_excerpt']['value'] = '';
					$form_fields['post_excerpt']['input'] = 'hidden';
				}
				if(get_option('appliance_hide_attachment_description',false)==true){
					$form_fields['post_content']['value'] = '';
					$form_fields['post_content']['input'] = 'hidden';
				}
				if(get_option('appliance_hide_attachment_link',false)==true){
	...
	//Hide file upload tabs
		function remove_media_library_tab($tabs) {
			if (!current_user_can('administrator') && get_option('appliance_hide_attachment_library',false)==true && isset($_REQUEST['post_id'])) {
				unset($tabs['library']);
			}
			return $tabs;
		}
