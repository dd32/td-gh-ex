...
add_theme_support( 'custom-background', $args );	
	...
	register_nav_menus( array(
		'primary' => __( 'Primary Navigation', 'agency' ),
	) );
...
<?php } ?>
<?php
add_filter('get_comments_number', 'agency_comment_count', 0);
...
?>
<?php
if ( ! function_exists( 'agency_comment' ) ) :
function agency_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		...
		<div class="comment-author vcard">
			...
			<?php printf( __( '%s', 'agency' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
		</div><!-- .comment-author .vcard -->
		<?php if ( $comment->comment_approved == '0' ) : ?>
			<em><?php _e( 'Your comment is awaiting moderation.', 'agency' ); ?></em>
			<br />
		<?php endif; ?>
		...
		<div class="comment-meta commentmetadata"><a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
			<?php
				...
				printf( __( '%1$s at %2$s', 'agency' ), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( __( '(Edit)', 'agency' ), ' ' );
			?>
	...
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'agency' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __('(Edit)', 'agency'), ' ' ); ?></p>
	<?php
			break;
	endswitch;
}
 ...
 *
 * To override agency_widgets_init() in a child theme, remove the action hook and add your own
 * function tied to the init hook.
 */
function agency_widgets_init() {
	// Area 1, located at the top of the sidebar.
	register_sidebar( array(
		'name' => __( 'Primary Widget Area', 'agency' ),
		'id' => 'primary-widget-area',
		'description' => __( 'The primary widget area', 'agency' ),
	...
	register_sidebar( array(
		'name' => __( 'First Footer Widget Area', 'agency' ),
		'id' => 'first-footer-widget-area',
		'description' => __( 'The first footer widget area', 'agency' ),
		'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
		'after_widget' => '</li>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => __( 'Second Footer Widget Area', 'agency' ),
		'id' => 'second-footer-widget-area',
		'description' => __( 'The second footer widget area', 'agency' ),
		'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
		'after_widget' => '</li>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => __( 'Third Footer Widget Area', 'agency' ),
		'id' => 'third-footer-widget-area',
		'description' => __( 'The third footer widget area', 'agency' ),
		'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
		'after_widget' => '</li>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => __( 'Fourth Footer Widget Area', 'agency' ),
		'id' => 'fourth-footer-widget-area',
		'description' => __( 'The fourth footer widget area', 'agency' ),
		'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
		'after_widget' => '</li>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
...
function agency_posted_on() {
	printf( __( '%2$s <span class="meta-sep">by</span> %3$s', 'agency' ),
		...
		sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
			...
			sprintf( esc_attr__( 'View all posts by %s', 'agency' ), get_the_author() ),
			get_the_author()
		)
	);
}
endif;
/** Register sidebars by running agency_widgets_init() on the widgets_init hook. */
...
function agency_auto_excerpt_more( $more ) {
	return ' &hellip;' ;
}
add_filter( 'excerpt_more', 'agency_auto_excerpt_more' );
...
add_filter( 'agency_blog_template_query_args', 'agency_exclude_categories_blogtemplate' );

function agency_exclude_categories_blogtemplate ( $args ) {

	if ( ! function_exists( 'agency_prepare_category_ids_from_option' ) ) { return $args; }

	$excluded_cats = array();

	// Process the category data and convert all categories to IDs.
	$excluded_cats = agency_prepare_category_ids_from_option( 'exclude_cat' );
...
/*-----------------------------------------------------------------------------------*/
/* agency_prepare_category_ids_from_option()
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'agency_prepare_category_ids_from_option' ) ) {

	function agency_prepare_category_ids_from_option ( $option ) {
...
function agency_filter_wp_title( $old_title, $sep, $sep_location ){
...
add_filter( 'wp_title', 'agency_filter_wp_title', 10, 3 );

function agency_of_register_js() {
	if (!is_admin()) {
...
function agency_of_single_scripts() {
	if(is_singular()) wp_enqueue_script( 'comment-reply' ); // loads the javascript required for threaded comments 
}
add_action('wp_print_scripts', 'agency_of_single_scripts');
