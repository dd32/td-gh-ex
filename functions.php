...
function advance_automobile_widgets_init() {
	...
	for ($i=1; $i<=$advance_automobile_widget_areas; $i++) {
		register_sidebar( array(
	...
	register_sidebar( array(
		'name'          => __( 'Shop Page Sidebar', 'advance-automobile' ),
		'description'   => __( 'Appears on shop page', 'advance-automobile' ),
	...
	register_sidebar( array(
		'name'          => __( 'Single Product Page Sidebar', 'advance-automobile' ),
		'description'   => __( 'Appears on shop page', 'advance-automobile' ),
...
function advance_automobile_sanitize_phone_number( $phone ) {
	return preg_replace( '/[^\d+]/', '', $phone );
}

function advance_automobile_sanitize_email( $email, $setting ) {
	$email = sanitize_email( $email );
	return ( ! is_null( $email ) ? $email : $setting->default );
}

function advance_automobile_sanitize_checkbox( $input ) {
	return ( ( isset( $input ) && true == $input ) ? true : false );
}

function advance_automobile_sanitize_float( $input ) {
	return filter_var($input, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
}

function advance_automobile_sanitize_number_range( $number, $setting ) {
...
if (!function_exists('advance_automobile_loop_columns')) {
	function advance_automobile_loop_columns() {
		$columns = get_theme_mod( 'advance_automobile_wooproducts_per_columns', 3 );
		return $columns; // 3 products per row
	}
}

//Change number of products that are displayed per page (shop page)
add_filter( 'loop_shop_per_page', 'advance_automobile_shop_per_page', 20 );
function advance_automobile_shop_per_page( $cols ) {
  	$cols = get_theme_mod( 'advance_automobile_wooproducts_per_page', 9 );
	return $cols;
}
...
function advance_automobile_scripts() {
	...
	// blocks-css
	wp_enqueue_style( 'advance-automobile-block-style', get_theme_file_uri('/css/blocks.css') );
	...
	wp_enqueue_style('advance-automobile-customcss', esc_url(get_template_directory_uri()).'/css/custom.css');
	...
	wp_enqueue_script('advance-automobile-customscripts-jquery', esc_url(get_template_directory_uri()).'/js/custom.js', array('jquery'));
	...
	wp_enqueue_style('advance-automobile-ie', esc_url(get_template_directory_uri()).'/css/ie.css', array('advance-automobile-basic-style'));
