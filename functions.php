	...
	function actinia_setup() {
		...
		*/
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'actinia_custom_background_args',
				array(
...
function actinia_scripts() {
	...
	wp_enqueue_script( 'actinia-functions', get_template_directory_uri() . '/js/functions.js', array(), '20151215', true );

	wp_enqueue_script( 'actinia-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );

	wp_enqueue_script( 'actinia-layout', get_template_directory_uri() . '/js/layout.js', array(), '20151215', true );
...
function actinia_excerpt_more( $more ) {
	return sprintf(
		' <a href="' . esc_url( get_permalink() ) . '" class="more-link"><span>%s</span></a>',
		esc_html__( '. . .', 'actinia' )
	);
}
add_filter( 'excerpt_more', 'actinia_excerpt_more' );

function actinia_modify_read_more_link() {
	return sprintf(
		' <a href="' . esc_url( get_permalink() ) . '" class="more-link more-link-newline"><span>%s</span></a>',
		esc_html__( '. . .', 'actinia' )
	);
}
add_filter( 'the_content_more_link', 'actinia_modify_read_more_link' );

function actinia_excerpt_length( $length ) {
	return 80;
}
add_filter( 'excerpt_length', 'actinia_excerpt_length', 999 );
