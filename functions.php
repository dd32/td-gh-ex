...
function alante_thinkup_child_frontscripts() {
	...
	wp_enqueue_style( 'alante-thinkup-style-emagazine', get_stylesheet_directory_uri() . '/style.css', array( 'alante-thinkup-style' ), wp_get_theme()->get('Version') );
}
...
function alante_thinkup_updateoption_child_settings() {
	...
	$name_child_color   = $name_theme_lower . '_thinkup_child_color_emagazine';
	...
	if( ! empty( $options_free ) ) {
		...
		if ( $options_child_settings != 1 ) {
			...
			$options_free['thinkup_styles_skin']       = 'emagazine';
