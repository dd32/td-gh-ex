...
if ( ! function_exists( 'virality_theme_customizer' ) ) :
	...
	function virality_theme_customizer( $wp_customize ) {
		...
		/* nav menu option */
		$wp_customize->add_section( 'virality_enable_nav_section' , array(
			'title'       => __( 'Enable header nav menu', 'virality' ),
			'priority'    => 32,
			'description' => __( 'Option to show/hide the navigation menu on header.', 'virality' ),
		) );
		
		$wp_customize->add_setting( 'virality_enable_nav', array (
			'sanitize_callback' => 'virality_sanitize_checkbox',
		) );
		
		 $wp_customize->add_control('nav_menu', array(
			'settings' => 'virality_enable_nav',
			'label' => __('Enable header nav menu?', 'virality'),
			'section' => 'virality_enable_nav_section',
			'type' => 'checkbox',
		));
		...
		/* social share buttons in posts option */
		$wp_customize->add_section( 'virality_social_share_section' , array(
			'title'       => __( 'Disable social share buttons', 'virality' ),
			'priority'    => 34,
			'description' => __( 'Option to show/hide the Facebook and Twitter share buttons in the posts.', 'virality' ),
		) );
		
		$wp_customize->add_setting( 'virality_social_share', array (
			'sanitize_callback' => 'virality_sanitize_checkbox',
		) );
		
		 $wp_customize->add_control('social_share', array(
			'settings' => 'virality_social_share',
			'label' => __('Disable social share buttons?', 'virality'),
			'section' => 'virality_social_share_section',
			'type' => 'checkbox',
		));
 ...
 * Facebook Open Graph
 */
add_action('wp_head', 'virality_fb_open_graph_tags');
function virality_fb_open_graph_tags() {
	if (is_single()) {
		global $post;
		...
		$description = virality_excerpt( $post->post_content, $post->post_excerpt );
...
function virality_excerpt($text, $excerpt){
