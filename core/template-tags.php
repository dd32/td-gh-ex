...
function igthemes_posts_navigation() {
    if (igthemes_option('numeric_pagination')) {
        ...
        echo paginate_links( array(
            ...
            'type'    => 'list',
            'prev_next'    => True,
            'prev_text'    => esc_html__('&#8592;', 'basic-shop'),
            'next_text'    => esc_html__('&#8594;', 'basic-shop'),
...
function igthemes_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( '%s', 'post date', 'basic-shop' ),
		'<span class="fa fa-clock-o"></span><a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( '%s', 'post author', 'basic-shop' ),
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'basic-shop' ) );
		if ( $categories_list && igthemes_categorized_blog() ) {
			printf( '<span class="cat-links"><span class="fa fa-folder-o"></span>' . esc_html__( '%1$s', 'basic-shop' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'basic-shop' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links"><span class="fa fa-tag"></span>' . esc_html__( '%1$s', 'basic-shop' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
...
function igthemes_entry_footer() {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'basic-shop' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
...
/**
 * Return an alternate title, without prefix, for every type used in the the_archive_title().
 */

add_filter('get_the_archive_title', function ($title) {
    if ( is_category() ) {
        $title = sprintf( __( '%s', 'basic-shop' ), single_cat_title( '', false ) );
    }
    return $title;
});
