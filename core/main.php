...
define( 'ANNA_LITE_MIN_PHP_VERSION', '5.3' );

/*-----------------------------------------------------------------------------------*/
/* Switches back to the previous theme if the minimum PHP version is not met */
/*-----------------------------------------------------------------------------------*/ 

if ( ! function_exists( 'anna_lite_check_php_version' ) ) {

	function anna_lite_check_php_version() {
	
		if ( version_compare( PHP_VERSION, ANNA_LITE_MIN_PHP_VERSION, '<' ) ) {
			add_action( 'admin_notices', 'anna_lite_min_php_not_met_notice' );
	...
	add_action( 'after_switch_theme', 'anna_lite_check_php_version' );
...
if ( ! function_exists( 'anna_lite_min_php_not_met_notice' ) ) {

	function anna_lite_min_php_not_met_notice() {
		?>
		<div class="notice notice-error is_dismissable">
			<p>
				<?php esc_html_e('You need to update your PHP version to run this theme.', 'anna-lite' ); ?><br />
				<?php
				printf(
					esc_html__( 'Actual version is: %1$s, required version is: %2$s.', 'anna-lite' ),
					PHP_VERSION,
					ANNA_LITE_MIN_PHP_VERSION
				);
				?>
			</p>
		</div>
...
if (!function_exists('anna_lite_scripts_styles')) {
	...
	function anna_lite_scripts_styles() {
		...
		wp_enqueue_script( 'anna-lite-template',get_template_directory_uri() . '/assets/js/anna-lite-template.js',array('jquery', 'imagesloaded', 'masonry'), '1.0.0', TRUE ); 

		if ( is_singular() ) wp_enqueue_script( 'comment-reply' );

		wp_enqueue_script( 'anna-lite-navigation', get_template_directory_uri() . '/assets/js/navigation.js' , array('jquery'), '1.0', TRUE ); 

		wp_localize_script( 'anna-lite-navigation', 'accessibleNavigationScreenReaderText', array(
			'expandMain'   => __( 'Open the main menu', 'anna-lite' ),
			'collapseMain' => __( 'Close the main menu', 'anna-lite' ),
			'expandChild'   => __( 'expand submenu', 'anna-lite' ),
			'collapseChild' => __( 'collapse submenu', 'anna-lite' ),
		));
