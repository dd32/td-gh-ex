...
if (!function_exists('avventura_lite_socials_function')) {

	function avventura_lite_socials_function() {
		...
		foreach ( $socials as $name => $attrs) { 
			...
			if (avventura_lite_setting('avventura_lite_footer_'.$name.'_button')): 

				$i++;	
				$html .= '<a href="'.esc_url(avventura_lite_setting('avventura_lite_footer_'.$name.'_button'), array( 'http', 'https', 'tel', 'skype', 'mailto' ) ).'" target="'.$attrs['target'].'" title="'.$name.'" button-title="'.ucwords($name).'" class="social"> <i class="'.$attrs['icon'].'" ></i> </a> ';
			
			endif;
			
		}
		
		if ( avventura_lite_setting('avventura_lite_footer_rss_button') == "on" ): 
	...
	add_action( 'avventura_lite_socials', 'avventura_lite_socials_function' );
