jQuery(document).ready(function ($) {
    ...
    $('.agency-ecommerce-plugin-install-notice-wrap a.button').click(function (e) {
        e.preventDefault();

        $(this).addClass('updating-message');
        $(this).text(agency_ecommerce_about_obj.button_text);
        var params = {
            data: {
                action: agency_ecommerce_about_obj.starter_site_install_action,
                security: agency_ecommerce_about_obj.nonce,
                request: 1
            },
            ajax_url: ajaxurl,
            ...
            success_callback: function (response, extra_params) {
                ...
                if (typeof response.redirect != "undefined") {
                    ...
                    if ('' !== agency_ecommerce_about_obj.redirect) {
                        window.location.href = agency_ecommerce_about_obj.redirect;
                    }
                }
            }

        };
        agency_ecommerce_install_recommanded_plugins(params);
    });
    ...
    function agency_ecommerce_install_recommanded_plugins(params) {
