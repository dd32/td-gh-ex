...
<?php } else { ?>
	...
	<?php while (have_posts()) : the_post(); ?>
		...
		<form action="<?php the_permalink(); ?>" id="contactForm" method="post">
				...
				<p class="textarea"><label for="commentsText">Message:</label></br>
					<?php if(isset ($commentError) ) { ?>
						<span class="error"><?php echo $commentError;?></span> 
					<?php } ?>
					<textarea name="comments" id="commentsText" rows="4" cols="45" class="requiredField"><?php if(isset($_POST['comments'])) { if(function_exists('stripslashes')) { echo stripslashes($_POST['comments']); } else { echo $_POST['comments']; } } ?></textarea>
				</p>
