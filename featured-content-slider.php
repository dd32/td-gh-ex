...
$slider_posts = smartline_get_featured_content();
...
if ( empty( $slider_posts ) and current_user_can( 'edit_theme_options' ) ) : ?>
	...
	<p class="frontpage-slider-empty-posts">
		<?php _e('There is no featured content to be displayed in the slider. To set up the slider, go to Appearance &#8594; Customize &#8594; Theme Options, and add a featured tag in the Post Slider section. The slideshow displays all your posts which are tagged with that keyword.', 'smartline-lite'); ?>
	</p>
		...
		<?php foreach ( $slider_posts as $post ) : setup_postdata( $post ); ?>
			...
			<?php endif;?>
				...
				<div class="slide-entry clearfix">
					...
					<a href="<?php esc_url(the_permalink()) ?>" class="slide-more-link"><?php _e('Read more &raquo;', 'smartline-lite'); ?></a>
				</div>
