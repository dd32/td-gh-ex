...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    <header class="archive-header">
        <h2 class="content-archive">
            <?php if (is_404()) {
                _e('Page Not Available', 'barista');
            } else if (is_search()) {
                printf(__('Nothing found for: <small>', 'barista') . get_search_query() . '</small>');
            } else {
                _e('Nothing Found', 'barista');
            }
            ?>
        </h2>
    </header>
    <div class="entry-content">
        ...
        <?php elseif (is_404()) : ?>
            <p><?php printf(__( "You tried going to %s, which doesn't exist, so that means I probably broke something.", 'barista'), '<code>' . home_url( esc_url( $_SERVER['REQUEST_URI'] ) ) . '</code>' ); ?></p>
            <?php get_search_form(); ?>
...
<div class="recent-posts">
    <?php if (is_404() || is_search()) { ?>
        <h3 class="recent-posts"><?php _e('Most Recent Posts', 'barista'); ?></h3>
            <ul>
                <?php
