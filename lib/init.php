...
// Register wp_nav_menu() menus (http://codex.wordpress.org/Function_Reference/register_nav_menus)
 ...
 add_theme_support( 'post-formats', array(
  ...
  add_image_size('featured_categories', 263, 190, true );
  add_image_size( 'popular-thumb', 256, 151, true );
  add_image_size( 'page-thumb', 750, 387, true );
  add_image_size('full-grid', 550, 350, true );
  add_post_type_support( 'attachment', 'page-atBackyards' );
...
function backyard_fav_output(){

  // Keep for fallback

  global $backyard;

  if(isset($backyard['favion_id']) && $backyard['favion_id']==1) {

  if(isset($backyard['favion_icon_upload']) && !empty($backyard['favion_icon_upload']) ) {

    echo '<link rel="shortcut icon" type="image/x-icon" href="'.esc_url($backyard['favion_icon_upload']).'" />';
...
add_action('wp_head', 'backyard_fav_output'); //front end

add_action('admin_head', 'backyard_fav_output'); //admin end
...
if ((is_user_logged_in()) && (!current_user_can('administrator') && !is_admin())) {
	global $backyard;
	if(isset($backyard['disable_wp_adminbar_id']) && $backyard['disable_wp_adminbar_id']==1) {
      show_admin_bar(true);
	}else 
	{
