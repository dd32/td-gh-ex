...
class LatestPostWidget extends WP_Widget
{
	function __construct()
	{
		...
		parent::__construct('LatestPostWidget', 'avocation Latest Post', $avocation_lp_widget_ops);
	}
	function form($avocation_lp_instance)
	{
		...
		<p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e( 'Title:', 'avocation' ); ?> <input id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($avocation_lp_title); ?>" /></label></p>
		 <p><label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php esc_html_e( 'Number of posts to show:', 'avocation' ); ?></label>
		<input id="<?php echo esc_attr($this->get_field_id( 'number' )); ?>" maxlength="2" name="<?php echo esc_attr($this->get_field_name( 'number' )); ?>" type="text" value="<?php echo esc_attr($avocation_lp_number); ?>" size="3" /></p> 
		 <p><input class="checkbox" type="checkbox" <?php esc_attr(checked( $avocation_lp_show_date )); ?> id="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'show_date' )); ?>" />
		 <label for="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>"><?php esc_html_e( 'Display post date?', 'avocation' ); ?></label></p>
		<?php
	}
	...
	echo $before_title . esc_html($avocation_lp_title) . $after_title;;
			...
			<li>
				<div class="popular-post">
	        	 ...
	        	 <?php if ( has_post_thumbnail() ) : ?>
						<?php the_post_thumbnail( 'avocation-latest-posts-widget', array( 'class' => 'img-responsive') ); ?>
					<?php endif; ?>
					</a>
					<div class="blog-date">
