...
?>
<?php
function appointway_optionsframework_add_admin() {
    global $query_string;
    
    $themename =  appointway_get_option('of_themename');      
    $shortname =  appointway_get_option('of_shortname'); 
   
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework' ) {
		if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
			$options =  appointway_get_option('of_template'); 
			appointway_reset_options($options,'optionsframework');
...
$of_page = add_theme_page($themename,'Theme Options','edit_theme_options', 'optionsframework', 'appointway_optionsframework_options_page', 'div');
	
	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$of_page", 'appointway_load_only');
} 
add_action('admin_menu', 'appointway_optionsframework_add_admin');
/*-----------------------------------------------------------------------------------*/
/* Options Framework Reset Function - of_reset_options */
/*-----------------------------------------------------------------------------------*/
function appointway_reset_options($options,$page = ''){
	global $wpdb;
	$count = 0;
	...
	if($page == 'optionsframework'){
		appointway_delete_option( 'of_options' );
	}
}
/*-----------------------------------------------------------------------------------*/
/* Build the Options Page - optionsframework_options_page */
/*-----------------------------------------------------------------------------------*/
function appointway_optionsframework_options_page(){
    $options =  appointway_get_option('of_template');      
    $themename =  appointway_get_option('of_themename');
?>
<div class="trail-notify">
        <h1>Get Appointway Theme PRO!</h1>
        <p style="font-size:15px; line-height: 20px;">You are using the Lite Version of Appointway Theme. Upgrade to Pro for extra features like Appointment Booking Integration, Home Page Slider, Contact Page, Gallery Page, FullWidth Page Templates, Multiple Color Options and much more.</p>
        <a href="http://www.inkthemes.com/wp-themes/appointway-wordpress-theme/" target="blank">Upgrade to Appointway PRO Theme here >></a>
    </div>
  ...
  <form action="" enctype="multipart/form-data" id="ofform">
   <?php wp_nonce_field('theme-update-option'); ?>
    ...
    <?php 
        ...
        $return = appointway_optionsframework_machine($options);
        ?>
    <div id="main">
      <div id="of-nav">
        <ul>
...
/*-----------------------------------------------------------------------------------*/
/* Ajax Save Action - appointway_ajax_callback */
/*-----------------------------------------------------------------------------------*/
add_action('wp_ajax_of_ajax_post_action', 'appointway_ajax_callback');
function appointway_ajax_callback() {
	...
	if($save_type == 'upload'){
		...
		$uploaded_file = wp_handle_upload($filename,$override);
				...
				appointway_update_option( $clickedID , $uploaded_file['url'] );
	...
	elseif ($save_type == 'options' OR $save_type == 'framework') {
		...
		foreach($options as $option_array){
			$id = $option_array['id'];
			$old_value = appointway_get_option($id);
			$new_value = '';
			...
			if(isset($option_array['id'])) { // Non - Headings...
					...
					if ( is_array($type)){
						foreach($type as $array){
							if($array['type'] == 'text'){
								...
								appointway_update_option( $id, stripslashes($new_value));
							}
						}                 
					}
					elseif($new_value == '' && $type == 'checkbox'){ // Checkbox Save
						
						appointway_update_option($id,'false');
					}
					elseif ($new_value == 'true' && $type == 'checkbox'){ // Checkbox Save
						
						appointway_update_option($id,'true');
					}
					elseif($type == 'multicheck'){ // Multi Check Save
						...
						foreach ($option_options as $options_id => $options_value){
							...
							if(!isset($output[$multicheck_id])){
							  appointway_update_option($multicheck_id,'false');
							}
							else{
							   appointway_update_option($multicheck_id,'true'); 
							}
						}
					} 
					elseif($type == 'typography'){
						...
						appointway_update_option($id,$typography_array);
					...
					elseif($type == 'border'){
						...
						appointway_update_option($id,$border_array);
...
/*-----------------------------------------------------------------------------------*/
function appointway_optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'text':
			$val = $value['std'];
			$std = appointway_get_option($value['id']);
			if ( $std != "") { $val = $std; }
		...
		break;
			...
			$select_value = appointway_get_option($value['id']);
		...
		case "upload":
			$value['std'] = '';
			if (isset($value['std'])){
			$output .= appointway_optionsframework_uploader_function($value['id'],$value['std'],null);
			}
		break;
		case "upload_min":
			
			$output .= appointway_optionsframework_uploader_function($value['id'],$value['std'],'min');
			
		break;
		case "color":
			$val = $value['std'];
			$stored  = appointway_get_option( $value['id'] );
		...
		case "typography":
			...
			$default = $value['std'];
			$typography_stored = appointway_get_option($value['id']);
		...
		case "border":
			...
			$default = $value['std'];
			$border_stored = appointway_get_option( $value['id'] );
		...
		if ( is_array($value['type'])) {
			foreach($value['type'] as $array){
					...
					$id = $array['id']; 
					$std = $array['std'];
					$saved_std = appointway_get_option($id);
...
/*-----------------------------------------------------------------------------------*/
/* OptionsFramework Uploader - appointway_optionsframework_uploader_function */
/*-----------------------------------------------------------------------------------*/
function appointway_optionsframework_uploader_function($id,$std,$mod){
	...
	$uploader = '';
    $upload = appointway_get_option($id);
	
	if($mod != 'min') { 
			$val = $std;
            if ( appointway_get_option( $id ) != "") { $val = appointway_get_option($id); }
