...
function multishop_custom_breadcrumbs() {
  ...
  $multishop_home = 'Home'; // text for the 'Home' link
  ...
  if (is_home() || is_front_page()) {
    ...
    if ( is_category() ) {
      ...
      echo $multishop_before . 'Archive by category "' . single_cat_title('', false) . '"' . $multishop_after;

    } elseif ( is_search() ) {
      echo $multishop_before . 'Search results for "' . get_search_query() . '"' . $multishop_after;

    } elseif ( is_day() ) {
      echo '<a href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a> ' . $multishop_delimiter . ' ';
      echo '<a href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a> ' . $multishop_delimiter . ' ';
      echo $multishop_before . get_the_time('d') . $multishop_after;

    } elseif ( is_month() ) {
      echo '<a href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a> ' . $multishop_delimiter . ' ';
      echo $multishop_before . get_the_time('F') . $multishop_after;

    } elseif ( is_year() ) {
      echo $multishop_before . get_the_time('Y') . $multishop_after;
      ...
      $multishop_post_type = get_post_type_object(get_post_type());
      echo $multishop_before . $multishop_post_type->labels->singular_name . $multishop_after;
      ...
      echo get_category_parents($multishop_cat, TRUE, ' ' . $multishop_delimiter . ' ');
      ...
      echo $multishop_before . 'Posts tagged "' . single_tag_title('', false) . '"' . $multishop_after;

    } elseif ( is_author() ) {
       global $author;
      $multishop_userdata = get_userdata($author);
      echo $multishop_before . 'Articles posted by ' . $multishop_userdata->display_name . $multishop_after;

    } elseif ( is_404() ) {
      echo $multishop_before . 'Error 404' . $multishop_after;
    }
