...
function bestblog_post_navi($before = '', $after = '') {
	...
	echo $before.'<nav class="page-navigation text-center"><ul class="pagination">'."";
	if ($start_page >= 2 && $pages_to_show < $max_page) {
		$first_page_text = esc_attr__( 'First', 'best-blog' );
		echo '<li><a href="'.esc_url(get_pagenum_link()).'" title="'.$first_page_text.'">'.$first_page_text.'</a></li>';
	}
	echo '<li>';
	previous_posts_link( esc_attr__('Previous', 'best-blog') );
	echo '</li>';
	...
	echo '<li>';
	next_posts_link( esc_attr__('Next', 'best-blog'), 0 );
	echo '</li>';
	if ($end_page < $max_page) {
		$last_page_text = esc_attr__( 'Last', 'best-blog' );
