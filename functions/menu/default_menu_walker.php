<?php 
function elitepress_page_menu_args( $args ) {
	if ( ! isset( $args['show_home'] ) )
		$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'elitepress_page_menu_args' );

 
function elitepress_fallback_page_menu( $args = array() ) {
	...
	if ( ! empty($args['show_home']) ) {
		if ( true === $args['show_home'] || '1' === $args['show_home'] || 1 === $args['show_home'] )
			$text = esc_html__('Home','elitepress');
		else
			$text = $args['show_home'];
		$class = '';
	...
	$list_args['title_li'] = '';
	$list_args['walker'] = new elitepress_walker_page_menu;
...
class elitepress_walker_page_menu extends Walker_Page{
