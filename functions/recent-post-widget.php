...
function __construct() {
  ...
  parent::__construct('multishop_recentpostwidget', ' '.__('Multishop Recent Post','multishop').' ', $multishop_widget_ops);
}
...
if(!empty($multishop_instance['post_number'])) { $multishop_instance['post_number']; } ?>
  <p>
    <label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_html_e('Title', 'multishop'); echo ":"; ?></label>
    <input id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" value="<?php if(!empty($multishop_instance['title'])) { echo esc_attr($multishop_instance['title']); } ?>" style="width:100%;" />
  </p>
  <p>
    <label for="<?php echo esc_attr($this->get_field_id( 'post_number' )); ?>"><?php esc_html_e('Number of post to show', 'multishop');  echo ":"; ?></label>
    <input id="<?php echo esc_attr($this->get_field_id( 'post_number' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'post_number' )); ?>" value="<?php if(!empty($multishop_instance['post_number'])) { echo esc_attr($multishop_instance['post_number']); } else { echo '5'; } ?>" style="width:100%;" />
  </p>
<?php
}
...
echo $before_title . esc_html($multishop_title) . $after_title;;
//widget code here ?>
