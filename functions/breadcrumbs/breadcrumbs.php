<?php
function wallstreet_custom_breadcrumbs() {
  $showOnHome = 0; // 1 - show breadcrumbs on the homepage, 0 - don't show
  $delimiter = ''; // delimiter between crumbs
  $home = esc_html__('Home','wallstreet'); // text for the 'Home' link
  ...
  if (is_home() || is_front_page()) {
    ...
    if ( is_category() ) {
      ...
      echo $before . esc_html__('Archive by category','wallstreet') . ' "'. single_cat_title('', false) . '"' . $after;
 
    } elseif ( is_search() ) {
      echo $before . esc_html__('Search results for ','wallstreet').' "'. get_search_query() . '"' . $after;
      ...
      echo $before . esc_html__('Posts tagged','wallstreet' ) . ' "'. single_tag_title('', false) . '"' . $after;
 
    } elseif ( is_author() ) {
       global $author;
      $userdata = get_userdata($author);
      echo $before . esc_html__('Articles posted by','wallstreet') . $userdata->display_name . $after;
 
    } elseif ( is_404() ) {
      echo $before . esc_html__('Error 404','wallstreet') . $after;
    }
 
    if ( get_query_var('paged') ) {
      if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
      echo ' ( ' . esc_html__('Page','wallstreet') . '' . get_query_var('paged'). ' )';
      if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
