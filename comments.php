...
if ( post_password_required() ) { ?>
	...
	<p class="nopassword"><?php echo esc_html__( 'This post is password protected. Enter the password to view any comments.', 'weaver-xtreme' ); ?></p>
	<?php
...
if ( comments_open() || ( $c_count > 0 && ! weaverx_getopt_checked( 'hide_old_comments' ) ) ) {
	...
	<header id="comments-title"<?php echo $ct_class; ?>>
		<h3><?php echo apply_filters( 'weaverx_comments_title', esc_html__( 'Comments', 'weaver-xtreme' ) ); ?></h3>
		<h4>
			<?php printf( "<em>%s</em> &#8212; ", get_the_title() );
			comments_number(); /* em-dash */ ?>
		</h4>
	</header>

	<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) { // are there comments to navigate through ?>
		<nav id="comment-nav-above">
			<h1 class="assistive-text"><?php echo esc_html__( 'Comment navigation', 'weaver-xtreme' ); ?></h1>
			<div class="nav-previous"><?php previous_comments_link( apply_filters( 'weaverx_older_comments', esc_html__( '&larr; Older Comments', 'weaver-xtreme' ) ) ); ?></div>
			<div class="nav-next"><?php next_comments_link( apply_filters( 'weaverx_newer_comments', esc_html__( 'Newer Comments &rarr;', 'weaver-xtreme' ) ) ); ?></div>
		</nav>
	...
	*/
	?>
	<p class="nocomments"><?php echo esc_html__( 'Comments are closed.', 'weaver-xtreme' ); ?></p>
	<?php
}
?>
