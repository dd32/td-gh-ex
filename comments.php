...
<?php if ( have_comments() ) : ?>
	...
	<?php do_action( 'themingstrap_comments_before' );?>
	<div class="alert alert-info">
		<h2 class="comments-title">
			<?php
			printf( _nx( '<span class="badge">1</span> Thought on &ldquo;%2$s&rdquo;', '<span class="badge">%1$s</span> Thoughts on &ldquo;%2$s&rdquo;', get_comments_number(), 'comments title', 'ThemingStrap' ),
		...
		<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // are there comments to navigate through ?>
			<nav id="comment-nav-below" class="comment-navigation" role="navigation">
				<ul class="pager">
					<li class="pull-right"><?php previous_comments_link( __( 'Older Comments &raquo;', 'ThemingStrap' ) ); ?></li>
					<li class="pull-left"><?php next_comments_link( __( '&laquo; Newer Comments', 'ThemingStrap' ) ); ?></li>
				</ul>
			</nav><!-- #comment-nav-below -->
		<?php endif; // check for comment navigation ?>
	</div><!-- #comments -->
<?php endif; // have_comments() ?>
<?php do_action( 'themingstrap_comments_after' );?>

<?php
// If comments are closed and there are comments
if ( ! comments_open() && '0' != get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) :
?>
	<div class="alert alert-info"><?php _e( 'Comments are closed.', 'ThemingStrap' ); ?></div>
<?php endif;
$args = array(
