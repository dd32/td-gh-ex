...
if ('comments.php' == basename($_SERVER['SCRIPT_FILENAME'])) {
    die(__('Please do not load this page directly. Thanks!', 'ad_mag_lite'));
}
...
if ( post_password_required() )
    ...
    if ( have_comments() ) { ?>
    <div id="comments">
        <h3><?php comments_number(__('No Comment', 'ad_mag_lite'), __('1 Comment', 'ad_mag_lite'), __('% Comments', 'ad_mag_lite')); ?></h3>
        ...
        if ( '' !== $prev_comments_link . $next_comments_link ) { ?>                
            <div class="pagination kopa-comment-pagination">
                <?php 
                $args = array(
                 'prev_text'    => __('Previous','ad_mag_lite'),
                 'next_text'    => __('Next','ad_mag_lite')
                );
        ...
        <article class="comment-wrap clearfix">
            ...
            <div class="media-body clearfix">
                <header class="clearfix">
                    <div class="pull-left">
                        <h4><?php _e( 'Pingback', 'ad_mag_lite' ); ?></h4>
                    ...
                    <div class="comment-button pull-right">
                        ...
                        edit_comment_link( __( 'Edit', 'ad_mag_lite' ) );
                        }?>
                    </div>
                </header>
                <p><a href="<?php if ( get_comment_author_url() ) { echo get_comment_author_url(); }?>" target="_blank" title="<?php _e('Pingback', 'ad_mag_lite');?>"><?php if ( get_comment_author_url() ) { echo get_comment_author_url(); }?></a></p>
            </div><!--comment-body -->
        </article>
    </li>
...
function ad_mag_lite_comment_form($args = array(), $post_id = null) {
    ...
    $fields['author'].= '<p class="input-label">'.__('Name *','ad_mag_lite').'</p>';
    ...
    $fields['email'].= '<p class="input-label">'.__('Email *','ad_mag_lite').'</p>';
    ...
    $fields['url'].= '<p class="input-label">'.__('Website','ad_mag_lite').'</p>';
    ...
    $defaults = array(
        ...
        'must_log_in'          => '<p class="must-log-in">' . sprintf(__('You must be <a href="%s">logged in</a> to post a comment.', 'ad_mag_lite'), wp_login_url(apply_filters('the_permalink', get_permalink($post_id)))) . '</p>',
        'logged_in_as'         => '<p class="logged-in-as">' . sprintf(__('Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'ad_mag_lite'), get_edit_user_link(), $user_identity, wp_logout_url(apply_filters('the_permalink', get_permalink($post_id)))) . '</p>',
        'comment_notes_before' => __('Your email address will not be published. Required fields are marked *', 'ad_mag_lite'),
        'comment_notes_after'  => '',
        'id_form'              => 'comments-form',
        'id_submit'            => 'submit-contact',
        'title_reply'          => __('POST YOUR COMMENTS', 'ad_mag_lite'),
        'title_reply_to'       => __('POST YOUR COMMENTS TO %s', 'ad_mag_lite'),
        'cancel_reply_link'    => __('(Cancel)', 'ad_mag_lite'),
        'label_submit'         => __('Post Comment', 'ad_mag_lite'),
        'format'               => 'xhtml',
    );
        ...
        <div class="widget kopa-comment-form-widget">            
            ...
            <div class="pd-20"> 
                <?php if (get_option('comment_registration') && !is_user_logged_in()) : ?>
                    <?php echo '<p class="must-log-in">' . sprintf(__('You must be <a href="%s">logged in</a> to post a comment.', 'ad_mag_lite'), wp_login_url(apply_filters('the_permalink', get_permalink($post_id)))) . '</p>'; ?>
                    <?php
