...
?>
	...
	<?php if ( post_password_required() ) : ?>
		<div class="nopassword"><?php _e( 'This post is password protected. Enter the password to view any comments.', 'asokay' ); ?></div>
	</div><!-- .comments -->
	<?php return;
		endif;
	?>
	...
	<?php if ( have_comments() ) : ?>
		<h2 id="comments-title">
			<?php
			    printf( _n( 'One Response to %2$s', '%1$s Responses to %2$s', get_comments_number(), 'asokay' ),
		...
		<nav id="comment-nav-above">
			<h1 class="section-heading"><?php _e( 'Comment navigation', 'asokay' ); ?></h1>
			<div class="nav-previous"><?php previous_comments_link( __( '&larr; Older Comments', 'asokay' ) ); ?></div>
			<div class="nav-next"><?php next_comments_link( __( 'Newer Comments &rarr;', 'asokay' ) ); ?></div>
		</nav>
	...
	<?php else : // this is displayed if there are no comments so far ?>
		...
		<?php else : // or, if we don't have comments:
			...
			?>
			<p class="nocomments"><?php _e( 'Comments are closed.', 'asokay' ); ?></p>
