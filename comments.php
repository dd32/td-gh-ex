...
<?php 
		...
		$aria_req = ( $req ? " aria-required='true'" : '' );
       	$fields =  array(
            'author' => '<div class="comment-welcomeback">'.sprintf(__('Welcome <strong>%s</strong>', 'bb10'), $comment_author).'<a href="javascript:olo_toggleCommentAuthorInfo();" id="toggle-comment-author-info">'.__('(Toggle)', 'bb10').'</a></div><div id="comment-author-info" class="row"><p class="comment-form-author col-sm-4"><input class="form-control" id="author" name="author" type="text" value="'.esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /><label for="author">' . __( 'Name', 'bb10' ) . '</label> ' . ( $req ? '<span class="required">' . __( '(required)', 'bb10' ) . '</span>' : '' ).'</p>',
            'email'  => '<p class="comment-form-email col-sm-4"><input class="form-control" id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /><label for="email">' . __( 'Email', 'bb10' ) . '</label>'. ( $req ? '<span class="required">' . __( '(required)', 'bb10' ) . '</span>' : '' ).'</p>',
            'url'    => '<p class="comment-form-url col-sm-4">'.'<input class="form-control" id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" />'.'<label for="url">' . __( 'Website', 'bb10') . '</label></p></div>',
	);
...
<div id="pings">
	<h3 id="pings-title"><span><?php echo hjyl_get_svg( array( 'icon' => 'chain' ) ); ?> <a><?php _e('Pingbacks', 'bb10'); ?></a></span></h3>
