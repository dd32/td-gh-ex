<?php
/**
 * auto car - About page class
 *
 * @package auto-car
 * @subpackage Admin
 * @since 1.0.0
 */
if ( ! class_exists( 'auto_car_About_Page' ) ) {

	// Include utils functions
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class auto_car_About_Page {
		/**
		 * Define the version of the class.
		 *
		 * @var string $version The auto_car_About_Page class version.
		 */
		...
		/**
		 * The single instance of auto_car_About_Page
		 *
		 * @var auto_car_About_Page $instance The  auto_car_About_Page instance.
		 */
		private static $instance;

		/**
		 * The Main auto_car_About_Page instance.
		 *
		 * We make sure that only one instance of auto_car_About_Page exists in the memory at one time.
		 *
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof auto_car_About_Page ) ) {
				self::$instance = new auto_car_About_Page;
		...
		public function setup_actions() {
			...
			add_action( 'wp_ajax_auto_car_about_page_dismiss_required_action', array( $this, 'dismiss_required_action_callback' ) );
			add_action( 'wp_ajax_nopriv_auto_car_about_page_dismiss_required_action', array( $this, 'dismiss_required_action_callback' ) );
		}
		...
		public function activation_admin_notice() {
			...
			if ( is_admin() && ( 'themes.php' == $pagenow ) && isset( $_GET['activated'] ) ) {
				add_action( 'admin_notices', array( $this, 'auto_car_about_page_welcome_admin_notice' ), 99 );
			}
		}
		 ...
		 */
		public function auto_car_about_page_welcome_admin_notice() {
		 ...
		 */
		public function auto_car_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap auto-car-about-wrap epsilon-wrap">';
				...
				if ( ! empty( $welcome_title ) ) {
					...
					echo '<img src="'.esc_url('https://codethemes.co/wp-content/uploads/2016/02/codethemes-logo.svg').'"></h1>';
				}
...
//
		...
		public function call_plugin_api( $slug ) {
			...
			$call_api = get_transient( 'auto_car_about_plugin_info_' . $slug );
			...
			if ( false === $call_api ) {
				...
				set_transient( 'auto_car_about_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
			}
		...
		public function get_plugin_icon( $arr ) {
			...
			if ( ! empty( $arr['svg'] ) ) {
				...
				$plugin_icon_url = get_template_directory_uri() . '/information/auto-car-notifications/auto-car-about-page/images/placeholder_plugin.png';
			}
			...
			if ( ! empty( $recommended_actions ) ) {
				...
				if ( ! empty( $actions ) && is_array( $actions ) ) {
					...
					foreach ( $actions as $action_key => $action_value ) {
						...
						echo '<div class="auto-car-about-page-action-required-box">';
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							if($network=='local' && $class=='install-now button') {
                                ?>
                                <p><?php echo esc_html__('If plugin installation failed then click on the&nbsp;','auto-car'); ?><a
                                            href="<?php echo esc_url(admin_url('themes.php?page=tgmpa-install-plugins')); ?>"
                                            target="_blank"><?php echo esc_html__('link.', 'auto-car'); ?></a>
                                </p>
                                <?php
                            }
                                ?>
		...
		public function child_themes() {
			echo '<div id="child-themes" class="auto-car-about-page-tab-pane">';
			...
			if ( ! empty( $child_themes ) ) {
				if ( ! empty( $child_themes['content'] ) && is_array( $child_themes['content'] ) ) {
					echo '<div class="auto-car-about-row">';
					for ( $i = 0; $i < count( $child_themes['content'] ); $i ++ ) {
						...
						if ( ! empty( $child['image'] ) ) {
							echo '<div class="auto-car-about-child-theme">';
							echo '<div class="auto-car-about-page-child-theme-image">';
							echo '<img src="' . esc_url( $child['image'] ) . '" alt="' . ( ! empty( $child['image_alt'] ) ? esc_html( $child['image_alt'] ) : '' ) . '" />';
							if ( ! empty( $child['title'] ) ) {
								echo '<div class="auto-car-about-page-child-theme-details">';
							...
							echo '</div><!--auto-car-about-page-child-theme-image-->';
							echo '</div><!--auto-car-about-child-theme-->';
						}// End if().
					}// End for().
					echo '</div>';
				}// End if().
        ...
        public function demo_import() {
            ...
            echo '<div class="feature-section auto-car-demo-import"><img src="'.esc_url(get_template_directory_uri().'/screenshot.png').'" width="300px"><div class="demo-import-footer"><h4>';
            printf(
                wp_kses(
                /* translators: 1: link to WP admin new post page. */
                    __( 'Need To Import Demo&nbsp;&nbsp;&nbsp;<a href="%1$s" class="button button-primary">Click Here</a>', 'auto-car' ),
                    array(
                        'a' => array(
                            'href' => array(),
                            'class' => array(),
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<div id="free_pro" class="auto-car-about-page-tab-pane auto-car-about-page-fre-pro">';
					echo '<table class="free-pro-table">';
					echo '<thead>';
					echo '<tr class="auto-car-about-page-text-right">';
					echo '<th></th>';
					...
					if ( ! empty( $free_pro['pro_theme_link'] ) && ! empty( $free_pro['get_pro_theme_label'] ) ) {
						...
						echo '</td>';
						echo '<td colspan="2" class="auto-car-about-page-text-right"><a href="' . esc_url( $free_pro['pro_theme_link'] ) . '" target="_blank" class="button button-primary button-hero">' . wp_kses_post( $free_pro['get_pro_theme_label'] ) . '</a></td>';
						echo '</tr>';
					}
					echo '</tbody>';
					echo '</table>';
