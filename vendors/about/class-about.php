...
if ( ! class_exists( 'Best_Commerce_About' ) ) {
	...
	class Best_Commerce_About {
		...
		/**
		 * Singleton instance of Best_Commerce_About.
		 *
		 * @var Best_Commerce_About $instance Best_Commerce_About instance.
		 */
		private static $instance;

		/**
		 * Main Best_Commerce_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Best_Commerce_About ) ) {
				self::$instance = new Best_Commerce_About;
		...
		public function setup_config() {
			...
			$dismiss_url = wp_nonce_url( add_query_arg( array( 'best-commerce-at-dismiss' => '1' ), admin_url( 'themes.php' ) ), 'best-commerce-action-dismiss-' . get_current_user_id(), 'best-commerce-dismiss-nonce' );
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'best-commerce' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'best-commerce' ), $this->theme_name ) . '</a>' ) . '<a href="' . esc_url( $dismiss_url ) . '" class="btn-at-dismiss">' . esc_html__( 'Dismiss', 'best-commerce' ) . '</a></p>';
		}
		...
		function hide_notice() {
			// Verify nonce.
			if (! ( isset( $_GET['best-commerce-dismiss-nonce'] ) && wp_verify_nonce( sanitize_key( $_GET['best-commerce-dismiss-nonce'] ), 'best-commerce-action-dismiss-' . get_current_user_id() ) ) ) {
				return;
			}

			if ( 1 === absint( $_GET['best-commerce-at-dismiss'] ) ) {
				update_user_meta( get_current_user_id(), 'best_commerce_at_dismissed', 1 );
			}
		}
		...
		public function activation_admin_notice() {
			...
			$dismissed_status = get_user_meta( get_current_user_id(), 'best_commerce_at_dismissed', true );
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_localize_script( 'best-commerce-about', 'bestCommerceAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'best-commerce' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'best-commerce' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'best-commerce' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'best-commerce' );
		...
		public function useful_plugins() {
			...
			if ( isset( $tgmpa ) ) {
				...
				if ( true === $complete ) {
					echo '<div class="all-active"><span class="dashicons dashicons-yes"></span>&nbsp;' . esc_html__( 'Great, all recommended plugins are intalled and active.', 'best-commerce' ) . '</div>';
				}
				...
				if ( true !== $complete ) {
					echo '<div class="button-wrap"><a href="' . esc_url( $url ) . '" class="button button-primary">' . esc_html__( 'Manage Plugins', 'best-commerce' ) . '</a></div>';
				}
