...
if ( ! class_exists( 'Best_Business_About' ) ) {
	...
	class Best_Business_About {
		...
		/**
		 * Singleton instance of Best_Business_About.
		 *
		 * @var Best_Business_About $instance Best_Business_About instance.
		 */
		private static $instance;

		/**
		 * Main Best_Business_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Best_Business_About ) ) {
				self::$instance = new Best_Business_About;
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_localize_script( 'best-business-about', 'bestBusinessAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'best-business' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'best-business' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'best-business' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'best-business' );
		...
		public function useful_plugins() {
			...
			if ( isset( $tgmpa ) ) {
				...
				if ( true === $complete ) {
					echo '<div class="all-active"><span class="dashicons dashicons-yes"></span>&nbsp;' . esc_html__( 'Great, all recommended plugins are intalled and active.', 'best-business' ) . '</div>';
				}
				...
				if ( true !== $complete ) {
					echo '<div class="button-wrap"><a href="' . esc_url( $url ) . '" class="button button-primary">' . esc_html__( 'Manage Plugins', 'best-business' ) . '</a></div>';
				}
