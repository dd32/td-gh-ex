 ...
 * Add breadcrumb.
 *
 * @action gridalicious_after_header
 *
 * @since Gridalicious 0.1
 */
if( !function_exists( 'gridalicious_add_breadcrumb' ) ) :

	function gridalicious_add_breadcrumb() {
...
add_action( 'gridalicious_after_header', 'gridalicious_add_breadcrumb', 50 );
 ...
 * @since Gridalicious 0.1
 */
if( !function_exists( 'gridalicious_custom_breadcrumbs' ) ) :

	function gridalicious_custom_breadcrumbs( $showOnHome, $delimiter ) {

		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'gridalicious' ); // text for the 'Home' link
		$text['category'] = __( '%1$s Archive for %2$s', 'gridalicious' ); // text for a category page
		$text['search']   = __( '%1$sSearch results for: %2$s', 'gridalicious' ); // text for a search results page
		$text['tag']      = __( '%1$sPosts tagged %2$s', 'gridalicious' ); // text for a tag page
		$text['author']   = __( '%1$sView all posts by %2$s', 'gridalicious' ); // text for an author page
		$text['404']      = __( 'Error 404', 'gridalicious' ); // text for the 404 page
		...
		if( is_front_page() ) {
			...
			if( get_query_var( 'paged' ) ) {
				...
				echo sprintf( __( 'Page %s', 'gridalicious' ), max( $paged, $page ) );
				if( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
