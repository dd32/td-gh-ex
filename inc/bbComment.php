...
function hjyl_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		...
		<div class="comment-author vcard">
			<?php
			$default= ''; echo get_avatar( $comment, 32, $default, $comment->comment_author );
			?>
			<div class="comment_meta">
				<h4><?php printf( '<cite class="fn"> %s </cite>', get_comment_author_link() ); ?></h4>
				<a class="comment_time" href="#comment-<?php comment_ID() ?>" title="<?php printf( '%s', comment_date('Y/m/d '),  comment_time()); ?>"><?php printf(__('%s','bb10'), time_ago()); ?></a>
		...
		<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
			<div id="comment-<?php comment_ID(); ?>"  class="comment">
				...
				<div class="comment-meta">
					<h4><?php printf( __( '%s', 'bb10'), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?></h4>
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>" title="<?php printf( __( '%1$s at %2$s', 'bb10' ), get_comment_date(),  get_comment_time() ); ?>"><?php printf(__('%s','bb10'), time_ago()); ?></a>
				</div>
				</div>
				<div class="comment-body"><?php comment_text(); ?></div>
			</div>
