...
add_action( 'customize_register', 'best_wp_page_sections_pro' );
function best_wp_page_sections_pro( $manager ) {
	...
	$manager->add_section(
		new customize_Customize_Section_Pro(
			...
			array(
				'title'    => esc_html__( 'PRO Feature', 'best-wp' ),
				'priority' => 1, 
				'pro_text' => esc_html__( 'View PRO Feature', 'best-wp' ),
				'pro_url'  => 'https://seosthemes.com/best-wordpress-theme/'
			)
		)
	);
}

function best_wp_customizer_scripts() {
...
add_action( 'customize_controls_enqueue_scripts', 'best_wp_customizer_scripts' );