...
if( is_singular('post') ) {
		...
		$title = __("Category", "avien-light") . " : " . single_cat_title("", false);
	...
	if (is_day()) {
        $title = __("Daily Archives", "avien-light") . " : " . get_the_date();
		$sub_title = themeofwp_option(''.$shortname.'_blog_subtitle');

    } elseif (is_month()) {
        $title = __("Monthly Archives", "avien-light") . " : " . get_the_date("F Y");
		$sub_title = themeofwp_option(''.$shortname.'_blog_subtitle');

    } elseif (is_year()) {
        $title = __("Yearly Archives", "avien-light") . " : " . get_the_date("Y");
		$sub_title = themeofwp_option(''.$shortname.'_blog_subtitle');
        ...
        $title = __("Blog Archives", "avien-light");
		$sub_title = themeofwp_option(''.$shortname.'_blog_subtitle');
...
elseif ( class_exists('bbPress') && is_singular( 'reply' ) || is_singular( 'reply' ) || is_singular( 'topic' ) || is_singular( 'forum' )    ) {
    ...
    $title = __("Author: ", "avien-light");
	
} elseif ( is_search() ) {
    $title = __("Search results for", "avien-light") . " : " . get_search_query();
	
} elseif ( is_tax( 'portfolios' ) ) {
    $title = __("Portfolio", "avien-light");
