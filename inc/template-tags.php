...
function app_landing_page_posted_on() {
	...
	if ( $tags_list ) {
		printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'app-landing-page' ) . '</span>', $tags_list ); // WPCS: XSS OK.
	}
}
endif;
...
function app_landing_page_entry_footer() {
	...
	/* if ( 'post' === get_post_type() ) { */
		...
		if ( $categories_list && app_landing_page_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'app-landing-page' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
*/
		/* translators: used between list items, there is a space after the comma */
/*		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'app-landing-page' ) );
