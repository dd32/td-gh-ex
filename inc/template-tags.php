...
function best_education_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && best_education_categorized_blog() ) {
			printf( '<span class="item-metadata post-category-label">' .'<span class="item-tablecell">'. esc_html__( 'Categorized in :', 'best-education' ).'</span> <span="post-category-label">' .' %1$s'. '</span></span>', $categories_list ); // WPCS: XSS OK.
		}
	}
 ...
 */
	function best_education_entry_tags() {
		...
		if ( $tags_list ) {
			printf( '<span class="tags-links item-metadata post-category-label">' .'<span class="item-tablecell">'. esc_html__( 'Tagged in :', 'best-education' ).'</span> <span="post-category-label">' .' %1$s'. '</span></span>', $tags_list ); // WPCS: XSS OK.
		}
	}
endif;
