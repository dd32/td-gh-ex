...
if ( ! function_exists( 'ace_corporate_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function ace_corporate_posted_on() {
...
if (  'post' == get_post_type() ) {
		...
		if ( 'post' == get_post_type() ) {
			...
			$categories_list = get_the_category_list( __( ', ','ace-corporate' ) );
			if ( $categories_list && ace_corporate_categorized_blog() ) {
				printf( '<span class="meta-sep"> / </span><span class="cat-links"> In ' .esc_html__( ' %1$s','ace-corporate' ) . '</span>', wp_kses_post($categories_list) );
			}
		}
	}
endif;

if ( ! function_exists( 'ace_corporate_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function ace_corporate_entry_footer() {
		// Hide category and tag text for pages.
		?><div class="footer-meta-wrap clearfix"><?php
		if ( 'post' == get_post_type() ) {

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', __( ' ','ace-corporate' ) );
			if ( $tags_list ) {
				if ( is_singular() ) {
					printf( '<span class="tags-links tagcloud">' . esc_html__( 'Posted in %1$s','ace-corporate' ) . '</span>', wp_kses_post($tags_list) );
				}
			}
		}
		...
		if ( ! is_single() && ! is_page() &&  ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link( __( 'Leave a comment','ace-corporate' ), __( '<i class="fa fa-comments-o"></i> 1 Comment','ace-corporate' ), __( '<i class="fa fa-comments-o"></i> % Comments','ace-corporate' ) );
			echo '</span>';
		}

		edit_post_link( __( 'Edit','ace-corporate' ), '<span class="edit-link btn"><i class="fa fa-pencil"></i> ', '</span>' );
		?></div>
		...
		if ( is_single() ){ ?>
			<div class="post-author">
				...
				<div class="author-desc">
					<h5><?php esc_html_e('Article By','ace-corporate'); ?> <a href="<?php echo esc_url(get_author_posts_url(get_the_author_meta( 'ID' ))); ?>"><?php the_author_meta('display_name'); ?></a></h5>
					<p><?php the_author_meta('description'); ?></p>

					<div class="author-links">
						<a class="author-link-posts upper" title="<?php esc_html_e('Author archive','ace-corporate'); ?>" href="<?php echo esc_url( get_author_posts_url(get_the_author_meta( 'ID' )) ); ?>"><i class="fa fa-archive"></i> <?php esc_html_e('Author archive','ace-corporate'); ?></a>
								...
								if (!empty($author_url)) : ?>
									...
									<a class="upper author-link-website" title="<?php esc_html_e('Author website','ace-corporate'); ?>" href="<?php echo esc_url( get_the_author_meta('user_url') ); ?>"><i class="fa fa-globe"></i> <?php esc_html_e('Author website','ace-corporate'); ?></a>
...
function ace_corporate_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'ace_corporate_categories' ) ) ) {
		...
		set_transient( 'ace_corporate_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so ace_corporate_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so ace_corporate_categorized_blog should return false.
		return false;
	}
}
}
...
if( !function_exists( 'ace_corporate_post_thumbnail' ) ) :
	/***
	...
	*
	*/
	function ace_corporate_post_thumbnail() {
...
if ( !function_exists( 'ace_corporate_post_content' ) ) :
	/*
	...
	*/
	...
	function ace_corporate_post_content() {
		if ( !get_the_content() ) {
			return;
		}
