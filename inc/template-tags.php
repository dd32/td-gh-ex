...
if ( ! function_exists( 'acoustics_posted_on' ) ) :
  ...
  $time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
		...
		$posted_on = sprintf(
			...
			esc_html_x( '%s', 'post date', 'acoustics' ),
...
$byline = sprintf(
			...
			esc_html_x( '%s', 'post author', 'acoustics' ),
...
// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'acoustics' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links"><i class="fa fa-folder"></i>' . esc_html__( '%1$s', 'acoustics' ) . '</span>', $categories_list ); // WPCS: XSS OK.
}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'acoustics' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
printf( '<span class="tags-links"><i class="fa fa-tags"></i>' . esc_html__( '%1$s', 'acoustics' ) . '</span>', $tags_list ); // WPCS: XSS OK.
}
		}
