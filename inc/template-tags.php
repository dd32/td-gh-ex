 ...
 * @return bool
 */
function bfront_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'bfront_categories' ) ) ) {
		...
		set_transient( 'bfront_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so bfront_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so bfront_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in bfront_categorized_blog.
 */
function bfront_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'bfront_categories' );
}
add_action( 'edit_category', 'bfront_category_transient_flusher' );
add_action( 'save_post',     'bfront_category_transient_flusher' );
