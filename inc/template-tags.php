	...
	function actinia_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && actinia_categorized_blog() ) {
				printf(
					'<span class="cat-links">' . esc_html( '%1$s %2$s' ) . '</span>',
					$categories_icon,
					$categories_list
				); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				printf(
					'<span class="tags-links">' . esc_html( '%1$s %2$s' ) . '</span>',
					$tag_icon,
					$tags_list
				); // WPCS: XSS OK.
			}
		...
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link(
				sprintf(
					wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'actinia' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() 
				),
				get_comments_number(),
				get_comments_number() 
			);
