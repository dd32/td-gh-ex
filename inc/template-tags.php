...
function skylark_content_nav( $nav_id ) {
	...
	<nav role="navigation" id="<?php echo $nav_id; ?>" class="<?php echo $nav_class; ?>">
		<h1 class="assistive-text"><?php _e( 'Post navigation', 'skylark' ); ?></h1>

	<?php if ( is_single() ) : // navigation links for single posts ?>

		<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'skylark' ) . '</span> %title' ); ?>
		<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'skylark' ) . '</span>' ); ?>
...
endif; // skylark_content_nav

if ( ! function_exists( 'skylark_comment' ) ) :
/**
...
function skylark_comment( $comment, $args, $depth ) {
	...
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'skylark' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( '(Edit)', 'skylark' ), ' ' ); ?></p>
	<?php
			break;
		default :
	?>
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			<footer>
				<div class="comment-author vcard">
					...
					<?php printf( __( '%s <span class="says">says:</span>', 'skylark' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
				</div><!-- .comment-author .vcard -->
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em><?php _e( 'Your comment is awaiting moderation.', 'skylark' ); ?></em>
					<br />
				<?php endif; ?>
				...
				<div class="comment-meta commentmetadata">
					...
					<?php
						...
						printf( __( '%1$s at %2$s', 'skylark' ), get_comment_date(), get_comment_time() ); ?>
					</time></a>
					<?php edit_comment_link( __( '(Edit)', 'skylark' ), ' ' );
					?>
...
endif; // ends check for skylark_comment()

if ( ! function_exists( 'skylark_posted_on' ) ) :
/**
...
function skylark_posted_on() {
	printf( __( 'Posted on <a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s" pubdate>%4$s</time></a><span class="byline"> by <span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>', 'skylark' ),
		...
		esc_attr( sprintf( __( 'View all posts by %s', 'skylark' ), get_the_author() ) ),
...
function skylark_categorized_blog() {
	...
	if ( '1' != $all_the_cool_cats ) {
		// This blog has more than 1 category so skylark_categorized_blog should return true
		return true;
	} else {
		// This blog has only 1 category so skylark_categorized_blog should return false
		return false;
	}
}
