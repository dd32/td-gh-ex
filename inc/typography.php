...
if ( ! function_exists( 'bekko_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'bekko_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function bekko_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', bekko_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $bekko_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $bekko_settings[$key], ':' ) !== false ) {
					$bekko_settings[$key] = current( explode( ':', $bekko_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $bekko_settings[$key] );

				// Grab the variants using the plain name
				$variants = bekko_get_google_font_variants( $bekko_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'bekko_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'bekko_fonts_subset','' );
		...
		$default_fonts = '//fonts.googleapis.com/css?family=Merriweather:300,300italic,regular,italic,700,700italic,900,900italic|Open+Sans+Condensed:300,300italic,700|Open+Sans:regular|Josefin+Sans:100,100italic,300,300italic,regular,italic,600,600italic,700,700italic';

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('bekko-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('bekko-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'bekko_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'bekko_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function bekko_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'bekko_fonts_customize_register' ) ) {
			// Bail if Bekko Premium is active
			return;
		}
		...
		$defaults = bekko_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Bekko_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'bekko_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'bekko_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'bekko_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Bekko_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'bekko' ),
					'settings' => array(
						'family' => 'bekko_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'bekko_settings[body_font_weight]',
						'transform' => 'bekko_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_control(
			new Bekko_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'bekko' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'bekko_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'bekko_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'bekko_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Bekko_Range_Slider_Control(
				...
				array(
					'type' => 'bekko-range-slider',
					'description' => __( 'Line height', 'bekko' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'bekko_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'bekko_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Bekko_Range_Slider_Control(
				...
				array(
					'type' => 'bekko-range-slider',
					'description' => __( 'Paragraph margin', 'bekko' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'bekko_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'bekko_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function bekko_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'bekko_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'bekko_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'bekko_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_bekko_get_all_google_fonts_ajax', 'bekko_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function bekko_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'bekko_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'bekko_customize_nonce' ] ), 'bekko_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'bekko_customize_nonce', 'bekko_customize_nonce' );
		...
		$fonts = bekko_get_all_google_fonts();
...
if ( ! function_exists( 'bekko_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: bekko_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function bekko_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, bekko_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'bekko_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'bekko_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: bekko_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function bekko_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'bekko_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function bekko_get_font_family_css( $font, $settings, $default ) {
		$bekko_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'bekko_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $bekko_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'bekko_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . bekko_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'bekko_add_to_font_customizer_list' ) ) {
	add_filter( 'bekko_typography_customize_list', 'bekko_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function bekko_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = bekko_get_all_google_fonts();
		$select_fonts = bekko_get_all_google_fonts( apply_filters( 'bekko_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $bekko_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $bekko_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $bekko_settings[ $setting ], bekko_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $bekko_settings[ $setting ] ) ) ] = array(
				'name' => $bekko_settings[ $setting ],
		...
		if ( function_exists( 'bekko_secondary_nav_get_defaults' ) ) {
			$secondary_nav_settings = wp_parse_args(
				get_option( 'bekko_secondary_nav_settings', array() ),
				bekko_secondary_nav_get_defaults()
			);

			$secondary_nav_id = strtolower( str_replace( ' ', '_', $secondary_nav_settings[ 'font_secondary_navigation' ] ) );

			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], bekko_typography_default_fonts() ) ) {
