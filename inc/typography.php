...
if ( ! function_exists( 'asagi_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'asagi_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function asagi_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', asagi_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $asagi_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $asagi_settings[$key], ':' ) !== false ) {
					$asagi_settings[$key] = current( explode( ':', $asagi_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $asagi_settings[$key] );

				// Grab the variants using the plain name
				$variants = asagi_get_google_font_variants( $asagi_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'asagi_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'asagi_fonts_subset','' );
		...
		$default_fonts = '//fonts.googleapis.com/css?family=Cinzel:700|Libre+Barcode+39+Extended+Text:regular|Playfair+Display:regular';
		...
		// Enqueue our fonts
		if ( $google_fonts ) {
			wp_enqueue_style('asagi-default-fonts', $default_fonts, array(), null, 'all' );
			wp_enqueue_style('asagi-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'asagi_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'asagi_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function asagi_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'asagi_fonts_customize_register' ) ) {
			// Bail if Asagi Premium is active
			return;
		}
		...
		$defaults = asagi_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Asagi_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'asagi_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'asagi_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'asagi_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Asagi_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'asagi' ),
					'settings' => array(
						'family' => 'asagi_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'asagi_settings[body_font_weight]',
						'transform' => 'asagi_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_control(
			new Asagi_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'asagi' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'asagi_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'asagi_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'asagi_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Asagi_Range_Slider_Control(
				...
				array(
					'type' => 'asagi-range-slider',
					'description' => __( 'Line height', 'asagi' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'asagi_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'asagi_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Asagi_Range_Slider_Control(
				...
				array(
					'type' => 'asagi-range-slider',
					'description' => __( 'Paragraph margin', 'asagi' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'asagi_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'asagi_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function asagi_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'asagi_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'asagi_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'asagi_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_asagi_get_all_google_fonts_ajax', 'asagi_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function asagi_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'asagi_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'asagi_customize_nonce' ] ), 'asagi_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'asagi_customize_nonce', 'asagi_customize_nonce' );
		...
		$fonts = asagi_get_all_google_fonts();
...
if ( ! function_exists( 'asagi_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: asagi_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function asagi_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, asagi_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'asagi_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'asagi_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: asagi_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function asagi_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'asagi_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function asagi_get_font_family_css( $font, $settings, $default ) {
		$asagi_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'asagi_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $asagi_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'asagi_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . asagi_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'asagi_add_to_font_customizer_list' ) ) {
	add_filter( 'asagi_typography_customize_list', 'asagi_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function asagi_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = asagi_get_all_google_fonts();
		$select_fonts = asagi_get_all_google_fonts( apply_filters( 'asagi_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $asagi_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $asagi_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $asagi_settings[ $setting ], asagi_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $asagi_settings[ $setting ] ) ) ] = array(
				'name' => $asagi_settings[ $setting ],
		...
		if ( function_exists( 'asagi_secondary_nav_get_defaults' ) ) {
			$secondary_nav_settings = wp_parse_args(
				get_option( 'asagi_secondary_nav_settings', array() ),
				asagi_secondary_nav_get_defaults()
			);

			$secondary_nav_id = strtolower( str_replace( ' ', '_', $secondary_nav_settings[ 'font_secondary_navigation' ] ) );

			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], asagi_typography_default_fonts() ) ) {
