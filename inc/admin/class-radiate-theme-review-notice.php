...
class Radiate_Theme_Review_Notice {
	...
	public function __construct() {
		...
		add_action( 'after_setup_theme', array( $this, 'radiate_theme_rating_notice' ) );
		add_action( 'switch_theme', array( $this, 'radiate_theme_rating_notice_data_remove' ) );
		add_action( 'admin_enqueue_scripts', array( $this, 'radiate_theme_review_notice_enqueue' ) );
	 ...
	 */
	public function radiate_theme_rating_notice() {
		...
		add_action( 'admin_notices', array( $this, 'radiate_theme_review_notice' ), 0 );
		add_action( 'admin_init', array( $this, 'radiate_ignore_theme_review_notice' ), 0 );
		add_action( 'admin_init', array( $this, 'radiate_ignore_theme_review_notice_partially' ), 0 );
	 ...
	 */
	public function radiate_theme_review_notice() {
		...
		<div class="notice updated theme-review-notice" style="position:relative;">
			<p>
				...
				/* Translators: %1$s current user display name. */
					esc_html__(
						'Howdy, %1$s! It seems that you have been using this theme for more than 15 days. We hope you are happy with everything that the theme has to offer. If you can spare a minute, please help us by leaving a 5-star review on WordPress.org.  By spreading the love, we can continue to develop new amazing features in the future, for free!', 'radiate'
					),
			...
			<div class="links">
				...
				<a href="?nag_radiate_ignore_theme_review_notice_partially=0" class="btn button-secondary">
					<span class="dashicons dashicons-calendar"></span>
					<span><?php esc_html_e( 'Maybe later', 'radiate' ); ?></span>
				</a>

				<a href="?nag_radiate_ignore_theme_review_notice=0" class="btn button-secondary">
			...
			<a class="notice-dismiss" style="text-decoration:none;" href="?nag_radiate_ignore_theme_review_notice=0"></a>
		</div>
	 ...
	 */
	public function radiate_ignore_theme_review_notice()
	{

		$user_id = get_current_user_id();

		/* If user clicks to ignore the notice, add that to their user meta */
		if (isset($_GET['nag_radiate_ignore_theme_review_notice']) && '0' === $_GET['nag_radiate_ignore_theme_review_notice']
		) {
			add_user_meta($user_id, 'radiate_ignore_theme_review_notice', 'true', true);
		}
	 ...
	 */
	public function radiate_ignore_theme_review_notice_partially() {

		$user_id = get_current_user_id();

		/* If user clicks to ignore the notice, add that to their user meta */
		if ( isset( $_GET['nag_radiate_ignore_theme_review_notice_partially'] ) && '0' === $_GET['nag_radiate_ignore_theme_review_notice_partially'] ) {
			update_user_meta( $user_id, 'nag_radiate_ignore_theme_review_notice_partially', time() );
		}
	 ...
	 */
	public function radiate_theme_rating_notice_data_remove() {
	 ...
	 */
	public function radiate_theme_review_notice_enqueue() {

		wp_enqueue_style( 'radiate-theme-review-notice', get_template_directory_uri() . '/inc/admin/css/theme-review-notice.css' );
