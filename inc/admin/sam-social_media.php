                        ...
                        if ( $sm_links ) :
                            ...
                            ?>
                                <li class="row<?php echo $cnt == count( $sm_links ) ? ' sam-no-border' : ''; ?> ">
                                    <label for="use-<?php echo esc_attr( $sm_key ); ?>" class=""><i class="icon-<?php echo esc_attr( $sm_key ) ; ?>"></i><?php echo esc_url( $sm_val['url'] ); ?></label>
                                    <div class="button-wrapper alignright">
                                        <a data-social-media-slug="<?php echo esc_attr( $sm_key ); ?>" data-social-media-url="<?php echo esc_url( $sm_val['url'] ); ?>" data-social-media-label="<?php echo esc_attr( $sm_val['label'] ); ?>" class="button3 edit-social-media" href="javascript:;"><?php _e( 'EDIT', 'sampression' );?></a>
                                        <a class="button4 delete-social-media" href="javascript:;"><?php _e( 'DELETE', 'sampression' );?></a>
                                    </div>
                                    <input type="hidden" name="social_media_slug[]" value="<?php echo esc_attr( $sm_key ); ?>" />
                                    <input type="hidden" name="social_media_url[]" value="<?php echo esc_url( $sm_val['url'] ); ?>" />
                                    <input type="hidden" name="social_media_label[]" value="<?php echo esc_attr( $sm_val['label'] ); ?>" />
                                </li>
                            <?php
                            endforeach; endif;
                            ?>
