	...
	public function tdi_notice() {
		...
		<div class="error updated tdi-notice" style="position:relative;">
			...
			$msg = sprintf(
				...
				'<p style="max-width: 700px;">' . esc_html__( '%1$s: Please update "%2$s" plugin to latest version to make sure your site is all secure. We released a security patch in the latest version of this plugin. %3$s', 'accelerate' ) . '</p>',
				'<strong>' . esc_html__( 'Action Required', 'accelerate' ) . '</strong>',
			...
			$msg .= sprintf(
				...
				'<p style="display: inline-block;">' . esc_html__( 'Also, if the purpose of importing the demo via "%1$s" plugin is fulfilled, you can simply delete this plugin now.', 'accelerate' ) . '</p>',
				'<strong>' . esc_html__( 'ThemeGrill Demo Importer', 'accelerate' ) . '</strong>'
			);
