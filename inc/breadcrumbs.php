...
if(empty($a1_options['remove-breadcrumbs'])) {
	function a1_custom_breadcrumbs() {
	  ...
	  $a1_home = __('Home','a1'); // text for the 'Home' link
	  ...
	  if (is_home() || is_front_page()) {
	    ...
	    if ( is_category() ) {
	      ...
	      echo $a1_before; _e('Archive by category' , 'a1');  echo single_cat_title('', false) . '"' . $a1_after;
	    } elseif ( is_search() ) {
	      echo $a1_before; _e('Search results for','a1'); echo get_search_query() . '"' . $a1_after;
	    } elseif ( is_day() ) {
		...
		if ($a1_i != count($a1_breadcrumbs)-1) echo ' ' . $a1_delimiter . ' ';
	      ...
	      echo $a1_before; _e('Posts tagged','a1'); echo single_tag_title('', false) . '"' . $a1_after;
	    } elseif ( is_author() ) {
	       global $author;
	      $a1_userdata = get_userdata($author);
	      echo $a1_before; _e('Articles posted by','a1'); echo $a1_userdata->display_name . $a1_after;
	    } elseif ( is_404() ) {
	      echo $a1_before; _e('Error','a1'); echo '404' . $a1_after;
	    }
