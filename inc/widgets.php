...
class akasse_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'akasse_rp', // Base ID
			__('Akasse Recent Posts', 'akasse'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'akasse' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'akasse' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'akasse' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','akasse' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:','akasse' ); ?></label>
...
add_action( 'widgets_init', 'register_akasse_widget' );  
function register_akasse_widget() {  
    register_widget( 'Akasse_Recent_Posts' );  
}  
