( function( api ) {
	...
	api.controlConstructor['asura-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.asura-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the asura_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'asura_get_all_google_fonts_ajax',
								asura_customize_nonce: asura_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.asura-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.asura-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.asura-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.asura-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.asura-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.asura-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.asura-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.asura-font-variant' );
			...
			control.container.on( 'change', '.asura-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.asura-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.asura-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.asura-font-family select' ).selectWoo();
	jQuery( '.asura-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".asura-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.asura-font-variant' ).hide();
		}
	});
