( function( api ) {
	...
	api.controlConstructor['bhaga-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.bhaga-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the bhaga_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'bhaga_get_all_google_fonts_ajax',
								bhaga_customize_nonce: bhaga_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.bhaga-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.bhaga-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.bhaga-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.bhaga-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.bhaga-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.bhaga-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.bhaga-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.bhaga-font-variant' );
			...
			control.container.on( 'change', '.bhaga-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.bhaga-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.bhaga-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.bhaga-font-family select' ).selectWoo();
	jQuery( '.bhaga-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".bhaga-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.bhaga-font-variant' ).hide();
		}
	});
