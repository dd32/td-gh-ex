( function( api ) {
	...
	api.controlConstructor['asagi-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.asagi-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the asagi_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'asagi_get_all_google_fonts_ajax',
								asagi_customize_nonce: asagi_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.asagi-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.asagi-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.asagi-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.asagi-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.asagi-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.asagi-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.asagi-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.asagi-font-variant' );
			...
			control.container.on( 'change', '.asagi-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.asagi-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.asagi-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.asagi-font-family select' ).selectWoo();
	jQuery( '.asagi-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".asagi-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.asagi-font-variant' ).hide();
		}
	});
