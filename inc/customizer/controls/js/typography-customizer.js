( function( api ) {
	...
	api.controlConstructor['bekko-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.bekko-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the bekko_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'bekko_get_all_google_fonts_ajax',
								bekko_customize_nonce: bekko_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.bekko-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.bekko-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.bekko-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.bekko-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.bekko-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.bekko-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.bekko-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.bekko-font-variant' );
			...
			control.container.on( 'change', '.bekko-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.bekko-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.bekko-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.bekko-font-family select' ).selectWoo();
	jQuery( '.bekko-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".bekko-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.bekko-font-variant' ).hide();
		}
	});
