...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Asura_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Asura_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'asura-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'asura-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), ASURA_VERSION, true );
			wp_enqueue_style( 'asura-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), ASURA_VERSION );

			wp_enqueue_script( 'asura-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'asura-typography-selectWoo' ), ASURA_VERSION, true );
			wp_enqueue_style( 'asura-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), ASURA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'asura_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'asura' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'asura' );
			$this->json[ 'google_fonts' ] = apply_filters( 'asura_typography_customize_list', asura_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = asura_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'asura' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'asura' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'asura' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'asura' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="asura-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="asura-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="asura-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="asura-font-transform">
					<label>
