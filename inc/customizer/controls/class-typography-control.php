...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Asagi_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Asagi_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'asagi-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'asagi-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), ASAGI_VERSION, true );
			wp_enqueue_style( 'asagi-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), ASAGI_VERSION );

			wp_enqueue_script( 'asagi-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'asagi-typography-selectWoo' ), ASAGI_VERSION, true );
			wp_enqueue_style( 'asagi-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), ASAGI_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'asagi_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'asagi' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'asagi' );
			$this->json[ 'google_fonts' ] = apply_filters( 'asagi_typography_customize_list', asagi_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = asagi_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'asagi' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'asagi' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'asagi' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'asagi' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="asagi-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="asagi-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="asagi-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="asagi-font-transform">
					<label>
