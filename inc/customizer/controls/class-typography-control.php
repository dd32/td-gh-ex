...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Bekko_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Bekko_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'bekko-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'bekko-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), BEKKO_VERSION, true );
			wp_enqueue_style( 'bekko-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), BEKKO_VERSION );

			wp_enqueue_script( 'bekko-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'bekko-typography-selectWoo' ), BEKKO_VERSION, true );
			wp_enqueue_style( 'bekko-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), BEKKO_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'bekko_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'bekko' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'bekko' );
			$this->json[ 'google_fonts' ] = apply_filters( 'bekko_typography_customize_list', bekko_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = bekko_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'bekko' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'bekko' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'bekko' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'bekko' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="bekko-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="bekko-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="bekko-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="bekko-font-transform">
					<label>
