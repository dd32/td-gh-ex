...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Bhaga_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Bhaga_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'bhaga-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'bhaga-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), BHAGA_VERSION, true );
			wp_enqueue_style( 'bhaga-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), BHAGA_VERSION );

			wp_enqueue_script( 'bhaga-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'bhaga-typography-selectWoo' ), BHAGA_VERSION, true );
			wp_enqueue_style( 'bhaga-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), BHAGA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'bhaga_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'bhaga' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'bhaga' );
			$this->json[ 'google_fonts' ] = apply_filters( 'bhaga_typography_customize_list', bhaga_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = bhaga_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'bhaga' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'bhaga' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'bhaga' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'bhaga' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="bhaga-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="bhaga-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="bhaga-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="bhaga-font-transform">
					<label>
