...
class SiteOrigin_Customizer_Helper {
	...
	function customize_register($wp_customize){
		...
		$wp_customize->add_panel( 'siteorigin_theme_settings', array(
			...
			'title'          => __('Theme Design', 'puro'),
			'description'    => sprintf( __('Theme Specific Customizations For %s.', 'puro'), $theme->get('Name') ),
		) );
	...
	function admin_enqueue( ){
		...
		wp_localize_script('siteorigin-customizer-admin', 'soCustomizeAdmin', array(
			'confirm' => __('Are you sure you want to reset your theme customizations?', 'puro'),
			'button' => __('Reset Customizations', 'puro'),
			'action' => wp_nonce_url(
				add_query_arg( array(
