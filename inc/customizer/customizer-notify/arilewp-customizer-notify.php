...
class ArileWP_Customizer_Notify {
	...
	public function arilewp_customizer_notify_scripts_for_customizer() {
		...
		wp_enqueue_style( 'arilewp-customizer-notify-css', get_template_directory_uri() . '/inc/customizer/customizer-notify/css/arilewp-customizer-notify.css', array());
		...
		wp_enqueue_script( 'arilewp-customizer-notify-js', get_template_directory_uri() . '/inc/customizer/customizer-notify/js/arilewp-customizer-notify.js', array( 'customize-controls' ));
		wp_localize_script(
	...
	public function arilewp_plugin_notification_customize_register( $wp_customize ) {
		...
		require_once get_template_directory() . '/inc/customizer/customizer-notify/arilewp-customizer-notify-section.php';
	...
	public function arilewp_customizer_notify_dismiss_recommended_action_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			if ( get_option( 'arilewp_customizer_notify_show' ) ) {
				...
				$arilewp_customizer_notify_show_recommended_actions_new = array();
				if ( ! empty( $arilewp_customizer_notify_recommended_actions ) ) {
					foreach ( $arilewp_customizer_notify_recommended_actions as $arilewp_lite_customizer_notify_recommended_action ) {
						if ( $arilewp_lite_customizer_notify_recommended_action['id'] == $action_id ) {
							$arilewp_customizer_notify_show_recommended_actions_new[ $arilewp_lite_customizer_notify_recommended_action['id'] ] = false;
						} else {
							$arilewp_customizer_notify_show_recommended_actions_new[ $arilewp_lite_customizer_notify_recommended_action['id'] ] = true;
						}
					}
					update_option( 'arilewp_customizer_notify_show', $arilewp_customizer_notify_show_recommended_actions_new );
				}
			}
		}
		die(); 
