 ...
 * @return mixed
 */
function astore_get_prevdem_img_src( $i = 0 ) {
	// prevent infinite loop
	if ( 10 == $i ) {
		return '';
	}

	$path = get_template_directory() . '/inc/demo-preview-images/img/';
	...
	if ( ! file_exists( $path . $img_name ) ) {
		...
		$i++;
		return astore_get_prevdem_img_src( $i );
	}
	...
	return get_template_directory_uri() . '/inc/demo-preview-images/img/' . $img_name;
}
 ...
 */
function astore_the_post_thumbnail( $input ) {
	if ( empty( $input ) ) {
		$placeholder = astore_get_prevdem_img_src();
		return '<img src="' . esc_url( $placeholder ) . '" alt="" class="attachment-astore-blog size-astore-blog wp-post-image">';
	}
	return $input;
}

add_filter( 'post_thumbnail_html', 'astore_the_post_thumbnail' );
