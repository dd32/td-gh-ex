...
function igthemes_edd_template( $template ) {
	...
	if ( is_singular( 'download' )  ) {
		$new_template = locate_template( array( 'edd/single-download.php' ) );
		if ( '' != $new_template ) {
			return $new_template ;
		}
	}
    elseif (is_post_type_archive( 'download' ) || is_tax( 'download_category' ) || is_tax( 'download_tag' )  ) {
		$new_template = locate_template( array( 'edd/archive-download.php' ) );
		if ( '' != $new_template ) {
			return $new_template ;
		}
	}
...
function igthemes_edd_products_per_page( $query ) {
    if ( is_admin() || ! $query->is_main_query() )
        return;
    if ( is_post_type_archive('download') || is_tax( 'download_category' ) ||  is_tax( 'download_tag' ) || is_singular('download') ) {
