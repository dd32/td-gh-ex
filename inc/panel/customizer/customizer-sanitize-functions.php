 ...
 * @since Catch Kathmandu 3.4
 */
function catchkathmandu_sanitize_checkbox( $input ) {
	if ( "1" == $input ) {
		return "1";
	} 
	else {
 ...
 * @since Catch Kathmandu 3.4
 */
function catchkathmandu_sanitize_custom_css( $input ) {
	if ( $input != '' ) { 
 ...
 */
function catchkathmandu_sanitize_image( $image, $setting ) {
	/*
 ...
 * @since Catch Kathmandu 3.4
 */
function catchkathmandu_sanitize_post_id( $input ) {
 ...
 * @since Catch Kathmandu 3.4
 */
function catchkathmandu_sanitize_category_list( $input ) {
	if ( $input != '' ) { 
		$args = array(
						'type'			=> 'post',
						'child_of'      => 0,
 ...
 *                    the setting default.
 */
function catchkathmandu_sanitize_number_range( $number, $setting ) {
 ...
 */
function catchkathmandu_sanitize_select( $input, $setting ) {
 ...
 * @since Catch Kathmandu 3.4
 */
function catchkathmandu_reset_all_settings( $input ) {
	if ( $input == 1 ) {
 ...
 * @since Catch Kathmandu 3.4
 */
function catchkathmandu_sanitize_important_link() {
	return false;
}
 ...
 * @since Catch Kathmandu 3.4
 */
function catchkathmandu_sanitize_reset_featured_image( $input ) {
 ...
 * @since Catch Kathmandu 3.4
 */
function catchkathmandu_sanitize_reset_layout( $input ) {
	//Reset Header Featured Image Options
	if( $input == 1 ) {
		global $catchkathmandu_options_settings, $catchkathmandu_options_defaults;
 ...
 * @since Catch Kathmandu 3.4
 */
function catchkathmandu_sanitize_reset_moretag( $input ) {
	//Reset Header Featured Image Options
	if( $input == 1 ) {
		global $catchkathmandu_options_settings, $catchkathmandu_options_defaults;
