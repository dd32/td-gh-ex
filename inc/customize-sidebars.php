 ...
 * @since Aladdin 1.0.0
 */
add_action( 'customize_register', 'aladdin_creare_section_sidebars', 11 );
function aladdin_creare_section_sidebars( $wp_customize ) {
	...
	foreach( $defaults['defined_sidebars'] as $sidebar_type_id => $sidebar_type) {
		...
		$wp_customize->add_section( $sidebar_type_id, array(
			...
			'description'    => ($sidebar_type_id != 'default' ? __( 'You can add custom sidebars for the page(s) "', 'aladdin' ) . $sidebar_type['title'] . __( '" by selecting options in this section. Default sidebars won\'t be shown on this page(s).', 'aladdin' ) 
														: __( 'Default sidebars for all pages, post and custom post types.', 'aladdin' )),
			'panel'  => 'sidebars',
		) );	
		...
		if( 'default' == $sidebar_type_id)
			$message = __( 'Register default sidebars for all pages', 'aladdin');
		else
			$message = __( 'Override default sidebars by custom sidebars on ', 'aladdin' ) . $sidebar_type['title'] . __( ' (must be checked). Please, choose sidebars which you want to use on this page.', 'aladdin' );
	...
	/* theme mods names: page_[id] - bool [0:1]; $slug_page_[id]*/
		...
		if( '1' != get_theme_mod( $name, null )  ) {
			...
			$wp_customize->add_section( $name, array(
				'priority'       => $section_priority++,
				'title'          => __('Page [', 'aladdin'). get_the_title( $page_id ). ' ('.$page_id.')]',
				'description'    =>  __( 'Per page sidebars for the page [', 'aladdin' ). get_the_title( $page_id ). ']',
				'panel'  => 'sidebars',
			) );	
		...
		//theme mod names: $defaults['defined_sidebars'][$x]
			...
			$wp_customize->add_control( $name, array(
				'label'      => __( 'Uncheck this mark to remove section and custom sidebars for the page [', 'aladdin' ). get_the_title( $page_id ). ']',
				'section'    => $name,
				'settings'   => $name,
				'type'       => 'checkbox',
				'priority'       => $priority++,
	...
	$wp_customize->add_section( 'sidebars_custom', array(
		'priority'       => 200,
		'title'          => __( 'Custom sidebars', 'aladdin' ),
		'description'    => __( 'In this section you can create a sidebar for a certain page. Select page and press save. Refresh Customizer to see newly added section for that page.', 'aladdin' ),
		'panel'  => 'sidebars',
	) );
	...
	$wp_customize->add_control( 'pages', array(
		'label'      => __( 'Select a page and save data. Refresh Customizer to customize new section.', 'aladdin' ),
 ...
 * @since Aladdin 1.0.0
 */
function aladdin_save_custom_sidebar( $value ) {
	//sanitize
	$value = absint( $value );
	if( 0 == $value )
		return;
...
add_action( 'customize_update_page_sidebar', 'aladdin_save_custom_sidebar' );
	...
	//theme mod names: $defaults['theme_sidebars'][$x]['slug']_$defaults['defined_sidebars'][$x]
		foreach( $defaults['theme_sidebars'] as $slug => $sidebars ) {
			...
			register_sidebar( array(
				'name' => __( '[', 'aladdin') . $page_id.__( '] ', 'aladdin') . $sidebars['title'] . ', ' . get_the_title( $page_id ),
				'id' => $slug . '-' . $name,
 ...
 * @since Aladdin 1.0.0
 */
function aladdin_get_sidebar_slug() {
	global $aladdin_sidebar_slug;
	if( isset($aladdin_sidebar_slug) )
		return $aladdin_sidebar_slug;
	...
	if( is_page() ) {
		...
		if( isset( $page_sidebars[ $page_id ]) ) {
			$aladdin_sidebar_slug = apply_filters( 'aladdin_sidebar_slug', 'page_'.$page_id );
			return $aladdin_sidebar_slug;
		}
	}
	...
	foreach( array_reverse( $defaults['defined_sidebars'] ) as $slug => $defined_sidebars ) {
		...
		if( '' == $defined_sidebars['callback'] ) {
			$aladdin_sidebar_slug = apply_filters( 'aladdin_sidebar_slug', $slug );;
			return $aladdin_sidebar_slug;
		}
	...
	return apply_filters( 'aladdin_sidebar_slug', null );
}
 ...
 * @since Aladdin 1.0.0
 */
function aladdin_get_page_sidebar_slug( $page_id ) {
 ...
 * @since Aladdin 1.0.0
 */
function aladdin_is_portfolio_page() {
