 ...
 *
 * Prevents Admiral from running on WordPress versions prior to 4.4,
 ...
 * Admiral on WordPress versions prior to 4.4.
 *
 * @global string $wp_version WordPress version.
 */
function admiral_upgrade_notice() {
	$message = sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'admiral' ), 'Admiral', '4.4', $GLOBALS['wp_version'] );
...
function admiral_customize() {
	wp_die( sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'admiral' ), 'Admiral', '4.4', $GLOBALS['wp_version'] ), '', array(
		'back_link' => true,
	) );
}
...
function admiral_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'admiral' ), 'Admiral', '4.4', $GLOBALS['wp_version'] ) );
	}
}
