 ...
 */
function catchflames_filter_wp_title( $title, $sep ) {
	global $page, $paged;
	...
	if ( $paged >= 2 || $page >= 2 )
		$title = "$title $sep " . sprintf( __( 'Page %s', 'catchflames' ), max( $paged, $page ) );

	return $title;	

}
add_filter( 'wp_title', 'catchflames_filter_wp_title', 10, 2 );
