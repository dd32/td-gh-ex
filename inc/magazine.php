...
if ( ! function_exists( 'beetle_magazine_widget_title' ) ) :
	 ...
	 */
	function beetle_magazine_widget_title( $widget_title, $category_id ) {

		// Check if widget shows a specific category.
		if ( $category_id > 0 ) {

			// Set URL and Title for Category.
			$category_title = sprintf( esc_html__( 'View all posts from category %s', 'beetle' ), get_cat_name( $category_id ) );
...
if ( ! function_exists( 'beetle_magazine_entry_meta' ) ) :
	/**
	 * Displays the date and author of magazine posts
	 */
	function beetle_magazine_entry_meta() {
...
if ( ! function_exists( 'beetle_magazine_entry_date' ) ) :
	/**
	 * Displays the date of magazine posts
	 */
	function beetle_magazine_entry_date() {
		echo '<div class="entry-meta">' . beetle_meta_date() . '</div>';
	}
endif;
