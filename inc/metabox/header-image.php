...
function academic_header_image_callback( $post ) {
    ...
    <p>
     <label for="academic-header-image" class="academic-row-title"><?php esc_html_e( 'Header Image', 'academic' )?></label>
     <select name="academic-header-image" id="academic-header-image">
      <option value=""><?php esc_html_e( 'Default ( Customizer Header Image )', 'academic' ); ?></option>
...
function academic_sidebar_header_image_save( $post_id ) {
    ...
    if( isset( $_POST[ 'academic-header-image' ] ) ) {
        update_post_meta( $post_id, 'academic-header-image', sanitize_text_field( $_POST[ 'academic-header-image' ] ) );
    }
