...
function automobile_car_dealer_customize_logo_resizer( $html ) {
	$size = get_theme_mod( 'automobile_car_dealer_logo_sizer' );
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = automobile_car_dealer_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = automobile_car_dealer_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'automobile_car_dealer_customize_logo_resizer' );

/* Helper function to determine the max size of the logo */
function automobile_car_dealer_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
...
function automobile_car_dealer_remove_theme_mod() {
	if ( isset( $_GET['remove_logo_size'] ) && 'true' == $_GET['remove_logo_size'] ){
		set_theme_mod( 'automobile_car_dealer_logo_sizer', '' );
	}
}
add_action( 'wp_loaded', 'automobile_car_dealer_remove_theme_mod' );