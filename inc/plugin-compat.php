...
add_action( 'after_setup_theme', 'bekko_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function bekko_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'bekko_construct_sidebars' );
}

if ( ! function_exists( 'bekko_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'bekko_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function bekko_woocommerce_start() { ?>
...
if ( ! function_exists( 'bekko_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'bekko_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function bekko_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'bekko_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'bekko_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function bekko_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'bekko_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'bekko_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'bekko_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function bekko_bbpress_css() {
...
if ( ! function_exists( 'bekko_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'bekko_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function bekko_buddypress_css() {
...
if ( ! function_exists( 'bekko_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'bekko_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function bekko_beaver_builder_css() {
