...
add_action( 'after_setup_theme', 'asura_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function asura_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'asura_construct_sidebars' );
}

if ( ! function_exists( 'asura_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'asura_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function asura_woocommerce_start() { ?>
...
if ( ! function_exists( 'asura_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'asura_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function asura_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'asura_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'asura_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function asura_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'asura_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'asura_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'asura_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function asura_bbpress_css() {
...
if ( ! function_exists( 'asura_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'asura_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function asura_buddypress_css() {
...
if ( ! function_exists( 'asura_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'asura_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function asura_beaver_builder_css() {
