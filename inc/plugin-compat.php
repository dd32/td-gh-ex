...
add_action( 'after_setup_theme', 'asagi_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function asagi_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'asagi_construct_sidebars' );
}

if ( ! function_exists( 'asagi_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'asagi_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function asagi_woocommerce_start() { ?>
...
if ( ! function_exists( 'asagi_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'asagi_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function asagi_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'asagi_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'asagi_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function asagi_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'asagi_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'asagi_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'asagi_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function asagi_bbpress_css() {
...
if ( ! function_exists( 'asagi_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'asagi_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function asagi_buddypress_css() {
...
if ( ! function_exists( 'asagi_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'asagi_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function asagi_beaver_builder_css() {
