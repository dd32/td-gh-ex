...
add_action( 'after_setup_theme', 'bhaga_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function bhaga_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'bhaga_construct_sidebars' );
}

if ( ! function_exists( 'bhaga_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'bhaga_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function bhaga_woocommerce_start() { ?>
...
if ( ! function_exists( 'bhaga_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'bhaga_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function bhaga_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'bhaga_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'bhaga_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function bhaga_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'bhaga_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'bhaga_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'bhaga_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function bhaga_bbpress_css() {
...
if ( ! function_exists( 'bhaga_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'bhaga_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function bhaga_buddypress_css() {
...
if ( ! function_exists( 'bhaga_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'bhaga_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function bhaga_beaver_builder_css() {
