 ...
 */
function advance_automobile_logo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'logo_size', array(
		'label'       => esc_html__( 'Logo Size','advance-automobile' ),
	...
	$wp_customize->add_setting('advance_automobile_site_title',array(
       'default' => true,
       'sanitize_callback'	=> 'advance_automobile_sanitize_checkbox'
    ));
    $wp_customize->add_control('advance_automobile_site_title',array(
       'type' => 'checkbox',
       'label' => __('Site Title','advance-automobile'),
       'section' => 'title_tagline'
    ));

    $wp_customize->add_setting('advance_automobile_tagline',array(
       'default' => true,
       'sanitize_callback'	=> 'advance_automobile_sanitize_checkbox'
    ));
    $wp_customize->add_control('advance_automobile_tagline',array(
       'type' => 'checkbox',
       'label' => __('Tagline','advance-automobile'),
       'section' => 'title_tagline'
    ));
}
add_action( 'customize_register', 'advance_automobile_logo_customize_register' );

/**
 * Add support for logo resizing by filtering `get_custom_logo`
 */
function advance_automobile_customize_logo_resize( $html ) {
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = advance_automobile_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = advance_automobile_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'advance_automobile_customize_logo_resize' );

/* Helper function to determine the max size of the logo */
function advance_automobile_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
...
function advance_automobile_customize_controls_js() {
	wp_enqueue_script( 'advance-automobile-customizer-controls', esc_url(get_template_directory_uri()) . '/inc/logo/js/customize-controls.js', array( 'jquery', 'customize-preview' ), '201709071000', true );
}
 ...
 */
function advance_automobile_customize_css() {
	wp_add_inline_style( 'customize-controls', '#customize-control-logo_size input[type=range] { width: 100%; }' );
}
add_action( 'customize_controls_enqueue_scripts', 'advance_automobile_customize_css' );

/**
 * Testing function to remove logo_size theme mod
 */
function advance_automobile_remove_theme_mod() {
	if ( isset( $_GET['remove_logo_size'] ) && 'true' == $_GET['remove_logo_size'] ){
		set_theme_mod( 'logo_size', '' );
	}
}
add_action( 'wp_loaded', 'advance_automobile_remove_theme_mod' );