 ...
 */
function advance_pet_care_logo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'logo_size', array(
		'label'       => esc_html__( 'Logo Size','advance-pet-care' ),
	...
	$wp_customize->add_setting('advance_pet_care_site_title',array(
       'default' => true,
       'sanitize_callback'	=> 'advance_pet_care_sanitize_checkbox'
    ));
    $wp_customize->add_control('advance_pet_care_site_title',array(
       'type' => 'checkbox',
       'label' => __('Site Title','advance-pet-care'),
       'section' => 'title_tagline'
    ));

    $wp_customize->add_setting('advance_pet_care_tagline',array(
       'default' => true,
       'sanitize_callback'	=> 'advance_pet_care_sanitize_checkbox'
    ));
    $wp_customize->add_control('advance_pet_care_tagline',array(
       'type' => 'checkbox',
       'label' => __('Tagline','advance-pet-care'),
       'section' => 'title_tagline'
    ));
}
add_action( 'customize_register', 'advance_pet_care_logo_customize_register' );

/**
 * Add support for logo resizing by filtering `get_custom_logo`
 */
function advance_pet_care_customize_logo_resize( $html ) {
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = advance_pet_care_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = advance_pet_care_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'advance_pet_care_customize_logo_resize' );

/* Helper function to determine the max size of the logo */
function advance_pet_care_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
