...
class aladdin_Colors_Class {
	...
	function __construct() {
		...
		$this->color_scheme = aladdin_get_theme_mod( 'color_scheme', $defaults['color_scheme'] );
				
		$this->add_scheme(0, __('Blue', 'aladdin'));
		$this->add_scheme(1, __('Green', 'aladdin'));
		
		$section_id = 'main_colors';
		$section_priority = 10;
		$p = 10;
		
		$this->add_section($section_id, __('Main Colors', 'aladdin'), __('Main website colors', 'aladdin'), $section_priority++);

	/* colors */
		
		$i = 'link_color';
		
		$this->add_color($i, $section_id, __('Link', 'aladdin'), $p++, false);
		$this->set_color($i, 0, '#1e73be');
		$this->set_color($i, 1, '#1e73be');
		
		$i = 'heading_color';
		
		$this->add_color($i, $section_id, __('H1-H6 heading', 'aladdin'), $p++, false);
		$this->set_color($i, 0, '#3f3f3f');
		$this->set_color($i, 1, '#3f3f3f');
		
		$i = 'heading_link';
		
		$this->add_color($i, $section_id, __('H1-H6 Link', 'aladdin'), $p++, false);
		$this->set_color($i, 0, '#1e73be');	
		$this->set_color($i, 1, '#1e73be');	
		
		$i = 'description_color';
		
		$this->add_color($i, $section_id, __('Description', 'aladdin'), $p++, false);
		$this->set_color($i, 0, '#ffffff');	
		$this->set_color($i, 1, '#ffffff');
		
		$i = 'hover_color';
		
		$this->add_color($i, $section_id, __('Link Hover', 'aladdin'), $p++, false, 'refresh');
		$this->set_color($i, 0, '#a72ec9');
		$this->set_color($i, 1, '#a72ec9');
	
		add_action( 'customize_register', array( $this, 'aladdin_create_colors_controls' ), 21 );
		add_action( 'aladdin_option_defaults', array( $this, 'aladdin_add_defaults' ) );
		add_action( 'customize_controls_print_scripts', array( $this, 'aladdin_print_customizer_js_colors') );

	}
	
	/* Print js for color scheme switching */
	public function aladdin_print_customizer_js_colors() {
		...
		jQuery( document ).ready(function( $ ) {
			...
			wp.customize( 'color_scheme', function( value ) {
				value.bind( function( to ) {
					...
					aladdin_refresh_colors(to);

				});
			});
			
			function aladdin_refresh_colors( color_scheme ) { 
	...
	public function aladdin_create_colors_controls( $wp_customize ) {
		...
		$wp_customize->add_section( 'color_scheme', array(
			...
			'description'    => __( 'This option refresh theme colors.', 'aladdin' ),
			'priority'       => 1,
			'panel'  => 'custom_colors',
		) );
		...
		$wp_customize->add_setting( 'color_scheme', array(
			...
			'sanitize_callback' => 'aladdin_sanitize_color_scheme'
		) );

		$wp_customize->add_control( 'color_scheme', array(
			'label'      => __( 'Color Scheme', 'aladdin' ),
		...
		foreach( $this->colors as $id => $colors ) {
			...
			$wp_customize->add_setting( $id, array(
				...
				'capability'     => 'edit_theme_options',
				'sanitize_callback' => 'aladdin_sanitize_hex_color'
			) );
			...
			if( $colors['is_has_opacity'] ) {
				$wp_customize->add_setting( $id.'_opacity', array(
					...
					'capability'     => 'edit_theme_options',
					'sanitize_callback' => 'aladdin_sanitize_opacity'
				) );
				
				$wp_customize->add_control( $id.'_opacity', array(
					'label'      => __('Opacity for the ', 'aladdin').$colors['text'],
					'section'    => $colors['section'],
 ...
 * @since Aladdin 1.0.0
 */
function aladdin_sanitize_color_scheme( $value ) {
	global $aladdin_colors_class;
	$defaults = aladdin_get_defaults();
	$possible_values = $aladdin_colors_class->get_schemes();
 ...
 * @since Aladdin 1.0.0
*/
function aladdin_hook_css_colors() {
...
?>
	<style type="text/css">	
		...
		.widget.aladdin_recent_posts .content article footer a,
		...
		a:hover,
		.widget.aladdin_recent_posts .content article footer a:hover,
...
add_action('wp_head', 'aladdin_hook_css_colors');