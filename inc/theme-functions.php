...
if ( ! function_exists( 'bekko_get_setting' ) ) {
	/**
	 * A wrapper function to get our settings.
	 *
	 */
	function bekko_get_setting( $setting ) {
		$bekko_settings = wp_parse_args(
			get_option( 'bekko_settings', array() ),
			bekko_get_defaults()
		);

		return $bekko_settings[ $setting ];
	}
}

if ( ! function_exists( 'bekko_get_layout' ) ) {
	/**
	...
	function bekko_get_layout() {
		...
		$layout = $bekko_settings['layout_setting'];

		// Get the individual page/post sidebar metabox value
		$layout_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_bekko-sidebar-layout-meta', true ) : '';
		...
		if ( is_single() && ! $buddypress ) {
			$layout = null;
			$layout = $bekko_settings['single_layout_setting'];
		}
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = null;
			$layout = $bekko_settings['blog_layout_setting'];
		}

		// Finally, return the layout
		return apply_filters( 'bekko_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'bekko_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function bekko_get_footer_widgets() {
		// Get current post
		global $post;
		...
		$widgets = $bekko_settings['footer_widget_setting'];

		// Get the individual footer widget metabox value
		$widgets_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_bekko-footer-widget-meta', true ) : '';
		...
		// Finally, return the layout
		return apply_filters( 'bekko_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'bekko_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 *
	 */
	function bekko_show_excerpt() {
		// Get current post
		global $post;
		...
		$more_tag = apply_filters( 'bekko_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		// Check the post format
		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		// Get the excerpt setting from the Customizer
		$show_excerpt = ( 'excerpt' == $bekko_settings['post_content'] ) ? true : false;
		...
		// Return our value
		return apply_filters( 'bekko_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'bekko_show_title' ) ) {
	/**
	 ...
	 */
	function bekko_show_title() {
		return apply_filters( 'bekko_show_title', true );
	}
}

if ( ! function_exists( 'bekko_padding_css' ) ) {
	/**
	 ...
	 */
	function bekko_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'bekko_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'bekko_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function bekko_get_navigation_location() {
		return apply_filters( 'bekko_navigation_location', bekko_get_setting( 'nav_position_setting' ) );
	}
}
