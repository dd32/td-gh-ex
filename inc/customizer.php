	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			array(
				...
				'description' => esc_html__( 'Background Color of Sticky Posts', 'actinia' ),
			)
		)
	);
	
	$wp_customize->add_setting(
		'actinia_tagline_color',
		array(
			'default'           => '#8a8a8a',
			'capability'        => 'edit_theme_options',
			'transport'         => 'postMessage',
...
function actinia_has_widgets() {
	...
	$widgets_primary = $widgets['sidebar-primary'];

	if ( is_active_sidebar( 'sidebar-primary' ) ) {
		//sidebar position control is displayed only if sidebar contains at least 1 widget, cookie widget excluded
		return count( $widgets_primary ) > 1 || ( count( $widgets_primary ) === 1 && strpos( $widgets_primary[0], 'cookie' ) === false );
	}
...
function actinia_elements() {
	...
	$rules = array(
		...
		array(
			'name'  => 'article:not(.sticky) .entry-header, article:not(.sticky) .wrapper',
			'props' => array(
		 ...
		 array(
			'name'  => 'article.sticky',
			'props' => array(
				'background' => $colors['actinia_color_sticky'],
			),
		),
		array(
			'name'  => 'article.sticky .entry-header, article.sticky .wrapper',
			'props' => array(
		...
		array(
			'name'        => '.search-form input[type=text], .search-form input[type=search]',
			'link_states' => array('focus'),
			'props'       => array(
				'border-bottom' => $colors['actinia_primary_color_b'],
				'box-shadow'    => '0 0.5em 0.5em ' . $colors['actinia_primary_color_b'],
			),
		),
		array(
			'name'  => '.anchor',
			'props' => array(
				'border-color' => $colors['actinia_primary_color_b'],
			),
		),
		array(
			'name'       => '.anchor',
			'after_link' => ':after',
			'props'      => array(
				'color' => $colors['actinia_primary_color_b']
			),
		),
	);
