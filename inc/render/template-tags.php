...
function igthemes_posted_on() {
    //AUTHOR
    $byline = sprintf(
          esc_html_x( apply_filters('igthemes-author','igthemes-author-text') . '%s', 'post author', 'basic-shop' ),
	...
	$posted_on = sprintf(
		esc_html_x( apply_filters('igthemes_date', 'igthemes_date_text') . '%s', 'post date', 'basic-shop' ),
...
function igthemes_entry_footer() {
	...
	if ( 'post' === get_post_type() && apply_filters( 'igthemes_post_cat', true ) ) {
		...
		if ( $categories_list && igthemes_categorized_blog() && apply_filters( 'igthemes_post_cat', true ) ) {
			printf( '<span class="cat-links">' . esc_html__( apply_filters('igthemes_cat_links', 'igthemes-cat-text') . '%1$s', 'basic-shop' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'basic-shop' ));
		if ( $tags_list && apply_filters( 'igthemes_post_tags', true ) ) {
			printf( '<span class="tags-links">' . esc_html__( apply_filters('igthemes_tags_links', 'igthemes-tags-text') . '%1$s', 'basic-shop' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
    //commnet
