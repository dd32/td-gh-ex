...
function aaron_meta_form( $post ) {
	...
	<p>
	<input type="checkbox" name="aaron_featured_image_header" id="aaron_featured_image_header" value="featured-image-header" 
	<?php echo ( aaron_get_meta( 'aaron_featured_image_header' ) === 'featured-image-header' ) ? 'checked' : ''; ?>>
	<label for="aaron_featured_image_header">
		<?php esc_html_e( 'Use the featured image as header image. This requires the header to be active, and a featured image to be set. The image will inherit the background position from the default header.', 'aaron' ); ?>
	</label>	
	</p>
	<p>
	<input type="checkbox" name="aaron_hide_action_meta" id="aaron_hide_action_meta" value="hide_action" 
	<?php echo ( aaron_get_meta( 'aaron_hide_action_meta' ) === 'hide_action' ) ? 'checked' : ''; ?>>
	<label for="aaron_hide_action_meta"><?php esc_html_e( 'Hide the Call to Action button.', 'aaron' ); ?></label>	
	</p>
	<p>
	<input type="checkbox" name="aaron_hide_highlights_meta" id="aaron_hide_highlights_meta" value="hide_highlights" 
	<?php echo ( aaron_get_meta( 'aaron_hide_highlights_meta' ) === 'hide_highlights' ) ? 'checked' : ''; ?>>
	<label for="aaron_hide_highlights_meta"><?php esc_html_e( 'Hide the Highlights.', 'aaron' ); ?></label>	
	</p>
	<p>
	<input type="checkbox" name="aaron_hide_tagline" id="aaron_hide_tagline" value="hide_tagline" 
	<?php echo ( aaron_get_meta( 'aaron_hide_tagline' ) === 'hide_tagline' ) ? 'checked' : ''; ?>>
	<label for="aaron_hide_tagline"><?php esc_html_e( 'Hide the site Tag line (Description).', 'aaron' ); ?></label>	
	</p>
	<p>
	<input type="checkbox" name="aaron_replace_title" id="aaron_replace_title" value="replace_title" 
	<?php echo ( aaron_get_meta( 'aaron_replace_title' ) === 'replace_title' ) ? 'checked' : ''; ?>>
	...
	<br>
	<p>
	<input type="checkbox" name="aaron_hide_image" id="aaron_hide_image" value="hide_image" 
	<?php echo ( aaron_get_meta( 'aaron_hide_image' ) === 'hide_image' ) ? 'checked' : ''; ?>>
...
function aaron_header_settings_save( $post_id ) {
	...
	if ( isset( $_POST['aaron_show_header'] ) ) {
		update_post_meta( $post_id, 'aaron_show_header', sanitize_text_field( wp_unslash( $_POST['aaron_show_header'] ) ) );
	} else {
		update_post_meta( $post_id, 'aaron_show_header', null );
	}
	// Use featured Image.
	if ( isset( $_POST['aaron_featured_image_header'] ) ) {
		update_post_meta( $post_id, 'aaron_featured_image_header', sanitize_text_field( wp_unslash( $_POST['aaron_featured_image_header'] ) ) );
	} else {
		update_post_meta( $post_id, 'aaron_featured_image_header', null );
	}
	// Hide Call to Action.
	if ( isset( $_POST['aaron_hide_action_meta'] ) ) {
		update_post_meta( $post_id, 'aaron_hide_action_meta', sanitize_text_field( wp_unslash( $_POST['aaron_hide_action_meta'] ) ) );
	} else {
	...
	if ( isset( $_POST['aaron_hide_highlights_meta'] ) ) {
		update_post_meta( $post_id, 'aaron_hide_highlights_meta', sanitize_text_field( wp_unslash( $_POST['aaron_hide_highlights_meta'] ) ) );
	} else {
	...
	if ( isset( $_POST['aaron_hide_tagline'] ) ) {
		update_post_meta( $post_id, 'aaron_hide_tagline', sanitize_text_field( wp_unslash( $_POST['aaron_hide_tagline'] ) ) );
	} else {
	...
	if ( isset( $_POST['aaron_replace_title'] ) ) {
		update_post_meta( $post_id, 'aaron_replace_title', sanitize_text_field( wp_unslash( $_POST['aaron_replace_title'] ) ) );
	} else {
	...
	if ( isset( $_POST['aaron_hide_image'] ) ) {
		update_post_meta( $post_id, 'aaron_hide_image', sanitize_text_field( wp_unslash( $_POST['aaron_hide_image'] ) ) );
	} else {
