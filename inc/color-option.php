<?php
	...
	if($advance_automobile_theme_color_second != false){
		$custom_css .='input[type="submit"], .logo a,.social-icons i:hover, #header main-menu-navigation ul li a, .search-box i, #slider .inner_carousel h2 , #slider .inner_carousel,#category h3, .page-box .metabox,.page-box-single .metabox,.metabox a, .woocommerce-message::before, h1.entry-title,h1.page-title,  .page-box h4, .new-text h4 a, .primary-navigation ul ul a{';
	...
	if($advance_automobile_theme_color_second != false){
		$custom_css .='.logo p, .page-box-single h3, #sidebar ul li a:hover, #sidebar ul li a:active, #sidebar ul li a:focus,span.post-title, p.logged-in-as a{';
