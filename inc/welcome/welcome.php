<?php
	if(!class_exists('Agency_lite_Welcome')) :

		class Agency_lite_Welcome {
			...
			public function __construct() {
				...
				$this->tab_sections = array(
					'getting_started' => __('Getting Started', 'agency-lite'),
					'recommended_plugins' => __('Recommended Plugins', 'agency-lite'),
					'support' => __('Support', 'agency-lite'),
					'free_vs_pro' => __('Free Vs Pro', 'agency-lite'),
					'more_wp_stuff' => __('More WordPress Stuff', 'agency-lite'),
				);
				...
				add_action( 'load-themes.php', array( $this, 'agency_lite_activation_admin_notice' ) );

				/* Create a Welcome Page */
				add_action( 'admin_menu', array( $this, 'agency_lite_Welcome_register_menu' ) );

				/* Enqueue Styles & Scripts for Welcome Page */
				add_action( 'admin_enqueue_scripts', array( $this, 'agency_lite_Welcome_styles_and_scripts' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_agency_lite_plugin_installer', array( $this, 'agency_lite_plugin_installer_callback' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_agency_lite_plugin_offline_installer', array( $this, 'agency_lite_plugin_offline_installer_callback' ) );

				/** Plugin Activation Ajax **/
				add_action( 'wp_ajax_agency_lite_plugin_activation', array( $this, 'agency_lite_plugin_activation_callback' ) );

				/** Plugin Activation Ajax (Offline) **/
				add_action( 'wp_ajax_agency_lite_plugin_offline_activation', array( $this, 'agency_lite_plugin_offline_activation_callback' ) );
			}

			/** Welcome Message Notification on Theme Activation **/
			public function agency_lite_activation_admin_notice() {
				global $pagenow;
			...
			public function agency_lite_Welcome_register_menu() {
				$action_count = get_option('agency_lite_plugin_installed_notif');
				$title        = $action_count > 0 ? esc_html__( 'Welcome', 'agency-lite' ) : esc_html__( 'Welcome', 'agency-lite' );
				add_theme_page( 'Welcome', $title , 'edit_theme_options', 'agency-welcome', array( $this, 'agency_lite_Welcome_screen' ));
			}
			...
			public function agency_lite_Welcome_styles_and_scripts() {
				wp_enqueue_style( 'accesspress-basic-welcome-screen', get_template_directory_uri() . '/inc/welcome/css/welcome.css' );
				wp_enqueue_script( 'accesspress-basic-welcome-screen', get_template_directory_uri() . '/inc/welcome/js/welcome.js', array( 'jquery' ) );

				wp_localize_script( 'accesspress-basic-welcome-screen', 'AgencyWelcomeObject', array(
					'admin_nonce'	=> wp_create_nonce('agency_lite_plugin_installer_nonce'),
					'activate_nonce'	=> wp_create_nonce('agency_lite_plugin_activate_nonce'),
					'ajaxurl'		=> esc_url( admin_url( 'admin-ajax.php' ) ),
					'activate_btn' => __('Activate', 'agency-lite'),
					'installed_btn' => __('Activated', 'agency-lite'),
					'demo_installing' => __('Installing Demo', 'agency-lite'),
					'demo_installed' => __('Demo Installed', 'agency-lite'),
					'demo_confirm' => __('Are you sure to import demo content ?', 'agency-lite'),
				) );
			}

			/** Plugin API **/
			public function agency_lite_call_plugin_api( $plugin ) {
			...
			/** Check For Icon **/
			public function agency_lite_check_for_icon( $arr ) {
				if ( ! empty( $arr['svg'] ) ) {
			...
			public function agency_lite_plugin_active($plugin) {
			...
			public function agency_lite_plugin_generate_url($status, $plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
			...
			public function agency_lite_plugin_installer_callback(){
				...
				// Get Plugin Info
				$api = $this->agency_lite_call_plugin_api($plugin);
			...
			public function agency_lite_plugin_offline_installer_callback() {
			...
			public function agency_lite_plugin_offline_activation_callback() {
			...
			public function agency_lite_plugin_activation_callback(){
				...
				// Get Plugin Info
				$api = $this->agency_lite_call_plugin_api(esc_attr($plugin));
				...
				if($api->name){
					...
					$status = 'failed';
					$msg = esc_html__('There was an error activating $api->name', 'agency-lite');
				}
			...
			public static function get_plugin_file( $plugin_slug ) {
	      	...
	      	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=agency-welcome&section=import_demo'),'agency-file-installation');
	...
	if(class_exists('Instant_Demo_Importer')) :
		...
		$demoimporter->demos = array(
			'agency-lite' => array(
				'title' => __('Uncode Demo', 'agency-lite'),
				'name' => 'agency-lite',
