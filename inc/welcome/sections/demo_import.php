	...
	foreach($req_plugins as $plugin) :
		...
		if( $plugin['host_type'] == 'bundled' ) {
			...
			if(!isset($info->errors)) :
				...
				$path_args = WP_PLUGIN_DIR.'/'.esc_attr($plugin['slug']).'/'.esc_attr($plugin['filename']);
				?>

				<?php if( isset( $plugin['class'] ) && !class_exists( $plugin['class'] ) ) : ?>
					<div class="action-tab warning">
						<h3><?php echo esc_html__("Install : ", 'accesspress-root'). esc_html($info->name). esc_html__(" Plugin", 'accesspress-root'); ?></h3>
