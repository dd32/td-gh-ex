...
if ( ! class_exists( 'Arrival_Welcome_Info' ) ) {
	...
	class Arrival_Welcome_Info {
		 ...
		 * Single instance.
		 *
		 * @var Arrival_Welcome_Info $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Arrival_Welcome_Info ) ) {
				self::$instance = new Arrival_Welcome_Info;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s Setup', 'arrival' ), $this->theme_name );
			/* translators: %s: theme name */
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s Setup', 'arrival' ), $this->theme_name );
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_arrival_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_arrival_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_page() {
			...
			<div class="wrap about-wrap wpopinfo-wrap">
				<h1><?php echo esc_html( $this->theme_name ); ?>&nbsp;-&nbsp;<?php esc_html_e('Version: ','arrival'); echo esc_html( $this->theme_version ); ?></h1>
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'arrival' ), wp_kses_post($method) );
			}
		}
		...
		public function useful_plugins() {
			...
			<div class="wpopsection wpopsection-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'arrival' ); ?></a>
				<?php endif; ?>
		...
		public function demo_content() {
			...
			if( class_exists('Wpop_Demo_Importer')){
				do_action('operation_importer_display');
			}else{
				?>
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'arrival-info', get_template_directory_uri() . '/inc/welcome/css/info.css', array(), '1.0.0' );
				wp_enqueue_script( 'arrival-info', get_template_directory_uri() . '/inc/welcome/js/info' . $min . '.js', array(), '1.0.0' );
			}
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'arrival_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'arrival_dismiss_status', 1 );
