...
<div class="feature-section recommended-plugins three-col demo-import-boxed" id="plugin-filter">
	<?php foreach ( $bee_news_recommended_plugins as $plugin => $prop ) { ?>
		...
		$active = $this->check_active( $plugin , $prop['plugin-file']);
		...
		<div class="col plugin_box">
			<?php if ( $prop['recommended'] ): ?>
				<span class="recommended"><?php _e( 'Recommended', 'bee-news' ); ?></span>
			<?php endif; ?>
			<img src="<?php echo esc_attr( $icon ) ?>" alt="plugin box image">
			<span
				class="version"><?php echo __( 'Version:', 'bee-news' ); ?><?php echo esc_html( $info->version ) ?></span>
			<span
