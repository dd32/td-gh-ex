...
class bee_news_Customizer_Helper {
	...
	public function bee_news_add_theme_options() {
		...
		foreach ( $dirs as $dir ) {
			...
			foreach ( $files as $file ) {
				if ( file_exists( get_template_directory() . '/include/libraries/customizer/settings/' . $dirname . '/' . $file . '.php' ) ) {
					include_once get_template_directory() . '/include/libraries/customizer/settings/' . $dirname . '/' . $file . '.php';
				}
			}
		}
	}
	...
	public function bee_news_change_default_panels() {
		...
		if ( ! bee_news_Helper::bee_news_on_iis() ) {
			...
			$wp_customize->get_control( 'custom_logo' )->description = esc_html__( 'The image logo, if set, will override the text logo. You can not have both at the same time. A tagline can be displayed under the text logo.', 'bee-news' );
