 ...
 * Add a widget
 */
class aladdin_SocialIcons extends WP_Widget {

	function aladdin_SocialIcons() {

		/* Widget settings. */
		$widget_ops = array(
		'classname' => 'aladdin_socialicons',
		'description' => __('Display Social Media Links.', 'aladdin' ));

		/* Widget control settings. */
		$control_ops = array(
		'width' => 250,
		'height' => 250,
		'id_base' => 'aladdin_socialicons_widget');

		/* Create the widget. */
		$this->WP_Widget('aladdin_socialicons_widget', __('AlSocial Media Icons', 'aladdin' ), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		foreach($instance as $id => $icon) {
			if(trim($icon) != '' && $id != 'is_vertical' && array_key_exists( $id, $icons ) ) {
				$out .= '<li><a style="background: url('.get_template_directory_uri().'/img/icons/'.($instance['is_small'] != ''  ? 'small/' : '' ) . esc_attr( $id ) . '.png)" href="'.esc_url($icon).'" target="_blank" title="'. esc_attr($id).'"></a></li>';
			}
		}
	...
	function form( $instance ) {
		...
		aladdin_echo_input_checkbox( $this, 'is_small', $instance, __( 'Small Icons.', 'aladdin'));
		aladdin_echo_input_checkbox( $this, 'is_vertical', $instance, __( 'Vertical Icons.', 'aladdin'));

		
		foreach ($icons as $id => $icon) {
			aladdin_echo_input_text( $this, $id, $instance, $id);
		}
	}
...
/* Register widget*/
function aladdin_register_social_widgets() {
	register_widget( 'aladdin_SocialIcons' );
}
add_action( 'widgets_init', 'aladdin_register_social_widgets' );
