...
class ActionsMB_Setting {
	...
	public function __construct( $manager, $name, $args = array() ) {
		...
		if ( $this->sanitize_callback )
			add_filter( "actionsmb_{$this->manager->name}_sanitize_{$this->name}", $this->sanitize_callback, 10, 2 );
	}
	...
	public function get_value() {
		...
		return ! $value && actionsmb()->is_new_post ? $this->default : $value;
	}
	...
	public function get_field_name() {
		...
		return "actionsmb_{$this->manager->name}_setting_{$this->name}";
	}
	...
	public function sanitize( $value ) {
		...
		return apply_filters( "actionsmb_{$this->manager->name}_sanitize_{$this->name}", $value, $this );
	}
