 ...
 * @link       https://github.com/justintadlock/actionsmb
...
class ActionsMB_Manager {
	...
	public function register_section( $section, $args = array() ) {
		...
		if ( ! is_object( $section ) ) {
			...
			$type = isset( $args['type'] ) ? actionsmb()->get_section_type( $args['type'] ) : actionsmb()->get_section_type( 'default' );
	...
	public function register_control( $control, $args = array() ) {
		...
		if ( ! is_object( $control ) ) {
			...
			$type = isset( $args['type'] ) ? actionsmb()->get_control_type( $args['type'] ) : actionsmb()->get_control_type( 'default' );
	...
	public function register_setting( $setting, $args = array() ) {
		...
		if ( ! is_object( $setting ) ) {
			...
			$type = isset( $args['type'] ) ? actionsmb()->get_setting_type( $args['type'] ) : actionsmb()->get_setting_type( 'default' );
	...
	public function print_template() { ?>
		...
		<script type="text/html" id="tmpl-actionsmb-manager-<?php echo esc_attr( $this->type ); ?>">
	...
	public function get_template() {
		actionsmb_get_manager_template( $this->type );
	}
}
