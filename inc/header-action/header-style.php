...
<?php 
	function asthir_headerstyle_one_item(){
 
	$asthir_menu_position = get_theme_mod( 'asthir_menu_position','right' );
	$asthir_logo_position = get_theme_mod( 'asthir_logo_position', 'left' );
...
?>
	...
	<div class="brand-menubar aakmstyle1">
		<div class="container">
			<div class="d-flex aak-menu-wrap aak-logo-<?php echo esc_attr($asthir_logo_position); ?>">
				<div class="aakbar-item site-branding">
					<?php do_action('asthir_logo_text'); ?>
				</div><!-- .site-branding -->
				<div class="aak-main-menu flex-grow-1 menu-<?php echo esc_attr($asthir_menu_position); ?>">
					<?php do_action( 'asthir_main_menu' ); ?>
				</div>
			</div>
		</div>
	</div><!-- #brand menu bar -->
...
add_action('asthir_header_style_one','asthir_headerstyle_one_item');

	function asthir_headerstyle_two_item(){
...
?>
		...
		$asthir_dheader_text = get_theme_mod( 'display_header_text', 1 );
		 ?>
		<?php if(has_header_image()): ?>
		<div class="site-branding has-himg text-center <?php if($asthir_dheader_text && has_custom_logo()): ?>aak-two-logo<?php endif; ?>">
			<div class="aak-header-img">
				<?php the_header_image_tag(); ?>
			</div>
		<?php else: ?>
			<div class="site-branding text-center <?php if($asthir_dheader_text && has_custom_logo()): ?>aak-two-logo<?php endif; ?>">
		<?php endif; ?>
		<div class="aakbar-item site-branding mr-lg-auto flex-grow-1">
			<div class="headerlogo-text htl-<?php echo esc_attr($asthir_logo_position); ?>">
		...
		<div class="brand-menubar">
			<div class="container">
				<div class="aakmenu-display aadhs-two">
					<div class="aak-menu-main menu-<?php echo esc_attr($asthir_menu_position); ?>">
						<?php do_action( 'asthir_main_menu' ); ?>
					</div>
				</div>
			</div>
...
<?php
	...
	add_action('asthir_header_style_two','asthir_headerstyle_two_item');
