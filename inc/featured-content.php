...
class Anderson_Featured_Content {

	/**
	 * The maximum number of posts that a Featured Content area can contain.
	 *
	 * @see Anderson_Featured_Content::init()
	 */
	public static $max_posts = 4;
	 ...
	 *
	 * @uses Anderson_Featured_Content::$max_posts
	 */
	public static function init() {

		add_filter( 'anderson_get_featured_content',     array( __CLASS__, 'get_featured_posts' )    );
	 ...
	 * Get featured posts
	 *
	 * @uses Anderson_Featured_Content::get_featured_post_ids()
	 *
	 * @return array
	 */
	...
	public static function get_featured_post_ids() {
		...
		if ( ! empty( $featured_ids ) ) {
			return array_map( 'absint', apply_filters( 'anderson_featured_content_post_ids', (array) $featured_ids ) );
		}
		...
		if ( $term ) {
			...
			return apply_filters( 'anderson_featured_content_post_ids', array() );
		}
		...
		return apply_filters( 'anderson_featured_content_post_ids', $featured_ids );
	}
	 ...
	 * @see Anderson_Featured_Content::validate_settings().
	 */
	 ...
	 *
	 * @uses Anderson_Featured_Content::get_featured_post_ids();
	 * @uses Anderson_Featured_Content::get_setting();
	 ...
	 * is deleted by Anderson_Featured_Content::validate_settings().
	 *
	 ...
	 *
	 * @uses Anderson_Featured_Content::validate_settings()
	 *
	 * @return void
	 */
	...
	public static function customize_register( $wp_customize ) {
		$wp_customize->add_section( 'anderson_featured_content', array(
			'title'          => __( 'Featured Content', 'anderson-lite' ),
			'description'    => sprintf( __( 'Easily feature all posts with the <a href="%1$s">"featured" tag</a> or a tag of your choice. Your theme supports up to %2$s posts in its featured content area.', 'anderson-lite' ), admin_url( '/edit.php?tag=featured' ), absint( self::$max_posts ) ),
			'priority'       => 40,
		 ...
		 *
		 * Sanitization callback registered in Anderson_Featured_Content::validate_settings().
		...
		$wp_customize->add_control( 'featured-content[tag-name]', array(
			...
			'section'        => 'anderson_featured_content',
			'priority'       => 20,
		) );
	...
	public static function enqueue_scripts() {
		wp_enqueue_script( 'anderson-featured-content-suggest', get_template_directory_uri() . '/js/featured-content-admin.js', array( 'jquery', 'suggest' ), '20150401', true );
	}
	 ...
	 * Anderson_Featured_Content::get_featured_content().
	 *
	 * @uses Anderson_Featured_Content::delete_transient()
	 *
	 * @param array $input
	 * @return array $output
	 */
...
Anderson_Featured_Content::setup();
