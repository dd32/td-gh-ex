...
class aladdin_items_category extends WP_Widget {
	...
	*/
	public function __construct() {
		...
		/* Widget settings. */
		$widget_ops = array(
		'classname' => 'aladdin_items_category',
		'description' => __('Display Items from the Category or latest posts.', 'aladdin' ));

		/* Widget control settings. */
		$control_ops = array(
		'width' => 250,
		'height' => 250,
		'id_base' => 'aladdin_items_category_widget');


		/* Create the widget. */		
		parent::__construct( 'aladdin_items_category_widget', __('Al Items from Category (Aladdin)', 'aladdin' ), $widget_ops, $control_ops );
			...
			<div class="main-wrapper-image <?php echo $pos_class;?>" style="padding:<?php echo esc_attr($instance['padding_top']);?>px <?php echo esc_attr($instance['padding_right']);?>% <?php echo esc_attr($instance['padding_bottom'])?>px <?php echo esc_attr($instance['padding_left'])?>%">
				...
				<div class="description">
					<article>
						...
						<a href="<?php echo esc_url( get_term_link( $instance['category'], $tax ) ); ?>" class="link" rel="bookmark"><?php _e('Open Archive', 'aladdin'); ?></a>
					</article> <!-- article -->
				</div> <!-- .description -->
	...
	*/
	...
	function defaults( $instance ){
		...
		$defaults = array('title' =>  __('Recent Posts', 'aladdin'),
						  'category'   => '0',	
						  'count'   => '4',	
						  'columns'   => 'column-4',
 ...
 * @since Aladdin 1.0.0
 */
 function aladdin_register_items_cat_widgets() {
	register_widget( 'aladdin_items_category' );
}
add_action( 'widgets_init', 'aladdin_register_items_cat_widgets' );
