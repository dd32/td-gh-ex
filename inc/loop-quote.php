...
?>
				<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<?php if(is_singular()){ ?>
						...
						the_title( sprintf( '<h2 class="entry-title">%1$s <a href="%2$s" title="%3$s" rel="bookmark">', hjyl_get_svg( array( 'icon' => 'quote') ), esc_url( get_permalink()), esc_html(get_the_title()) ), '</a></h2>' );
						}
						?>
						<div class="date">
						...
						<?php }else{ ?>
						<div class="date">
							<span class="day"><a href="<?php echo esc_url( the_permalink() ); ?>" title="<?php the_title_attribute( array( 'before' => '', 'after' => '' ) ); ?>" rel="bookmark"><?php echo hjyl_get_svg( array( 'icon' => 'quote') ); ?></a></span>
						</div>
						<?php } ?>
					</header>
