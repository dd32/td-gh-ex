...
if( ! class_exists( 'Best_News_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Best_News_Control_Repeater extends WP_Customize_Control {
        ...
        public function __construct( $manager, $id, $args = array() ) {
            ...
            $this->row_label = array(
                'type' => 'text',
                'value' => esc_attr__( 'row', 'best-news' ),
                'field' => false,
            );
            ...
            if ( empty( $this->button_label ) ) {
             ...
             $this->button_label = sprintf( esc_attr__( 'Add new %s', 'best-news' ), $this->row_label['value'] );
         }
        ...
        public function enqueue() {    
            ...
            wp_enqueue_script( 'best-news-repeater-js', get_template_directory_uri() . '/assets/customizer/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
            wp_enqueue_style( 'best-news-repeater-css', get_template_directory_uri() . '/assets/customizer/repeater.css', null );
        }
        ...
        protected function render_content() {
            ...
            <?php if ( isset( $this->choices['limit'] ) ) : ?>
                ...
                printf( esc_attr__( 'Limit: %s rows', 'best-news' ), esc_html( $this->choices['limit'] ) ); ?></p>
            <?php endif; ?>
            <button class="button-secondary repeater-add" disabled ><?php echo esc_html( $this->button_label ); ?></button>
        ...
        public function repeater_js_template() {
            ...
            <label>
                ...
                <span class="description customize-control-description">{{{ field.description }}}</span>
                <# } #>
                <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'best-news' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
            ...
            <figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'best-news' ); ?>" >
                <# if ( field.default ) { #>
                <# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
                <img src="{{{ defaultImageURL }}}">
                <# } else { #>
                <?php esc_attr_e( 'No Image Selected', 'best-news' ); ?>
                <# } #>
            </figure>
            ...
            <div class="actions">
                ...
                <button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'best-news' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'best-news' ); ?>" >
                    ...
                    <# } else { #>
                    <?php esc_attr_e( 'Add Image', 'best-news' ); ?>
                    <# } #>
                </button>
                <# if ( field.default.id ) { #>
            ...
            <figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'best-news' ); ?>" >
                <# if ( field.default ) { #>
                ...
                <# } else { #>
                <?php esc_attr_e( 'No File Selected', 'best-news' ); ?>
                <# } #>
            </figure>
            
            <div class="actions">
                <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
                <button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'best-news' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'best-news' ); ?>" >
                    <# if ( field.default ) { #>
                    <?php esc_attr_e( 'Change File', 'best-news' ); ?>
                    <# } else { #>
                    <?php esc_attr_e( 'Add File', 'best-news' ); ?>
                    <# } #>
                </button>
                <# if ( field.default.id ) { #>
                <input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
