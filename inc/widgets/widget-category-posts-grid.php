...
class Anderson_Category_Posts_Grid_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => esc_html__( 'Displays your posts from a selected category in a grid layout. Please use this widget ONLY in the Magazine Homepage widget area.', 'anderson-lite' )
		);
		parent::__construct('anderson_category_posts_grid', sprintf( esc_html__( 'Category Posts: Grid (%s)', 'anderson-lite' ), 'Anderson' ), $widget_ops);
	...
	function render( $settings ) {
		...
		$settings['layout'] = ( $settings['thumbnails'] == true ? 'small-post' : 'big-post' );
		$image = ( $settings['thumbnails'] == true ? 'anderson-category-posts-widget-small' : 'anderson-category-posts-widget-big' );
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('number'); ?>"><?php esc_html_e( 'Number of posts:', 'anderson-lite' ); ?>
				...
				<br/><span class="description"><?php esc_html_e( 'Please chose an even number (2, 4, 6, 8).', 'anderson-lite' ); ?></span>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id('thumbnails'); ?>">
				...
				<?php esc_html_e( 'Display as small posts grid with thumbnails', 'anderson-lite' ); ?>
			</label>
		</p>
...
// Register Widget
add_action( 'widgets_init', 'anderson_register_category_posts_grid_widget' );

function anderson_register_category_posts_grid_widget() {
