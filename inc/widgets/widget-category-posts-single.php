...
class Smartline_Category_Posts_Single_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => esc_html__( 'Displays a single post from a selected category. Please use this widget ONLY in the Magazine Homepage widget area.', 'smartline-lite' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct('smartline_category_posts_single', sprintf( esc_html__( 'Category Posts: Single (%s)', 'smartline-lite' ), 'Smartline' ), $widget_ops);
	...
	function form( $instance ) {
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id( 'postmeta' ); ?>" name="<?php echo $this->get_field_name( 'postmeta' ); ?>">
				...
				<option value="4" <?php selected( $settings['postmeta'], 4); ?>><?php esc_html_e( 'Display date, author and comments', 'smartline-lite' ); ?></option>
			</select>
		</p>
