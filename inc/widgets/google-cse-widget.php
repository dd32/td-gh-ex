...
class aadya_googlecse_widget extends WP_Widget {

         public function __construct() {

			/* Widget settings. */
			$widget_ops = array( 'classname' => 'google-custom-search', 'description' => __('Enable Google Custom Search on Your Website.', 'aadya') );

			/* Widget control settings. */
			$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'widget-google-cse' );

			/* Create the widget. */
			$this->WP_Widget( 'widget-google-cse', __('(Aadya) Google Custom Search', 'aadya'), $widget_ops, $control_ops );		
        ...
        public function form( $instance ) {		
			...
			?>			
		<p>  
			<label for="<?php echo $this->get_field_id( 'google_cse_unique_id' ); ?>"><?php _e('Search Engine Id:', 'aadya'); ?></label>  
		...
		<p>  
			<label for="<?php echo $this->get_field_id( 'search_results_page_url' ); ?>"><?php _e('Results Page URI:', 'aadya'); ?></label>  
        ...
        public function widget( $args, $instance ) {
			...
			<form class="form-inline" role="form" method="get" id="cse-search-box" action="<?php echo $search_results_page_url; ?>">					
				...
				<span class="input-group-btn">
					<button type="submit" class="btn btn-primary" name="sa"><i class="fa fa-search"></i></button>
				</span>
				</div>
			</form>	
