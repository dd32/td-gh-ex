...
class Beetle_Magazine_Posts_Boxed_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Setup Widget
		parent::__construct(
			'beetle-magazine-posts-boxed', // ID
			sprintf( esc_html__( 'Magazine Posts: Boxed (%s)', 'beetle' ), wp_get_theme()->Name ), // Name
			array( 
				'classname' => 'beetle_magazine_posts_boxed', 
				'description' => esc_html__( 'Displays your posts from a selected category in a boxed layout. Please use this widget ONLY in the Magazine Homepage widget area.', 'beetle' ) 
			) // Args
		);
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_beetle_magazine_posts_boxed', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_beetle_magazine_posts_boxed', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
			...
			while( $posts_query->have_posts() ) :
				...
				if( isset($i) and $i == 0 ) : ?>
					...
					<article id="post-<?php the_ID(); ?>" <?php post_class( 'large-post clearfix' ); ?>>
						...
						<a href="<?php the_permalink() ?>" rel="bookmark"><?php the_post_thumbnail( 'beetle-thumbnail-large' ); ?></a>
				...
				<?php else: ?>
					...
					<article id="post-<?php the_ID(); ?>" <?php post_class( 'medium-post clearfix' ); ?>>
						...
						<?php if ( has_post_thumbnail() ) : ?>
							<a href="<?php the_permalink() ?>" rel="bookmark"><?php the_post_thumbnail( 'beetle-thumbnail-medium' ); ?></a>
						<?php endif; ?>
			...
			while( $posts_query->have_posts() ) :
				...
				<?php else: ?>
					...
					<article id="post-<?php the_ID(); ?>" <?php post_class( 'small-post clearfix' ); ?>>
						...
						<?php if ( has_post_thumbnail() ) : ?>
							<a href="<?php the_permalink() ?>" rel="bookmark"><?php the_post_thumbnail( 'beetle-thumbnail-small' ); ?></a>
						<?php endif; ?>
	...
	function widget_title( $args, $settings ) {
		...
		if( ! empty( $widget_title ) ) :
			...
			if( true == $settings['category_link'] and $settings['category'] > 0 ) : 
				...
				$link_title = sprintf( esc_html__( 'View all posts from category %s', 'beetle' ), get_cat_name( $settings['category'] ) );
...
?>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e( 'Title:', 'beetle' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id('category'); ?>"><?php esc_html_e( 'Category:', 'beetle' ); ?></label><br/>
		...
		<p>
			<label for="<?php echo $this->get_field_id('layout'); ?>"><?php esc_html_e( 'Post Layout:', 'beetle' ); ?></label><br/>
			<select id="<?php echo $this->get_field_id('layout'); ?>" name="<?php echo $this->get_field_name('layout'); ?>">
				<option <?php selected( $settings['layout'], 'horizontal' ); ?> value="horizontal" ><?php esc_html_e( 'Horizontal Arrangement', 'beetle' ); ?></option>
				<option <?php selected( $settings['layout'], 'vertical' ); ?> value="vertical" ><?php esc_html_e( 'Vertical Arrangement', 'beetle' ); ?></option>
			</select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_date' ); ?>">
				...
				<?php esc_html_e( 'Display post date', 'beetle' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_author' ); ?>">
				...
				<?php esc_html_e( 'Display post author', 'beetle' ); ?>
			</label>
		</p>
...
<?php
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'widget_beetle_magazine_posts_boxed', 'widget' );
...
// Register Widget
add_action( 'widgets_init', 'beetle_register_magazine_posts_boxed_widget' );

function beetle_register_magazine_posts_boxed_widget() {

	register_widget( 'Beetle_Magazine_Posts_Boxed_Widget' );
