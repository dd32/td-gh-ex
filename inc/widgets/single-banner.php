...
if ( !class_exists('NNfy_Single_Banner') ) {
	class NNfy_Single_Banner extends WP_Widget{

		function __construct(){

			$widget_options = array(
				'description' 					=> esc_html__('This widget show banner', '99fy'), 
				'customize_selective_refresh' 	=> true,
			);

			parent:: __construct('NNfy_Single_Banner', esc_html__( 'NNfy: Banner', '99fy'), $widget_options );
	        ...
	        <?php echo $args['before_widget']; 
				...
				if ( !empty($image) ): ?>
        			<div class="sidebar-banner">
        				<?php if ($banner_link): ?><a href="<?php echo esc_url( $banner_link ); ?>"> <?php endif ?><img src="<?php echo esc_url( $image ) ; ?>" alt="<?php esc_html_e('Banner Image', '99fy'); ?>"><?php if ($banner_link): ?>
        					</a>
        				<?php endif ?>
        			</div>
        		<?php endif ?>
		...
		public function form($instance){ 
			...
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('banner_link')); ?>"><?php echo esc_html__('Banner Link:' ,'99fy') ?></label>
...
function NNfy_Single_Banner() {
    register_widget( 'NNfy_Single_Banner' );
}
add_action( 'widgets_init', 'NNfy_Single_Banner' );