...
class smartline_Category_Posts_Columns_Widget extends WP_Widget {
	...
	?>
		<div id="widget-category-posts-columns" class="widget-category-posts clearfix">
			...
			if ( $category_one > 0 && $category_two > 0 ) :
				...
				smartline_display_category_posts_columns($category_one, $category_two);
				
			else : 
			
				_e( 'Please specify two categories on the Category Posts Widget settings.', 'smartline-lite' );
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id('category_one'); ?>"><?php _e('Left Category:', 'smartline-lite'); ?><br/>
		...
		<p>
			<label for="<?php echo $this->get_field_id('category_two'); ?>"><?php _e('Right Category:', 'smartline-lite'); ?><br/>
