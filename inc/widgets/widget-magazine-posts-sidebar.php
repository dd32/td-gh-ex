...
class Admiral_Magazine_Posts_Sidebar_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			esc_html__( 'Magazine (Sidebar)', 'admiral' ), // Name.
			array(
				'classname' => 'admiral-magazine-sidebar-widget',
	...
	private function default_settings() {
		...
		$defaults = array(
			'title'    => esc_html__( 'Magazine (Sidebar)', 'admiral' ),
			'category' => 0,
			'number'   => 4,
			'excerpt'  => false,
		);
	...
	function render( $settings ) {
		...
		// Get cached post ids.
		$post_ids = admiral_get_magazine_post_ids( $this->id, $settings['category'], $settings['number'] );
		...
		if ( $posts_query->have_posts() ) :
			...
			set_query_var( 'admiral_post_excerpt', (bool) $settings['excerpt'] );
	...
	function widget_title( $args, $settings ) {
		...
		if ( ! empty( $widget_title ) ) :
			...
			$widget_title = admiral_magazine_widget_title( $widget_title, $settings['category'] );
	...
	function update( $new_instance, $old_instance ) {
		...
		admiral_flush_magazine_post_ids();
