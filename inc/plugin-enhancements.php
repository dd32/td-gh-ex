...
class Theme_Plugin_Enhancements {
	...
	function __construct() {
		...
		if ( empty( $this->dependencies ) ) :
			...
			$this->plugins = array(
	    		...
	    		'message' => sprintf(
					__( 'The %1$s is required to use some of this theme’s features, including: ', 'apostrophe' ),
				'<strong>' . __( 'Jetpack plugin', 'apostrophe' ) . '</strong>' ),
	...
	function get_module_name( $module ) {
		$module_names = array(
			   'none'                 => __( 'no specific module needed', 'apostrophe' ),
			   'custom-content-types' => __( 'Custom Content Types module', 'apostrophe' ),
		);
	...
	function admin_notices() {
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					__( ' Please activate %1$s. %2$s', 'apostrophe' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . __( 'Activate', 'apostrophe' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					__( ' Please install %1$s. %2$s', 'apostrophe' ),
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . __( 'Install', 'apostrophe' ) . '</a>' : ''
				);
			}
		...
		foreach ( $this->unactivated_modules as $module => $features ) :
			...
			$notice .= '<p>';
			$notice .= sprintf(
				__( 'To use %1$s, please activate the Jetpack plugin’s %2$s.', 'apostrophe' ),
