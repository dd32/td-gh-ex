<?php
/**
 * Atento function for rendering meta-boxes in single post/page area
 *
 * @package Atento
 */
...
/*----------------------------------------------------------------------
# Start Class Atento_Post_Global_Meta_Box
-------------------------------------------------------------------------*/
if ( ! class_exists( 'Atento_Post_Global_Meta_Box' ) ) {

    class Atento_Post_Global_Meta_Box {
        ...
        public function post_meta( $post ) {
            ...
            add_meta_box(
                'post_meta_fields',
                $obj->labels->singular_name . ' '. esc_html__( 'Settings', 'atento' ),
        ...
        public function display_meta_box( $post ) {
            ...
            // Empty notice
            $empty_notice = '<p>'. esc_html__( 'No meta settings available for this post type or user.', 'atento' ) .'</p>';
                ...
                <div class="metabox-settings-tabs">
                    <ul class="metabox-tab-nav">
                        ...
                        foreach ( $active_tabs as $tab ) {
                            $tab_count++;
                            // Define tab title
                            $tab_title = $tab['title'] ? $tab['title'] : esc_html__( 'Other', 'atento' ); ?>
        ...
        private function meta_array( $post = null ) {
            ...
            // Default variable
            $default = esc_html__( 'Default', 'atento' );

            // Sidebar Layout Tab
            $array['sidebar']  = array(
                'title'     => esc_html__( 'Sidebar Layout', 'atento' ),
                'settings'  => array(
                    'sidebar_layout'    =>array(
                        'description'   => esc_html__( 'Default value is inherit from single post or page settings.', 'atento' ),
                        'id'            => $prefix . 'sidebar_layout',
                        'type'          => 'radio',
                        'options'       => atento_content_layouts( array( 'default-sidebar' => $default ) ),
            ...
            return apply_filters( 'atento_metabox_array', $array, $post );
        }
    }

    // Class needed only in the admin
    if ( is_admin() ) {
        new Atento_Post_Global_Meta_Box;
    }
}
