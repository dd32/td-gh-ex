...
function conica_premium_admin_menu() {
    ...
    $conica_upgrade_page = add_theme_page( __( 'Premium', 'conica' ), '<span class="premium-link">' . __( 'Premium', 'conica' ) . '</span>', 'edit_theme_options', 'premium_upgrade', 'conica_render_upgrade_page' );
}
 ...
 */
function conica_load_upgrade_page_scripts( $hook ) {
    global $conica_upgrade_page;
    if ( $hook != $conica_upgrade_page )
        return;
    
    wp_enqueue_style( 'conica-upgrade-css', get_template_directory_uri() . '/upgrade/css/upgrade-admin.css' );
    wp_enqueue_script( 'caroufredsel', get_template_directory_uri() . '/js/jquery.carouFredSel-6.2.1-packed.js', array( 'jquery' ), conica_THEME_VERSION, true );
...
function conica_render_upgrade_page() {
	$theme = basename( get_template_directory() ); // = conica
		...
		case 'order-entered' :
			...
			$result = wp_remote_get(
				add_query_arg( array(
					...
					'theme' => $theme
				), CONICA_UPDATE_URL . '/premium/' . $theme . '/validate-order.php' )
			);
			...
			<div class="wrap upgrade-page-wrap">
			    ...
			    <div class="upgrade-page-inner-wrap">
			    	...
			    	<div class="upgrade-order-number-info-form-after">
					    ...
					    <h3 class="upgrade-page-sub-title"><?php _e( "Order Number: ", 'conica' ) ?><big><strong><?php echo get_theme_mod( $option_name ); ?></strong></big></h3>
					    
					    <?php if ( is_null( $valid ) ) : ?>
					    
							<p>
								<?php _e( "There was a problem contacting our validation servers.", 'conica' ) ?><br /><br />
								<?php _e( "Please try again later, or upgrade manually using the ZIP file we sent you.", 'conica' ) ?>
							</p>
							<p class="submit">
						...
						elseif ( $valid ) : ?>
							...
							<p>
								<?php _e( "We've validated your order number, and it works!", 'conica' ) ?>
								<br /><br />
								<?php
								printf(
									__( 'You can now update your theme on the <a href="%s">Themes page</a>,<br />but <strong>please note</strong> this can take a few minutes to show up so please be patient :)', 'conica' ),
									admin_url( 'themes.php' )
								); ?>
								<br /><br />
								<?php _e( 'This update will add all the premium features.', 'conica' ) ?>
							</p>
							<p class="submit">
								<?php
								$theme_update_url = wp_nonce_url( admin_url( 'update.php?action=upgrade-theme&amp;theme=' . urlencode( $theme ) ), 'upgrade-theme_' . $theme );
								$update_onclick = 'onclick="if ( confirm(\'' . esc_js( __( "Updating may lose the theme settings you've made to the free version. Click 'OK' to update.", 'conica' ) ) . '\') ) { return true; } return false;"'; ?>
								<a href="<?php echo esc_url( $theme_update_url ) ?>" <?php echo $update_onclick ?> class="upgrade-result-button">
									<?php _e( 'Update Theme Now', 'conica' ) ?>
								</a>
							</p>
						...
						<?php else : ?>
							...
							<p>
								<?php _e( "We couldn't validate your order number.", 'conica' ) ?><br />
								<?php _e( "There might be a problem with our validation server.", 'conica' ) ?><br /><br />
								<?php _e( "Please try again later, or upgrade manually using the ZIP file we sent you.", 'conica' ) ?>
							</p>
							<p class="submit">
								<a href="<?php echo esc_url( admin_url( 'themes.php?page=premium_upgrade' ) ) ?>" class="upgrade-result-button">
 ...
 */
function conica_premium_dashboard_note() {
	$theme = basename( get_template_directory() ); // = conica
	$option_name = $theme . '_user_order_number';
	$order_number = get_theme_mod( $option_name );
...
add_filter( 'rightnow_end', 'conica_premium_dashboard_note' );
