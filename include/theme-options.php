<?php
/**
 * theme options and features for 
 * @theme BetiLu
 */
...
<div class="options-container">
    <figure> <h1><?php _e( 'Options Page', 'betilu' ); ?></h1>  </figure>
	<h2><?php _e( 'BetiLu Theme Settings', 'betilu' ); ?></h2>
		...
		<?php endif; ?>
	        ...
	        <form action="options.php" method="POST">
                ...
                ?>
            <hr>         
                <h3><?php _e( 'Top Right Logo - Square or Circular', 'betilu' ); ?></h3>     
                <table class="options-table">
                <tr><td><label><?php _e( 'Select Logo Style - Select Square or Circular', 'betilu' ); ?></label></td>
                <td><select name="betilu_theme_options[border_radius]" class="betilu_logo_select">  
<option value="0px" <?php if ( $options['border_radius'] == '0px' ) echo 'selected="selected"'; ?>><?php _e( 'Square', 'betilu' ); ?></option>
<option value="9999px" <?php if ( $options['border_radius'] == '9999px' ) echo 'selected="selected"'; ?>><?php _e( 'Circular', 'betilu' ); ?></option>  
                    </select></td></tr>
                </table>
            ...
            <hr>
                ...
                <table class="options-table">
                <tr><td><div><?php _e( 'Add mid-section banner text here. Same text will appear on lower banner.', 'betilu' ); ?></div><br />         
...
value="<?php if (!empty($options['betilu_new_text'])) echo esc_attr( $options['betilu_new_text'] ); ?>" /></td></tr>
                ...
                <p><img src="<?php echo get_template_directory_uri(); ?>/images/info_black.png" height="24"/><?php _e( 'You can add HTML4 tags such as', 'betilu' ); ?> <code>&lt;h1&gt; &lt;h2&gt; &lt;h3&gt; &lt;p&gt; &lt;b&gt;</code></p>
            <hr>
                <h3><?php _e( 'Sidebar-Top-Right, Middle-Bar and Bottom-Bar Background set here', 'betilu' ); ?></h3>
                <table class="options-table"><tr><td><label><?php _e( 'Change sidebar-right and middle banner background color', 'betilu' ); ?></label>
<input type="text" name="betilu_theme_options[top_backgrnd]" size="20" value="<?php echo esc_attr( $options['top_backgrnd'] ); ?>" class="betilu-color-field" /></td></tr>
                ...
                <p><img src="<?php echo get_template_directory_uri(); ?>/images/info_black.png" height="24"/><?php _e( 'Selected color will appear on all mid-sections, Sticky posts and the Sidebar', 'betilu' ); ?></p>
            <hr>
                <h3><?php _e( 'Add your Phone and eMail plus social media links here', 'betilu' ); ?></h3>
                <table class="options-table">
                <tr>
                <?php $options = get_option( 'betilu_theme_options' ); ?>
                <td><label><?php _e( 'Phone Number', 'betilu' ); ?></label> </td><td><input type="text" name="betilu_theme_options[phonenumber]" size="40" 
value="<?php if( !empty( $options['phonenumber'] ) ) echo esc_attr( $options['phonenumber'] ); ?>" /></td></tr>
                <tr><td><label><?php _e( 'Facebook', 'betilu' ); ?></label> </td><td><input type="text" name="betilu_theme_options[facebookurl]" size="40" 
value="<?php if( !empty( $options['facebookurl'] ) ) echo esc_url( $options['facebookurl'] ); ?>" /></td</tr>
                <tr><td><label><?php _e( 'Twitter', 'betilu' ); ?></label> </td><td><input type="text" name="betilu_theme_options[twitterurl]" size="40" 
value="<?php if( !empty( $options['twitterurl'] ) ) echo esc_url( $options['twitterurl'] ); ?>" /></td</tr>
                <tr><td><label><?php _e( 'E-Mail', 'betilu' ); ?></label> </td><td><input type="text" name="betilu_theme_options[betilu_email]" size="40" 
value="<?php if( !empty( $options['betilu_email'] ) ) echo esc_url( $options['betilu_email'] ); ?>" /></td</tr> 
                </table>
                <p><img src="<?php echo get_template_directory_uri(); ?>/images/info_black.png" height="24"/><?php _e( 'Appears under Single Post Ariticles. Type only id for Twitter and url without the', 'betilu' ); ?> <code>https://</code> <?php _e( 'for facebook.', 'betilu' ); ?></p>
            <hr>
                <?php submit_button(); ?>
                </form>
</div>
