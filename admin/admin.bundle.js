...
var withBlockControls = createHigherOrderComponent(function (BlockEdit) {
	return function (props) {
		...
		var varName = 'has-no-margin-bottom';

		// Check if we actually have custom class names to work with
		var allowsCustomClassName = hasBlockSupport(props.name, 'customClassName', true);

		// Check if the class is in the className attribute
		var hasCustomClassName = function hasCustomClassName() {
   ...
   */
		var addCustomClassName = function addCustomClassName(customClassName) {
			var newClassName = '';
			if (typeof props.attributes.className !== 'string') {
				newClassName = customClassName;
			} else {
				newClassName = (0, _uniq3.default)([customClassName].concat(_toConsumableArray(props.attributes.className.split(' ')))).join(' ').trim();
			}
   ...
   * @param  {string} customClassName The class name we want to remove
  	 */
		var removeCustomClassName = function removeCustomClassName(customClassName) {

			var classNameArray = props.attributes.className.split(' ');
			classNameArray.splice(classNameArray.indexOf(varName), 1);
			var newClassName = classNameArray.join(' ').trim();
   ...
   */
		var toggleCustomClassName = function toggleCustomClassName() {
		...
		if (allowsCustomClassName && props.isSelected) {
			return React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						PanelBody,
						{ title: __('AGNCY', 'agncy-js'), initialOpen: false },
						React.createElement(CheckboxControl, {
							label: React.createElement(
								'strong',
								null,
								' ',
								__('Remove bottom margin', 'agncy-js'),
								' '
							),
							help: __('Remove the bottom margin and let the block below directly follow.', 'agncy-js'),
							checked: hasCustomClassName(),
							onChange: function onChange(value) {
								toggleCustomClassName();
							}
						})
					)
				)
...
wp.hooks.addFilter('editor.BlockEdit', 'agncy/with-block-controls', withBlockControls);
/* WEBPACK VAR INJECTION */}.call(this, __webpack_require__(11)))
