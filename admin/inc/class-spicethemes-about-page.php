...
if (!class_exists('Rockers_About_Page')) {
	class Rockers_About_Page {
		...
		function __construct() {
			...
			add_action('wp_ajax_rockers_update_rec_acts', array($this, 'update_recommended_actions_watch'));
			add_action('load-themes.php', array($this, 'activation_admin_notice'));
			/* enqueue script and style for welcome screen */
			add_action( 'admin_enqueue_scripts', array( $this, 'style_and_scripts' ) );
			
			/* load welcome screen */
			add_action( 'rockers_info_screen', array( $this, 'getting_started' ), 	    10 );
			add_action( 'rockers_info_screen', array( $this, 'github' ), 		            40 );
			add_action( 'rockers_info_screen', array( $this, 'welcome_free_pro' ), 				50 );
			add_action( 'rockers_info_screen', array( $this, 'recommended_actions' ), 				50 );
			add_action( 'rockers_info_screen', array( $this, 'changelog' ), 				60 );
			}
	...
	public function style_and_scripts( $hook_suffix ) {
		...
		if ( 'appearance_page_rockers-welcome' == $hook_suffix ) {
			...
			wp_enqueue_style('rockers-welcome_customizer', RC_TEMPLATE_DIR_URI . '/admin/assets/css/welcome_customizer.css');
	...
	public function scripts_for_customizer() {
		...
		if( get_option('rockers_show_required_actions') ):
			$rockers_show_required_actions = get_option('rockers_show_required_actions');
		else:
			$rockers_show_required_actions = array();
		endif;

		if( !empty($rockers_required_actions) ):
			foreach( $rockers_required_actions as $rockers_required_action_value ):
				if(( !isset( $rockers_required_action_value['check'] ) || ( isset( $rockers_required_action_value['check'] ) && ( $rockers_required_action_value['check'] == false ) ) ) && ((isset($rockers_show_required_actions[$rockers_required_action_value['id']]) && ($rockers_show_required_actions[$rockers_required_action_value['id']] == true)) || !isset($rockers_show_required_actions[$rockers_required_action_value['id']]) )) :
					$nr_actions_required++;
				endif;
			endforeach;
		endif;

		wp_localize_script( 'rockers-info-screen-customizer-js', 'rockersLiteWelcomeScreenCustomizerObject', array(
		...
		public function add_theme_info_menu() {
			...
			$title = sprintf(esc_html__('About %1$s %2$s', 'rockers'), esc_html($theme->get('Name')), $count);
			add_theme_page(sprintf(esc_html__('Welcome to %1$s %2$s', 'rockers'), esc_html($theme->get('Name')), esc_html($theme->get('Version'))), $title, 'edit_theme_options', 'rockers-welcome', array($this, 'print_welcome_page'));
		}
		...
		public function welcome_admin_notice() {
			...
			<div class="updated notice is-dismissible">
				<p><?php echo sprintf( esc_html__("Rockers theme is installed. To take full advantage of the features this theme has to offer visit our %1\$s welcome page %2\$s", "rockers"), '<a href="' . esc_url( admin_url( 'themes.php?page=rockers-welcome' ) ) . '">', '</a>' ); ?></p>
				<p><a href="<?php echo esc_url( admin_url( 'themes.php?page=rockers-welcome' ) ); ?>" class="button" style="text-decoration: none;"><?php esc_html_e( 'Get started with Rockers theme', 'rockers' ); ?></a></p>
			</div>
			<?php
		}
	  ...
	  <div class="container-fluid">
		<div class="row">
		<div class="col-md-12">
			<div class="wrap theme-info-wrap rockers-wrap">
				...
				<div class="theme-welcome-container" style="min-height:300px;">
					<div class="theme-welcome-inner">
						...
						?>
						 ...
						 <ul class="rockers-nav-tabs" role="tablist">
							<?php echo wp_kses_post($tabs_head); ?>
						 </ul>
						 
						 	<div class="rockers-tab-content">

			<?php do_action( 'rockers_info_screen' ); ?>
	...
	/**
		...
		public function changelog() {
			...
			?>
			<div id="changelog" class="rockers-tab-pane panel-close">
		...
		public function get_tabs_array() {
			...
			$tabs_array[]	= 	array(
					'link'      => 'changelog',
					'name'      => esc_html__('Changelog', 'rockers')
			);
		...
		public function get_recommended_plugins() {
			...
			$plugins = apply_filters('rockers_recommended_plugins', array());
			return $plugins;
		}

		public function get_useful_plugins() {
			$plugins = apply_filters('rockers_useful_plugins', array());
			return $plugins;
		}
		...
		public function get_recommended_actions() {
			...
			$act_count           = 0;
			$actions_todo = get_option( 'rockers_recommended_actions', array());
		...
		public function get_plugin_buttion($slug, $name) {
			...
			if (!$is_installed) {
				...
				$button_html        = sprintf('<a class="spicethemes-plugin-install install-now button-secondary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
					...
					sprintf(esc_html__('Install %s now', 'rockers'), esc_html($name)),
					esc_html($name),
					esc_html__('Install and activate', 'rockers')
				);
				...
				$button_html = sprintf('<a class="spicethemes-plugin-activate activate-now button-primary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
					...
					sprintf(esc_html__('Activate %s now', 'rockers'), esc_html($name)),
					esc_html($name),
					esc_html__('Activate', 'rockers')
				);
			} elseif ($is_activeted) {
				$button_html = sprintf('<div class="action-link button disabled"><span class="dashicons dashicons-yes"></span> %s</div>', esc_html__('Active', 'rockers'));
				$is_done     = true;
			}
		...
		public function update_recommended_actions_watch() {
			if (isset($_POST['action_id'])) {
				...
				update_option('rockers_recommended_actions', $actions_todo);
			}
			echo json_encode(get_option('rockers_recommended_actions'));
			wp_die();
		}
...
function rockers_useful_plugins_array($plugins){
	$plugins[] = array(
					'slug'     => 'elementor',
				);
...
add_filter('rockers_useful_plugins', 'rockers_useful_plugins_array');

function rockers_recommended_plugins_array($plugins){
	
		$plugins[] = array(
						'name'     => esc_html__('SpiceBox', 'rockers'),
						'slug'     => 'spicebox',
						'desc'     => esc_html__('To access the advance Frontpage sections and other theme features, please install the SpiceBox plugin.', 'rockers'),
					);
				
		$plugins[] = array(
					'name'     => esc_html__('Contact Form 7', 'rockers'),
					'slug'     => 'contact-form-7',
					'desc'     => esc_html__('To display the contact form on the contact section in homepage, please install the Contact Form 7 plugin', 'rockers'),
				);
	
	return $plugins;
}
add_filter('rockers_recommended_plugins', 'rockers_recommended_plugins_array');

function Rockers_About_Page() {
	return Rockers_About_Page::get_instance();
}
global $rockers_about_page;
$rockers_about_page = Rockers_About_Page();