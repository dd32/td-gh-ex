...
if (class_exists('WP_Customize_Control') && !class_exists('elitepress_Plugin_Install_Control')) {
	/**
	 *
	 * @see WP_Customize_Section
	 */
	class elitepress_Plugin_Install_Control extends WP_Customize_Control {
		/**
		 ...
		 * @var string
		 */
		public $type = 'elitepress_plugin_install_control';
		public $name = '';
		public $slug = '';
		...
		protected function render_content() {
			...
			if(!is_object($elitepress_about_page)){
				return;
			}?>
...
function elitepress_hide_customizer_notice(){
	if(isset($_POST['webriti_plugin_slug']) && !empty($_POST['webriti_plugin_slug'])){
		...
		update_option('elitepress_hide_customizer_notice_'.$plugin_slug, true);
		echo true;
	}
	wp_die();
}
add_action('wp_ajax_webriti_hide_customizer_notice', 'elitepress_hide_customizer_notice');