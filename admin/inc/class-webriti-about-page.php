...
if (!class_exists('Webriti_About_Page')) {
	...
	public function quality_style_and_scripts( $hook_suffix ) {
		...
		if ( 'appearance_page_quality-welcome' == $hook_suffix ) {
			...
			wp_enqueue_style( 'quality-info-css', get_template_directory_uri() . '/admin/assets/css/bootstrap.css' );
			...
			wp_enqueue_style('welcome_customizer', get_template_directory_uri() . '/admin/assets/css/welcome_customizer.css');
	...
	public function quality_scripts_for_customizer() {
		...
		public function welcome_admin_notice() {
			...
			printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'quality' ), esc_html( $theme_info->Name ), esc_html( $theme_info->Version ) ); ?>
				...
				<p><?php echo sprintf( esc_html__( "Welcome! Thank you for choosing Webriti's Quality WordPress theme. To take full advantage of the features this theme has to offer visit our %swelcome page%s.", "quality"), '<a href="' . esc_url( admin_url( 'themes.php?page=quality-welcome' ) ) . '">', '</a>' ); ?></p>
				<p><a href="<?php echo esc_url( admin_url( 'themes.php?page=quality-welcome' ) ); ?>" class="button button-blue-secondary button_quality" style="text-decoration: none;"><?php _e( 'Get started with Quality', 'quality' ); ?></a></p>
			</div>
			<style>
			.quality-notice {
	 ...
	 * 
	 */
	public function quality_recommended_actions() {
...
$quality_about_page = Webriti_About_Page();