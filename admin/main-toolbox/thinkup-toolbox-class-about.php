 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'alante_thinkup_about_page' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class alante_thinkup_about_page {
		/**
		 * Define the version of the class.
		 *
		 * @var string $version The alante_thinkup_about_page class version.
		 */
		...
		/**
		 * The single instance of alante_thinkup_about_page
		 *
		 * @var alante_thinkup_about_page $instance The  alante_thinkup_about_page instance.
		 */
		private static $instance;

		/**
		 * The Main alante_thinkup_about_page instance.
		 *
		 * We make sure that only one instance of alante_thinkup_about_page exists in the memory at one time.
		 *
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof alante_thinkup_about_page ) ) {
				self::$instance = new alante_thinkup_about_page;
		...
		function register() {
			if ( ! empty( $this->menu_name ) && ! empty( $this->page_name ) ) {
				add_theme_page( $this->menu_name, $this->page_name, 'activate_plugins', 'thinkup-welcome', array(
					$this,
					'alante_thinkup_about_page_render',
				) );
			}
		}
		...
		public function activation_admin_notice() {
			...
			add_action( 'admin_notices', array( $this, 'alante_thinkup_about_page_welcome_admin_notice' ), 99 );
		}

		/**
		 * Display an admin notice linking to the about page
		 */
		public function alante_thinkup_about_page_welcome_admin_notice() {
			if ( ! empty( $this->notification ) ) {

				// display notice if not previously dismissed
				if ( current_user_can( 'edit_theme_options' ) && !get_option( 'alante_thinkup_notice_welcome' ) ) {

					echo '<div class="thinkup-toolbox-about updated notice is-dismissible">';
					echo '<a class="notice-dismiss" href="' . esc_url(wp_nonce_url(remove_query_arg(array('activated'), add_query_arg('alante-thinkup-hide-notice', 'welcome')), 'alante_thinkup_hide_notices_nonce', '_alante_thinkup_notice_nonce')) . '">';
					echo '<span class="screen-reader-text">' . esc_html__('Dismiss this notice.', 'alante'). '</span>';
					echo '</a>';
		 ...
		 */
		public function alante_thinkup_about_page_render() {
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			if ( 'themes.php' === $pagenow || 'appearance_page_thinkup-welcome' == $hook_suffix ) {
				...
				wp_enqueue_style( 'alante-thinkup-about-page-css', get_template_directory_uri() . '/admin/main-toolbox/assets/css/thinkup-toolbox-backend.css' );
 ...
 */
if (!function_exists('alante_thinkup_hide_notice')) {
	function alante_thinkup_hide_notice() {
		if (isset($_GET['alante-thinkup-hide-notice']) && isset($_GET['_alante_thinkup_notice_nonce'])) {
			if (!wp_verify_nonce($_GET['_alante_thinkup_notice_nonce'], 'alante_thinkup_hide_notices_nonce')) {
				wp_die(esc_html__('Action failed. Please refresh the page and retry.', 'alante'));
			}
			if (!current_user_can('edit_theme_options')) {
				wp_die(esc_html__('You do not have the necessary permission to perform this action.', 'alante'));
			}
			$hide_notice = sanitize_text_field($_GET['alante-thinkup-hide-notice']);
			update_option('alante_thinkup_notice_' . $hide_notice, 1);
		}
	}
}
add_action('wp_loaded', 'alante_thinkup_hide_notice');