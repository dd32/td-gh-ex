<?php
	...
	function weaverx_cz_choices_site_layout() {
		return array (
			''          => '',
			'fullwidth' => esc_html__( 'Wide - Extend BG to full width', 'weaver-xtreme' ),
			'stretched' => esc_html__( 'Stretched - Expand to full width', 'weaver-xtreme' ),
			//'boxed' 		=> 	esc_html__( 'Boxed - Traditional constrained layout', 'weaver-xtreme' ),
			//'custom' 		=> 	esc_html__( 'Custom - Use Custom Full Width Options', 'weaver-xtreme' )
		...
		class WeaverX_XPlus_Control extends WP_Customize_Control {
			...
			public function render_content() {
				...
				echo '<span class="description customize-control-description">' .
					sprintf( weaverx_filter_text( __( '<strong>&#9733; <em>Add this setting!</em> Get %s</strong>', 'weaver-xtreme' ) ), $link ) .
					'</span>';
			}
		}
	endif;
		...
		if ( $root == 'tagline' || $root == 'content_h' || strpos( $root, '_title' ) > 0 ) {
			if ( $transport == 'refresh' ) {
				$opt[$root . '_normal'] = weaverx_cz_checkbox_refresh(
					esc_html__( 'Normal Weight for ', 'weaver-xtreme' ) . $label,
					''
				);
			} else {
