<?php if(has_post_thumbnail()) : ?>
		...
		<?php promax_post_image(); ?>
		<?php if ( of_get_option('promax_authstamp' ) =='on') { ?><span class="authmt"> <?php echo promax_meta_date(); echo promax_meta_author();?></span><?php } ?>
...
<?php } ?>  </a>
</div><?php if ( of_get_option('promax_authstamp' ) =='on') { ?><span class="authmt"> <?php echo promax_meta_date(); echo promax_meta_author();?></span><?php } ?>
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		<h2 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_title(); ?></a></h2>
		<div class="entry"><?php promax_excerpt('promax_excerptlength_index', 'promax_excerptmore'); ?></div></span>
	</article>
