...
function optimize_notice() {
	if ( isset( $_GET['activated'] ) ) {
		$return = '<div class="updated activation"><p><strong>';
					$my_theme = wp_get_theme();
		if ( isset( $_GET['previewed'] ) ) {
			$return .= sprintf( __( 'Settings saved and %s activated successfully.', 'optimize' ), $my_theme->get( 'Name' ) );
		} else {
...
add_action( 'admin_notices', 'optimize_notice' );

/*
 * Hide core theme activation message.
 */
function optimize_admincss() { ?>
	<style>
	.themes-php #message2 {
		display: none;
	}
...
<?php }
add_action( 'admin_head', 'optimize_admincss' );
