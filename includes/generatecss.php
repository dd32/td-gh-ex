...
function weaverx_fwrite_current_css() {
	...
	if (!$theme_dir_exists) {
        weaverx_f_file_access_fail('Unable to create directory. Probably a file system permission problem. Directory' . $save_dir);
	}
	$theme_dir_writable = $theme_dir_exists;

   if (!weaverx_f_is_writable($save_dir)) {
        weaverx_f_file_access_fail('Directory not writable. Probably a file system permission problem. Directory: ' . $save_dir);
...
// =========================== CONTENT AREAS ===============================
    ...
    if ( weaverx_getopt('restrict_img_border') )
        $img_tag = '.comment-content img[height],img[class*="align"],img[class*="wp-image-"],img[class*="attachment-"],img.featured-image,img.avatar';
    else
...
// ============================== CUSTOM OPTIONS =================================
    ...
    if ( ($val = weaverx_getopt('custom_fontfamily_a')) )
        weaverx_f_write($sout, '.customA-family{font-family:' . $val . ';}');
    if ( ($val = weaverx_getopt('custom_fontfamily_b')) )
        weaverx_f_write($sout, '.customB-family{font-family:' . $val . ';}');
...
$menu_bars = array (
    ...
    'm_extra_sub' => '.menu-extra .wvrx-menu ul li a,.menu-extra .wvrx-menu ul li a'
);
...
// =Menus ------------------------------------------------------------
    ...
    foreach ($menu_links as $id => $tag) {
        weaverx_put_color( $sout, $id . '_color', $tag );
        weaverx_put_bgcolor( $sout, $id . '_hover_bgcolor', $tag . ':hover' );
        weaverx_put_color( $sout, $id . '_hover_color', $tag . ':hover' );
    }
...
*/
    ...
    if ( $smart != 1 ) {
        ...
        weaverx_f_write( $sout, "\n");
        weaverx_f_write( $sout, ".smart-rm{margin-right:{$smart}%;}\n" );
    }
    ...
    $sb_areas = array ('primary' => '#primary-widget-area',
...
// custom widths for header, footer widget areas
    ...
    if ( $hide_tag == 'hide' || $hide_title == 'hide'
        ...
        weaverx_f_write( $sout, '#nav-header-mini{margin-top:32px!important;}' );
    }
...
// Colors need to go last because they might have CSS +
    ...
    $titles = array(
        'site_title' => '.wrapper #site-title a',
    ...
    foreach ( $titles as $title => $rule ) {
        ...
        if ( ( $val = (int)weaverx_getopt($title . '_underline_int') ) ) {
            $titleColor = weaverx_get_title_color($title . '_color');
    ...
    if ( ! $override ) {
        ...
        foreach ( $sb_horizontal_areas as $id => $tag ) {     // horizontal area widths
            ...
            if ( $w ) {
                ...
                weaverx_f_write( $sout, $ltag . '{width:' . $w . '%;}' . "\n");
            }
        }
    }
...
function weaverx_get_title_color( $title ) {
    ...
    $titleColor = weaverx_getopt($title);

	if ($titleColor == '') $titleColor = weaverx_getopt('content_color');   // content_color, etc, colors are generated by $area . '_color'
    if ($titleColor == '') $titleColor = weaverx_getopt('container_color');
    if ($titleColor == '') $titleColor = weaverx_getopt('wrapper_color');
