...
function optimize_core_style() {
	...
	wp_enqueue_style( 'optimize-responsive', $directory_uri . '/includes/core/bootstrap/css/responsive.css', array( 'bootstrap', 'bootstrap-responsive' ) );
...
add_action( 'admin_menu', 'optimize_add_core' );

// Define markup for the core page.
function optimize_display_core() {
			...
			<div id="core_container">
				<div class="row-fluid">
					<div id="core_header" class="span12">
						...
						<h3><?php _e( 'You Should Try Our Other Themes Also', 'optimize' ); ?></h3>
					</div>
				</div>
				...
				<div id="core_themes" class="row-fluid">
					...
					$themes = optimize_get_themes( $request );
					...
					foreach ( $themes->themes as $theme ) {
						if( $active_theme != $theme->name ) {
							...
							$theme_details = optimize_get_themes( $request );
							?>
...
// Get all optimize themes by using API.
function optimize_get_themes( $request ) {

	// Generate a cache key that would hold the response for this request:
	$key = 'optimize_' . md5( serialize( $request ) );
