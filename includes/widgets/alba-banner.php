...
class alba_banner extends WP_Widget {
	function __construct() {
		parent::__construct(
			'alba_banner',
			__( 'Alba Banner Heading', 'alba' ),
			array(
				'description' => __( 'Displays a banner heading.', 'alba' ),
			)
		);
	}
	...
	function form( $instance ) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'style' ) ?>"><?php _e( 'Banner Style', 'alba' ) ?></label>
            <select class="widefat alba-banner-select-style" name="<?php echo $this->get_field_name( 'style' ) ?>" id="<?php echo $this->get_field_id( 'style' ) ?>">
                <option value="none" <?php selected( $instance['style'], 'none' ) ?>><?php esc_html_e( 'None', 'alba' ) ?></option>
                <option value="bottom-shadow" <?php selected( $instance['style'], 'bottom-shadow' ) ?>><?php esc_html_e( 'Bottom Shadow', 'alba' ) ?></option>
                <option value="bg-color" <?php selected( $instance['style'], 'bg-color' ) ?>><?php esc_html_e( 'Background Color', 'alba' ) ?></option>
                <option value="bg-image" <?php selected( $instance['style'], 'bg-image' ) ?>><?php esc_html_e( 'Background Image', 'alba' ) ?></option>
            </select>
        </p>
        
        <p class="alba-banner-select-style-bgimage" style="display: none;">
            <label for="<?php echo $this->get_field_id( 'banner-bg-image' ) ?>"><?php _e( 'Banner Background Image Url', 'alba' ) ?></label>
            ...
            <span class="widgets-desc"><?php _e( 'Upload the image into the <a href="' . admin_url( 'upload.php' ) . '" target="_blank">Media Section</a>, then add the URL to the image here', 'alba' ) ?></span>
        </p>
        
        <p class="alba-banner-select-style-bgcolor" style="display: none;">
            <label for="<?php echo $this->get_field_id( 'banner-bg-color' ) ?>"><?php _e( 'Banner Background Color', 'alba' ) ?></label>
            ...
            <span class="widgets-desc"><?php _e( 'Enter the hash value of the color you want the background to be. Eg: #000000 (Don\'t forget the hash)', 'alba' ) ?></span>
        </p>
        
        <p>
            <label for="<?php echo $this->get_field_id( 'banner-full-width' ) ?>"><?php _e( 'Banner Full Width', 'alba' ) ?></label>
            <select class="widefat" name="<?php echo $this->get_field_name( 'banner-full-width' ) ?>" id="<?php echo $this->get_field_id( 'banner-full-width' ) ?>">
                <option value="off" <?php selected( $instance['banner-full-width'], 'off' ) ?>><?php esc_html_e( 'No', 'alba' ) ?></option>
                <option value="on" <?php selected( $instance['banner-full-width'], 'on' ) ?>><?php esc_html_e( 'Yes', 'alba' ) ?></option>
            </select>
        </p>
        
		<p>
			<label for="<?php echo $this->get_field_id( 'banner-heading' ) ?>"><?php _e( 'Banner Heading', 'alba' ) ?></label>
		    ...
		    <span class="widgets-desc"><?php _e( 'Add html <a href="http://www.tizag.com/htmlT/htmlbold.php" target="_blank">bold tags</a> around the words you want to take on the theme color', 'alba' ) ?></span>
        </p>
        
        <p>
            <label for="<?php echo $this->get_field_id( 'banner-text' ) ?>"><?php _e( 'Banner Text', 'alba' ) ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'button-link' ) ?>"><?php _e( 'Button Link', 'alba' ) ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'button-text' ) ?>"><?php _e( 'Button Text', 'alba' ) ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'button-target' ) ?>"><?php _e( 'Button Target', 'alba' ) ?></label>
			<select class="widefat" name="<?php echo $this->get_field_name( 'button-target' ) ?>" id="<?php echo $this->get_field_id( 'button-target' ) ?>">
				<option value="" <?php selected( $instance['button-target'], '' ) ?>><?php esc_html_e( 'None', 'alba' ) ?></option>
				<option value="_blank" <?php selected( $instance['button-target'], '_blank' ) ?>><?php esc_html_e( '_blank', 'alba' ) ?></option>
			</select>
		</p>
		<?php
	}
