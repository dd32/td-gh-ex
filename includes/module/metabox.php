...
if ( ! function_exists( 'best_business_add_theme_meta_box' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function best_business_add_theme_meta_box() {
...
add_action( 'add_meta_boxes', 'best_business_add_theme_meta_box' );

if ( ! function_exists( 'best_business_render_theme_settings_metabox' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function best_business_render_theme_settings_metabox() {
		...
		$values = get_post_meta( $post_id, 'best_business_settings', true );
		...
		<p><strong><?php esc_html_e( 'Choose Layout', 'best-business' ); ?></strong></p>
		<?php
		$dropdown_args = array(
			'id'          => 'best_business_settings_post_layout',
			'name'        => 'best_business_settings[post_layout]',
			'selected'    => $fields['post_layout'],
			'add_default' => true,
			);
		best_business_render_select_dropdown( $dropdown_args, 'best_business_get_global_layout_options' );
	}
...
if ( ! function_exists( 'best_business_save_theme_settings_meta' ) ) :
	...
	function best_business_save_theme_settings_meta( $post_id, $post ) {
		...
		if ( isset( $_POST['best_business_settings'] ) && is_array( $_POST['best_business_settings'] ) ) {
			$post_value = wp_unslash( $_POST['best_business_settings'] );

			if ( ! array_filter( $post_value ) ) {

				delete_post_meta( $post_id, 'best_business_settings' );
