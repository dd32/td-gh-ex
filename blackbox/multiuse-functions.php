 ...
 * @return void
 */
function aamla_markup( $context = '', $open = '<div%s>', $close = '</div>', $callback = '', $callback_args = '' ) {
	if ( ! $context ) {
		return;
	}

	// Short circuit filter.
	$check = apply_filters( "aamla_markup_{$context}", false, $context );
	if ( false !== $check ) {
		return;
	}

	printf( $open, aamla_get_attr( $context ) ); // WPCS xss ok.

	$hook = str_replace( '-', '_', $context );
	do_action( "aamla_inside_{$hook}", "inside_{$hook}" );
 ...
 */
function aamla_attr( $slug, $attr = array() ) {
	echo aamla_get_attr( $slug, $attr ); // WPCS xss ok.
}
 ...
 * @return string
 */
function aamla_get_attr( $slug, $attr = array() ) {
	$out = '';
 ...
 */
function aamla_icon( $args = array() ) {
	$icon_markup = aamla_get_icon( $args );
	if ( $icon_markup ) {
		echo $icon_markup; // WPCS xss ok.
	}
}
 ...
 */
function aamla_get_icon( $args = array() ) {
	 ...
	 *
	 * Example 1 with title: <?php echo aamla_get_svg( array( 'icon' => 'arrow-right', 'title' => __( 'This is the title', 'textdomain' ) ) ); ?>
	 *
	 * Example 2 with title and description: <?php echo aamla_get_svg( array( 'icon' => 'arrow-right', 'title' => __( 'This is the title', 'textdomain' ), 'desc' => __( 'This is the description', 'textdomain' ) ) ); ?>
	 *
 ...
 */
function aamla_nav_menu( $nav_id, $menu_id, $label, $location, $is_toggle = false, $args = array() ) {

	if ( $is_toggle ) {
		$toggle = sprintf(
			'<button aria-controls="%1$s" aria-expanded="false" class="menu-toggle">%2$s%3$s%4$s</button>',
			esc_attr( $menu_id ),
			aamla_get_icon( array( 'icon' => 'bars' ) ),
			aamla_get_icon( array( 'icon' => 'close' ) ),
			esc_html__( 'Menu', 'aamla' )
		);
	} else {
		$toggle = '';
	}
	...
	printf(
		'%1$s<nav id="%2$s" class="%3$s" aria-label="%4$s">%5$s</nav>',
		$toggle,
		esc_attr( $nav_id ),
		esc_attr( $nav_id ),
		esc_attr( $label ),
 ...
 * @param array  $widgets        Registered sidebar(s) to be displayed.
 * @param string $before_widgets Markup to be displayed before widgets.
 * @param string $after_widgets  Markup to be displayed after widgets.
 */
function aamla_widgets( $id, $class, $label, $widgets = array(), $before_widgets = '', $after_widgets = '' ) {

	// Short circuit filter.
	$check = apply_filters( "aamla_widgets_{$id}", false, $id );
	if ( false !== $check ) {
		return;
	}

	printf( '<aside id="%1$s" class="%2$s" area-label="%3$s">', esc_attr( $id ), esc_attr( $class ), esc_attr( $label ) );
	printf( '<h2 class="screen-reader-text">%s</h2>', esc_attr( $label ) );
	echo apply_filters( 'aamla_before_widgets', $before_widgets, $id );
	foreach ( $widgets as $widget ) {
		if ( is_active_sidebar( $widget ) ) {
			dynamic_sidebar( $widget );
		}
	}
	echo apply_filters( 'aamla_after_widgets', $after_widgets, $id );
	printf( '</aside>' );
}
 ...
 * to provide additional provision to change required template using filters.
 *
 * @since 1.0.0
 *
