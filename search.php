...
<section>
  <div class="impressive-container container">
      <div class="site-breadcumb"> 
		<?php if ( have_posts() ) : ?>        
			<h1><?php printf(/* translators: %s is search query  */ esc_html__( 'Search Results for: %s', 'impressive' ), get_search_query() ); ?></h1>
			<ol class="breadcrumb breadcrumb-menubar">
				<?php if (function_exists('impressive_custom_breadcrumbs')) impressive_custom_breadcrumbs(); ?>
			</ol>
		<?php else : ?>
				<h1><?php  esc_html_e('Nothing Found','impressive'); ?></h1>
		<?php endif; ?>	        
      </div>
	...
	<?php endwhile;  
		...
		<div class="blog-search-not-found">
			<h3>
			<?php esc_html_e('Sorry, but nothing matched your search terms. Please try again with some different keywords.','impressive'); ?>
			</h3>
			 <?php get_search_form(); ?>
		</differentiv>	 
	<?php endif; ?>			 
