...
<div id="content">
	...
	<?php if ( have_posts() ) : ?>
		...
		<h4 class="page-title"><?php printf( __( 'Search Results for: %s', 'leftside' ), get_search_query() ); ?></h4>
		...
		<?php while ( have_posts() ) : the_post(); ?>
			...
			<div class="postmetadata">
				...
				<?php if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) : ?>
					<?php echo '|'; ?> <?php comments_popup_link( __( 'Leave a response', 'leftside' ), __( '1 response', 'leftside' ), __( '% responses', 'leftside' ) ); ?>
				<?php endif; ?>
			</div>
		...
		<div class="post-nav">
			<?php next_posts_link(__( 'Older posts &raquo;', 'leftside' )); ?>
			<?php previous_posts_link(__( '&laquo; Newer posts', 'leftside' )); ?>
		</div>
