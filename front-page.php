<?php get_header(); ?>
	...
	<?php if ( !is_page() ) {
		/*
		 * Add hidden text to provide page structure. Page structure helps non-sighted users navigate the page, but also informs screen readers about how your information relates.
		 */
	?>
		...
		<?php }
			...
			<?php } else { ?>
				...
				<div <?php post_class( 'post-content' ); ?> id="post-<?php the_ID(); ?>">
					...
					echo $post_link;
					edit_post_link( sprintf( __( 'Edit %s', 'universal' ), get_the_title() ), '<p class="edit">', '</p>' );
					?>
				</div>
