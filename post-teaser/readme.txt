...
**4.0.1:**
* Fix minor issue that cause PostTeaser activation in certain hosting environment to fail.

**4.0.0:**

* Fix crashes when post contain `<!--more-->` tag.
* Reintroduce preview of template in PostTeaser options page (broken since Wordpress 2.3.x).
* Fix PostTeaser options page format (broken since WordPress 2.3.x).
* Enhance PostTeaser options page user interface.
* Add new option to put a PostTeaser block at the end of the post text like the "Read the rest of this entry »" created by the "<!--more-->" tag but more flexible.
* Add new control to turn on or off tease message.

= Known issue(s): =

* Current translation files will not have translation for new options introduced in this release. Apologies to Non-English users of PostTeaser. This problem will exist and will break the translation files further in up coming release due to heavy remodeling the PostTeaser admin page. For translators, please hold off any effort to fix it immediately until I get the admin pages in order.
* Upon upgrading to PostTeaser 4.0.x, the stats and the premalink will be gone by default. The reason is the new options created to control the tease message and since it was not there in the previous versions of PostTeaser, none of it will be set. Make sure you reconfigure PostTeaser once you have updated to PostTeaser 4.0.x.

= Incompatibilities: =

* Search Unleashed (0.2.29) - Causes edit post save/update/publish to malfunction but does not affect the actual post saving/updating/publishing when activated together with PostTeaser. *Note that the actual post saving/updating/publishing is not affected, just the page it shows after that is wrong.*
...
1. Have Post Teaser "display the post in full", and display the message that it did so (like what happens if you have a post that is too short to be teasered)
