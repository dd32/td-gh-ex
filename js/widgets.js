...
jQuery(function($){
      ...
      jQuery(document).on('click', 'input.select-img', function( event ) {
      	...
      	var BestBusinessCustomThemeImage = wp.media.controller.Library.extend({
      		defaults : _.defaults({
        ...
        best_business_file_frame = wp.media.frames.best_business_file_frame = wp.media({
        	button: {
        		text: jQuery( this ).data( 'uploader_button_text' )
        	},
        	state : 'custom-theme-insert-image',
        	states : [
        	new BestBusinessCustomThemeImage()
        	],
        	multiple: false
        });

        // When an image is selected, run a callback.
        best_business_file_frame.on( 'select', function() {

        	var state = best_business_file_frame.state('custom-theme-insert-image');
