( function( $ ) {
	...
	var $style = $( '#antonine-colour-scheme-css' ),
	api = wp.customize;

	if ( ! $style.length ) {
		$style = $( 'head' ).append( '<style type="text/css" id="antonine-colour-scheme-css" />' )
	...
	api( 'site_post_background_colour', function( value ) {
		value.bind( function( to ) {
			...
			$( '.content-holder').css("background-color", to);
			$( '.read-more-holder').css("background-color", to);
	...
	api( 'site_alltext_colour', function( value ) {
		value.bind( function( to ) {
			$( 'html body' ).css("color", to );
			$( 'html .home article' ).css("color", to );
		} );
	} );
