...
class CSSOriginVariable extends CSSPrimitiveValue {
	...
	/**
	 * @var bool Are we in Javascript mode?
	 */
	private $jsmode = false;
	
	/**
	 * @var string The processed variable
	 */
	...
	/**
	 * Substitutes values from the values array into the CSS.
	...
	public function substitute($values){
		...
		if(empty($m)) {
			if(!isset($values[$v])) $values[$v] = false;
			$processed = $values[$v];
		}
		else {
			if(!isset($values[$v][$m])) $values[$v][$m] = false;
			$processed = $values[$v][$m];
		}
		
		if(is_array($processed)) $processed = serialize($processed);
	...
	/**
	 * @param bool $jsmode
	 * @internal param bool $mode If set to true, __toString() will return Javascript code.
	 */
