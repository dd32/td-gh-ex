jQuery(function($){
    ...
    if(window.location.hash != ''){
        start = $('#origin-options-page .nav-tab-wrapper a[href="'+window.location.hash+'"]').index();
    }
    
    // Start by setting up the tabs
    $('#origin-options-page .nav-tab-wrapper a').click(function(){
        var $$ = $(this);
        var id = $$.attr('href').substr(1);
        $('.options-page-container').hide().filter('#page-'+id).show();
        $('#origin-options-page .nav-tab-wrapper a')
            .removeClass('nav-tab-active');
        $$.addClass('nav-tab-active');
        $('#origin-options-form').attr('action', window.location.href.split('#')[0] + $$.attr('href'));
    }).eq(start).click();
    
    $('.origin-options .type-taxonomy_select select, .origin-options .type-post_select select').each(function(){
        var $$ = $(this);
        var order;
        ...
        if($$.attr('data-order') != undefined){
            ...
            $('form#origin-options-form').submit(function(){
                var order = [];
    ...
    $('.origin-options .type-select select').chosen({
        disable_search_threshold: 6,
        allow_single_deselect: true
    });
